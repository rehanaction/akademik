<?php
	...
	class uForm {
		...
		function getLabel($data,$value,$key = false) {
			...
			if($data['type'] == 'D')
				...
				if($data['type'][1] == 'I')
					...
					if ($key)
						$value = '<u class="ULink" onclick="goDownload(\''.$data['uptype'].'\',\''.$key.'\')">'.$value.'</u>';
					else
				...
				if ($key)
					$value .= '<u class="ULink" onclick="goDeleteFile(\''.$nameid.'\',\''.$key.'\')">Hapus file</u>';
				else
		...
		function getInput($data,$value=null,$br=false) {
			if($data['readonly'])
			...
			switch($data['type'][0]) {
				case 'A':
					...
					break;
				case 'C':
					$input = UI::createCheckBox($nameid,$option,$value,true,$br,$add);
					break;
				case 'D':
					ob_flush();
...
<?php
		...
		function getPathImageMahasiswa($conn,$key,$show=false) {
			...
			// buat direktori
			if(!$show and !file_exists($dir.$angkatan))
				mkdir($dir.$angkatan,0755,true);
			
			$file = $dir.$angkatan.'/'.$key.'.jpg';
		...
		function getImageMahasiswa($conn,$key,$edit=true,$add='') {
			if($edit) {
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImageMahasiswa($conn,$key,true).'" style="cursor:pointer" '.$add.'>';
				$img .= self::getImageInput();
			}
			else
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImageMahasiswa($conn,$key,true).'" '.$add.'>';
			
			return $img;
		}
		
		function getPathImageSKPI($conn,$key,$show=false) {
			// ambil periode masuknya
			$debug = $conn->debug;
			$conn->debug = false;
		...
		function getImageSKPI($conn,$key,$edit=true,$add='') {
			if($edit) {
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImageSKPI($conn,$key,true).'" style="cursor:pointer" '.$add.'>';
				$img .= self::getImageInput();
			}
			else
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImageSKPI($conn,$key,true).'" '.$add.'>';
			
			return $img;
		}
		
		function reloadImageSKPI($conn,$key,$alert=false) {
?>
			<html>
				<body>
					...
					<script type="text/javascript">
						...
						img.attr("src","<?= self::getPathImageSKPI($conn,$key,true) ?>");
						img.hideWait();
