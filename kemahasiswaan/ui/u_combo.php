<?php
	...
	class uCombo {
		...
		function tingkatPrestasi($conn,&$valvar,$nameid='tingkatprestasi',$add='',$empty=true) {
			$data = mCombo::tingkatPrestasi($conn);
			
			return self::combo($data,$valvar,$nameid,$add,$empty,'Tingkat Prestasi');
		}		
		function jenisMhs() {
			$data = array('' =>'Semua','0'=>'Reguler','2'=>'Transfer');
			
			return UI::createSelect('jenisMhs',$data,'','ControlStyle');
		}
		...
		function jenisbeasiswa($conn,&$valvar,$nameid='jenisbeasiswa',$add='',$empty=true) {
			$data = mCombo::jenisbeasiswa($conn);
			
			return self::combo($data,$valvar,$nameid,$add,$empty,'Jenis Beasiswa');
		}
		
		function sumberbeasiswa($conn,&$valvar,$nameid='sumberbeasiswa',$add='',$empty=true) {
			$data = mCombo::sumberbeasiswa($conn);
			
			return self::combo($data,$valvar,$nameid,$add,$empty,'Sumber Beasiswa');
		}
		...
		function periode($conn,&$valvar,$nameid='periode',$add='',$empty=true) {
		...
		function roleMhs($conn,&$valvar,$nameid='role',$add='',$empty=true) {
			$data = mCombo::roleMhs($conn);
		...
		function listStatusTransfer($nameid='mhstransfer',$value='',$add='') {
			$data = array('1'=>'Transfer Dalam Universitas','2'=>'Transfer Dari Luar Universitas');
