<?php
	...
	function getDistribusidosenwali($conn,$jurusan,$semester,$tahun) {
		$sql = "SELECT w.nipdosenwali,count(m.nama) as jumlah,akademik.f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) AS namadosenwali
                ...
                LEFT JOIN sdm.ms_pegawai p on w.nipdosenwali=p.idpegawai::text
                JOIN akademik.ms_mahasiswa m ON w.nim = m.nim
                JOIN gate.ms_unit u ON m.kodeunit = u.kodeunit
				Where u.kodeunit = '$jurusan'
	...
	function getStatistikKRS($conn,$kodeunit,$r_periode) {
		...
		$sql="select j.kodeunit,j.namaunit,count(p.nim) as jumlahmhs,count(case when p.prasyaratspp!=0 then p.nim end) as mhsbayar,
	...
	function getRekapMengajar($conn,$kodeunit,$periode,$periodegaji,$sistemkuliah='',$nopengajuan='',$nip=''){
		...
		$sql="select k.tglkuliah , k.perkuliahanke , k.periode , k.thnkurikulum , k.kodeunit , k.kodemk , k.kelasmk , k.jeniskuliah , k.kelompok,
				...
				k.waktumulairealisasi,k.waktuselesairealisasi,g.honordosen,g.nopengajuan,g.nopembayaran,k.validhonorkuliah,g.validhonor,k.isonline,mj.tugasmengajar
	...
	function getPrestasiByMhs($conn,$nim){
		...
		$sql="	select p.*,namajenisprestasi , namatingkatprestasi , namakategoriprestasi, (case when p.isvalid='-1' then coalesce(pp.poin,0) else 0 end) as poin
				...
				left join kemahasiswaan.ms_poinprestasi pp on p.kodejenisprestasi = pp.kodejenisprestasi and p.kodetingkatprestasi = pp.kodetingkatprestasi and p.kodekategoriprestasi = pp.kodekategoriprestasi and p.kodejenispeserta=pp.kodejenispeserta
				where nim = '$nim' and p.istampil = -1 order by isvalid ";

		return  $conn->getArray($sql);
	}
	function getAktivitasByMhs($conn,$nim){
		$sql = " select p.idpengalaman,p.periode,p.nim,p.kodekegiatan,p.tglpengajuan, p.tglkegiatan,p.namakegiatan,p.keterangan,p.isvalid,
						p.tglvalidasi,p.penyelenggara,p.jenisaktivitas, sk.namakegiatan as namakodekegiatan, kp.poinkegiatan 
						from kemahasiswaan.mw_pengalamanmhs p 
						join akademik.ms_mahasiswa m using (nim)
						join kemahasiswaan.ms_strukturkegiatan sk on sk.kodekegiatan::text = p.kodekegiatan::text
						join akademik.ak_prodi pr on pr.kodeunit = m.kodeunit
						left join kemahasiswaan.ms_strukturkegiatanpoin kp on kp.periode::text = p.periode::text and kp.kodekegiatan::text = p.kodekegiatan::text and kp.kodejenjang = pr.kode_jenjang_studi
						";
						
		$sql .=" where nim = '$nim' and p.istampil = -1 order by p.isvalid";
	...
	*/
	...
	function getRekapKlaim($conn,$idunit,$tglawal,$tglakhir){
		...
		$sql = "select a.isditerima,a.isvalid,m.kodeunit,j.namaunit,m.nim,m.nama,j.namaunit
		...
		$sql .=" join akademik.ms_mahasiswa m using (nim)
				...
				left join gate.ms_unit j on j.kodeunit=m.kodeunit
		...
		$rs = $conn->Execute($sql);
		while($row = $rs->fetchRow()){
				$a_data['sum'][$row['kodeunit']]['namaunit'] = $row['namaunit'];
				$a_data['sum'][$row['kodeunit']]['kodeunit'] = $row['kodeunit'];
			if (!empty ($row['isvalid'])){
				$a_data['sum'][$row['kodeunit']]['daftarklaim'] += 1;
				
				if (!empty ($row['isditerima']))
				$a_data['sum'][$row['kodeunit']]['jumlahditerima'] += 1;
				
				$a_data['detail'][$row['kodeunit']][] = $row;
			}
		}
	...
	function getKlaim($conn,$idunit,$tglawal,$tglakhir){
		require_once(Route::getModelPath('unit'));
		$unit = mUnit::getData($conn,$idunit);

		$sql = "select nopolis,nama,tglpengajuan,jumlahklaim,tglkejadian,a.keterangan,a.isvalid,a.isditerima
				from kemahasiswaan.mw_klaimasuransi a
				join kemahasiswaan.ms_asuransimhs ma on a.idasuransimhs = ma.idasuransimhs ";
	//if( (!empty($tglawal)) and (!empty($tglakhir)) )
			//$sql .= " and tglpengajuan between '$tglawal'::date and '$tglakhir'::date ";
	...
	function getPolismhs($conn,$nim){
		$sql = "select nim, m.nama, u.namaunit as prodi, f.namaunit as fakultas, namajenisasuransi, namaasuransi, nopolis, case when s.isaktif = -1 then 'Aktif' else 'Nonaktif' end as isaktif, waktudaftar
		...
		$data = array();
		foreach ($rs as $val) {
			$data[$val['nim']]['nim'] = $val['nim'];
			$data[$val['nim']]['nama'] = $val['nama'];
			$data[$val['nim']]['prodi'] = $val['prodi'];
			$data[$val['nim']]['fakultas'] = $val['fakultas'];
			$data[$val['nim']]['asu'][] = $val;
		}
		return $data;
	}
	...
	function getSKPI($conn,$nim) {
		//$conn->debug = true;
			// data mahasiswa
			/*
			$sql = "select m.nim, m.nama, m.tmplahir, m.tgllahir,m.tglregistrasi,m.mhstransfer,
					...
					p.namaunit as namafakultas,penguasaanpengetahuan,penguasaanpengetahuanen,sikapkhusus,sikapkhususen,
			...
			*/
			$sql = "select m.nim, m.nama, m.tmplahir, m.tgllahir,m.tglregistrasi,m.mhstransfer,
					...
					p.ketua as nipdekan, akademik.f_namalengkap(k.gelardepan,k.namadepan, k.namatengah, k.namabelakang,k.gelarbelakang) as dekan,
					p.namaunit as namafakultas,penguasaanpengetahuan,penguasaanpengetahuanen,sikapkhusus,sikapkhususen,
					formatnomor,tglskpi,skp.namauniten
					from akademik.ms_mahasiswa m
					left join gate.ms_unit u on m.kodeunit = u.kodeunit
					left join akademik.ak_prodi pr on m.kodeunit = pr.kodeunit
					left join gate.ms_unit p on p.kodeunit = u.kodeunitparent
		...
		// get pengalaman untuk SKPI, dapet dari query list_pengalaman
		function getPengalaman($conn,$nim){
			$sql = "select idpengalaman,p.periode, p.nim, m.nama, namakegiatan, namakegiatanen, p.tglpengajuan, isvalid, p.kodekategori
			from kemahasiswaan.mw_pengalamanmhs p 
			join akademik.ms_mahasiswa m on p.nim = m.nim 
			join akademik.ak_prodi pr on pr.kodeunit = m.kodeunit 
			...
			where istampil='-1' and isvalid='-1' and p.nim = ".Query::escape($nim);

			return  $conn->GetArray($sql);
		}
		// get KKN untuk SKPI
		function getKKN($conn,$nim){
			$sql = "select a.idkp,b.judulkp,b.judulkpen 
			from akademik.ak_pesertakp a
			left join akademik.ak_kp b on a.idkp=b.idkp
			where a.nim= ".Query::escape($nim);

			return  $conn->GetArray($sql);
		}
		// get Skripsi untuk SKPI
		function getSKRIPSI($conn,$nim){
			$sql = "select * from akademik.r_tadetail where nim =".Query::escape($nim);

			return  $conn->GetArray($sql);
		}
		// get Prestasi untuk SKPI
		function getPrestasi($conn,$nim){
			$sql = "select idprestasi, namaprestasi, namaprestasien, isvalid, tglprestasi 
			from kemahasiswaan.ms_prestasimhs p
			where istampil='-1' and isvalid='-1' and nim=".Query::escape($nim);
		...
		function getPoinByPeriode($conn,$idunit,$periode,$nim){
			require_once(Route::getModelPath('unit'));
			$unit = mUnit::getData($conn,$idunit);

			$sql = "select m.nim, m.nama,sum(ppr.poin) as prestasi, sum(poinkegiatan) as kegiatan,sum(poinpelanggaran) as pelanggaran
			from akademik.ms_mahasiswa m
			left join gate.ms_unit u on m.kodeunit = u.kodeunit
			left join gate.ms_unit up on u.kodeunitparent = up.kodeunit
			join akademik.ak_perwalian p on m.nim = p.nim and p.periode = '$periode' and p.statusmhs in ('A')
			left join (
				select pr.nim,po.poin
				from kemahasiswaan.ms_prestasimhs pr
				left join kemahasiswaan.ms_poinprestasi po
				using (kodejenisprestasi,kodetingkatprestasi,kodekategoriprestasi,kodejenispeserta)
				where isvalid = '-1' ) ppr on m.nim = ppr.nim
			left join (
				select pr.nim, poinkegiatan
				from kemahasiswaan.mw_pengalamanmhs pr
				join akademik.ms_mahasiswa m using(nim)
				join akademik.ak_prodi u using(kodeunit)
				left join kemahasiswaan.ms_strukturkegiatanpoin po
				on pr.kodekegiatan = po.kodekegiatan::varchar and pr.periode = po.periode and u.kode_jenjang_studi = po.kodejenjang
				where isvalid = '-1'";
				if(!empty($periode))
					$sql .=" and pr.periode = '$periode' ";
				$sql .= " ) ppa on m.nim = ppa.nim
			left join kemahasiswaan.mw_pelanggaranmhs ppl on m.nim = ppl.nim ";
			if(!empty($periode))
				$sql .=" and ppl.periode = '$periode' ";
			$sql .=" where 1=1 ";
			if(!empty($idunit))
				$sql .=" and (u.infoleft >= ".(int)$unit['infoleft']." and u.inforight <= ".(int)$unit['inforight']." ) "  ;

			$sql .=" group by m.nim  order by nim ";

			return $conn->GetArray($sql);
		}

		function getDataTranskripSoftskill($conn,$nim){
			$sql = "select f.namaunit as namafakultas,m.tgllahir,m.tmplahir,nim,m.nama as namamhs,u.namaunit as namaprodi,a.namakegiatan,tglkegiatan,a.periode,sk.namakegiatan as peran, pk.namakegiatan as tingkatkegiatan, jenisaktivitas,skp.poinkegiatan,tglkegiatanakhir 
				from kemahasiswaan.mw_pengalamanmhs a
				join akademik.ms_mahasiswa m using (nim)
				join gate.ms_unit u using (kodeunit)
				join akademik.ak_prodi p on u.kodeunit = p.kodeunit
				join kemahasiswaan.ms_strukturkegiatan sk on sk.kodekegiatan = a.kodekegiatan::int
				left join kemahasiswaan.ms_strukturkegiatan pk on pk.kodekegiatan = sk.parentkodekegiatan
				join kemahasiswaan.ms_strukturkegiatanpoin skp on skp.kodekegiatan = a.kodekegiatan::int and a.periode = skp.periode and skp.kodejenjang = p.kode_jenjang_studi
				join gate.ms_unit f on f.kodeunit = u.kodeunitparent
				where a.nim = '$nim' and a.isvalid <> '0'";
			$a_data = $conn->GetAll($sql);

			return $a_data;
		}

		function getDataTranskripSoftskillPrestasi($conn,$nim){
			$sql = "select a.penyelenggara, nim,m.nama as namamhs,a.namaprestasi,tglprestasi,u.namaunit as namaprodi,kodejenispeserta,namajenispeserta,sk.poin,namakategoriprestasi,namatingkatprestasi,tglprestasiakhir from kemahasiswaan.ms_prestasimhs a
				join akademik.ms_mahasiswa m using (nim)
				join gate.ms_unit u using (kodeunit)
				join kemahasiswaan.ms_poinprestasi sk using (kodejenisprestasi, kodetingkatprestasi, kodekategoriprestasi, kodejenispeserta)
				join kemahasiswaan.lv_jenispeserta using (kodejenispeserta)
				join kemahasiswaan.lv_kategoriprestasi using (kodekategoriprestasi)
				join kemahasiswaan.lv_tingkatprestasi using (kodetingkatprestasi)
				where a.nim = '$nim' and a.isvalid <> '0'";
