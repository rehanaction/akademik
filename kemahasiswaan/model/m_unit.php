<?php
	...
	class mUnit extends mModel {
		...
		function listQuery() {
			$sql = "select u.*, akademik.f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as nama, up.namaunit as namaparent 
					from ".static::table()." u
		...
		// mendapatkan nama ka biro
		function getNamaKaBiro($conn) {
			/* $sql = 	  "select  
				       concat_ws(' ', gelardepan, namadepan, namatengah, namabelakang, gelarbelakang) as nama 
				       from ".static::table()." u 
				       left join sdm.ms_pegawai p 
				       		on u.birokmhs = p.idpegawai :: text
				       where kodeunit = '20000000'"; */
			
			$sql = "select namakabiro from kemahasiswaan.ms_setting where idsetting = 1";
		...
		function getFakJur($conn,$a_kolom,$r_sort,$a_filter) {
			$sql = "select level,kodeunit, namaunit,iskrs,ispenilaian,tglmulainilai,tglakhirnilai,jmlminimalseminar ,jmlminimalpoin from gate.ms_unit where level >= 1 and isakad=-1";
