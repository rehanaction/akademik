<?php
	...
	class mPegawai extends mBiodata {
		...
		function listQuery() {
			$sql = "select a.namaagama,r.*,akademik.f_namalengkap(r.gelardepan,r.namadepan,r.namatengah,r.namabelakang,r.gelarbelakang) as nama,u.namaunit,pk.namapangkat,s.jabatanstruktural, f.jabatanfungsional from ".self::table()." r 
		...
		function getListPendidikan($conn,$nik) {
			$sql = "select lj.namapendidikan, rp.* from ".static::table('pe_rwtpendidikan')." rp left join sdm.ms_pegawai mp on mp.idpegawai=rp.idpegawai 
					left join sdm.lv_jenjangpendidikan lj on lj.idpendidikan=rp.idpendidikan where (mp.idpegawai::text = '$nik') and isvalid='Y' order by nourutrpen";
			$rs = $conn->Execute($sql);
		...
		function insertPendidikan($conn,$record) {
			return Query::recInsert($conn,$record,static::table('pe_pendidikan'));
		}
		...
		function getListPangkat($conn,$nip) {
			$sql = "select * from ".static::table('pe_rwtpangkat')." pr left join sdm.ms_pegawai p on p.idpegawai=pr.idpegawai 
					left join sdm.ms_pangkat mp on mp.idpangkat=pr.idpangkat where (idpegawai::text='$nip') and isvalid='Y' order by nourutrp";
			$rs = $conn->Execute($sql);
		...
		function getListJabatan($conn,$nip) {
			...
			$sql = "select * from ".static::table('pe_rwtstruktural')." pr left join sdm.ms_pegawai p on p.idpegawai=pr.idpegawai 
					...
					left join sdm.ms_jenispejabat mj on mj.idjnspejabat=pr.idjnspejabat where (idpegawai::text = '$nip') and isvalid='Y' order by tmtmulai";
			$rs = $conn->Execute($sql);
		...
		function getListPenghargaan($conn,$nip) {
			$sql = "select * from ".static::table('pe_penghargaan')." pp left join sdm.ms_pegawai p on p.idpegawai=pp.idpegawai 
					left join sdm.lv_penghargaan lp on lp.idpenghargaan=pp.idpenghargaan where (idpegawai::text = '$nip') order by nourutpenghargaan";
			$rs = $conn->Execute($sql);
		...
		function getListSuamiIstri($conn,$nip) {
			$sql = "select pi.tgllahir as tgllahir2,pi.* from ".static::table('pe_istrisuami')." pi left join sdm.ms_pegawai p on p.idpegawai=pi.idpegawai 
					where idpegawai::text = '$nip' order by nourutist";
			$rs = $conn->Execute($sql);
		...
		function getListAnak($conn,$nip) {
			$sql = "select pa.tgllahir as tgllahiranak,pa.* from ".static::table('pe_anak')." pa left join sdm.ms_pegawai p on p.idpegawai=pa.idpegawai 
					where idpegawai::text='$nip' order by nourutanak";
			$rs = $conn->Execute($sql);
		...
		function getArray($conn,$unit='') {
			if(!empty($unit)) {
				...
				$sql = "select p.idpegawai, akademik.f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang)||' ('||p.idpegawai::text||')' as nama 
						from ".static::table()." p 
				...
				$sql = "select idpegawai, akademik.f_namalengkap(gelardepan,namadepan,namatengah,namabelakang,gelarbelakang)||' ('||idpegawai::text||')' as nama from ".static::table()." order by nama";
		...
		function findDosen($conn,$str,$col='',$key='') {
			...
			$sql = "select $key, $col as label from ".static::table()."
					where isdosen=-1 and lower($col::varchar) like '%$str%' order by ".static::order;
