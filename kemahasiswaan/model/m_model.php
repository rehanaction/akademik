<?php
	...
	class mModel {
		...
		function dataQuery($key) {
			...
			// $sql = "select * from sdm.v_pegawai where ".static::getCondition($key);

			return $sql;
		}

		// mendapatkan kolom
		function getDataField($conn,$key,$field) {
			$sql = "select $field from ".static::table()." where ".static::getCondition($key);
		...
		// menemukan data, untuk autocomplete keperluan sim kemahasiswaan
		function findMhsBeasiswa($conn,$str,$col='',$key='') {
			global $conf;
			...
			$sql = "select $key, $col as label from ".static::table()."
					where lower($col::varchar) like '%$str%' and statusmhs in('A','C','T') order by ".static::order;
