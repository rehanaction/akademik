<?php
	...
	class mUser extends mModel {
		...
		function getAksesMhs($conn){
			$sql =" select  u.*,nim||' - '||nama as nim,namarole as koderole, namaunit as kodeunit 
					 from gate.sc_user u 
					 join akademik.ms_mahasiswa m on u.username = m.nim
					 join gate.sc_userrole ur on ur.userid = u.userid 
					 join gate.sc_role r on ur.koderole = r.koderole and kemahasiswaan = 1 
					 join gate.ms_unit un on ur.kodeunit = un.kodeunit ";
		
			$data =  $conn->GetArray($sql);
			return $data;
		}
		
		function deleteAkses($conn,$table,$condition){
