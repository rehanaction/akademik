...
//	$conn->debug = true;
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'nim', 'label' => 'Mahasiswa', 'type' => 'X', 'text' => 'nim_mhs','size'=>'30');
	$a_kolom[] = array('kolom' => 'koderole', 'label' => 'Role', 'type' => 'S', 'option' => mCombo::roleMhs($conn));
	$a_kolom[] = array('kolom' => 'kodeunit', 'label' => 'Unit', 'type' => 'S', 'option' => mCombo::unit($conn,false));
	
	// properti halaman
	$p_title = 'Setting Akses Organisasi';
	$p_tbwidth = 760;
	$p_aktivitas = 'BIODATA';
	...
	if($r_act == 'insert' and $c_insert) {
		...
		$record = array('userid' => $_POST['i_nim'],'koderole' => $_POST['i_koderole'],'kodeunit' => $_POST['i_kodeunit']);
		
		list($p_posterr,$p_postmsg) = $p_model::insertCRecordDetail($conn,$a_kolom,$record,'role');
		//var_dump($record);die;
		...
		$r_key = CStr::removeSpecial($_POST['key']);
		list($user,$role) = explode('|',$r_key);
		
		list($p_posterr,$p_postmsg) = $p_model::deleteAkses($conn,'gate.sc_userrole',"userid = $user and koderole='$role' ");
	}
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<?		}
								if($c_delete) { ?>
						<td align="center"><img id="<?= $t_key.'|'.$row['koderole'] ?>" title="Hapus Data" src="images/delete.png" onclick="goDelete(this)" style="cursor:pointer"></td>
						<?		} ?>
					</tr>
					<?		}
						}
...
$(document).ready(function() {
	$("#nim_mhs").xautox({strpost: "f=acmahasiswauser", targetid: "i_nim"});
});
</script>
<script type="text/javascript">
