<?php
	...
	$a_kolom[] = array('kolom' => 'tglmulaiakhirnilai','label' => 'Jumlah Seminar');
	
	// ada submit
	$r_act = $_POST['act'];
	if($r_act == 'simpanperiodenilai' and $c_edit) {
		$record=array();
		$record['jmlminimalseminar']=$_POST['jmlminimalseminar_'.$_POST['kodemk']];
		$record['jmlminimalseminar']=$_POST['jmlminimalseminar_'.$_POST['kodemk']];
				...
				<table width="<?= $p_tbwidth+300 ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td nowrap valign="bottom" align="center">
							<?php if($row['level']==2) { 
							?>
								<input type="text" name="jmlminimalseminar_<?= $row['kodeunit']?>" id="jmlminimalseminar_<?= $row['kodeunit']?>" class="ControlStyle" maxlength="10" size="10" value="<?= $row['jmlminimalseminar']?>">
