<?php
	...
	// combo
	$r_tanggal = Modul::setRequest($_POST['tanggal'],'PRESTASI.TANGGAL');
	$r_sdtanggal = Modul::setRequest($_POST['sdtanggal'],'PRESTASI.SDTANGGAL');
	
	$a_tingkat = array('' => '-- Semua Tingkat --') + mTingkatPrestasi::getArray($conn);
	$r_tingkat = Modul::setRequest($_POST['tingkatprestasi'],'TINGKATPRESTASI',$a_tingkat);
	$l_tingkat = UI::createSelect('tingkatprestasi',$a_tingkat,$r_tingkat,'ControlStyle',true,'onchange="goSubmit()"');
	...
	$a_kolom[] = array('kolom' => 'namajenisprestasi', 'label' => 'Jenis Prestasi');
	$a_kolom[] = array('kolom' => 'namaprestasi', 'label' => 'Nama Prestasi');
	$a_kolom[] = array('kolom' => 'tglprestasi', 'label' => 'Tanggal');
	$a_kolom[] = array('kolom' => 'poin', 'label' => 'Poin');
	$a_kolom[] = array('kolom' => 'p.isvalid', 'label' => 'Valid');
	...
	//var_dump(Modul::getUserName());
	if(Akademik::isMhs()) {
		$a_filter[] = $p_model::getListFilter('p.nim',Modul::getUserName());
	}
	
	// mendapatkan data
	if(!empty($r_tingkat)) $a_filter[] = $p_model::getListFilter('p.kodetingkatprestasi',$r_tingkat);
	...
	$a_filtercombo = array();
	$a_filtercombo[] = array('label' => 'Tingkat Prestasi', 'combo' => $l_tingkat);
