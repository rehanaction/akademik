<?php
	...
	class Page {
		...
		// mendapatkan tr data
		function getDataTR($row,$field,$separator=' ',$inputonly=false) {
			$tr = '<tr>'."\n";
			$tr .= '<td class="LeftColumnBG" width="120" style="white-space:nowrap">'."\n";
			$tr .= Page::getDataLabel($row,$field);
			$tr .= '</td>'."\n";
			$tr .= '<td class="RightColumnBG">'."\n";
			$tr .= ($inputonly ? Page::getDataInputOnly($row,$field) : Page::getDataInput($row,$field,$separator));
			$tr .= '</td>'."\n";
			$tr .= '</tr>';
...
?>
			<table width="100%" cellpadding="4" cellspacing="2" align="center" class="GridStyle">
				...
				<?		}
						foreach($kolom as $datakolom) { ?>
					<td<?= empty($datakolom['align']) ? '' : ' align="'.$datakolom['align'].'"' ?>><?= uForm::getLabel($datakolom,$row[$datakolom['kolom']],$t_key) ?></td>
				<?		}
						if ($link) { ?>
