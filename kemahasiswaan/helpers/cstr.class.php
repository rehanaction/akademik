<?php
	...
	class CStr {
		...
		function cStrNull($item,$stripapp=true,$removeSpecial=true) {
			if(strval($item) == '')
				return 'null';
			else if($removeSpecial)
				return CStr::removeSpecial($item,$stripapp);
			else
				return $item;
		}
		//null ke strip (-)
		function cStrStrip($item,$stripapp=true) {
			if(strval($item) == '')
				return '-';
			else
				return CStr::removeSpecial($item,$stripapp);
		...
		//* mengubah format tanggal dari yyyy-mm-dd menjadi format indonesia
		function formatDateInd2($ymd,$full=true,$dmy=false,$delim='-',$intd=true) {
			if($ymd == '')
				return '';
			if($ymd == 'null')
				return 'null';
			...
			return $y.' '.Date::indoMonth($m,$full).' '.$d;
		}
		...
		function formatNumber($num,$dec=0,$trim=false) {
			...
			if(strval($num) == '' or !is_numeric($num))
				return $num;
		...
		function formatSize($byte) {
			...
			$kode = static::sizeCodes();

			$i = 0;
			$sisa = $byte;
			while($sisa > static::byte and $i < 8) {
				$sisa /= static::byte;
				$i++;
			}
		...
		function revertSize($ini) {
			...
			$ini = strval($ini);
			$inilen = strlen($ini);
			$idx = array_search($ini[$inilen-1],$kode);

			if($idx !== false) {
				$num = (int)substr($ini,0,$inilen-1);
				for($i=0;$i<$idx;$i++)
					$num *= static::byte;
		...
		function IPKterbilang($x){
			$length = strlen($x);
			$strarray = str_split($x);
			$ipk;
			for ($i=0; $i<$length; $i++) {
				$ipk .= CStr::convertBil($strarray[$i])." ";
			}
