<?php
	...
	class CStr {
		...
		function formatNumber($num,$dec=0,$trim=false,$bracket=false) {
			if(strval($num) == '')
				return $num;
		...
		function formatNumberRep($format,$num,$dec=0,$trim=false,$bracket=false) {
			if(strval($num) == '')
				return $num;
			...
			if($format == 'xls') {
				...
				return self::formatNumber($num,$dec,$trim,$bracket);
		}
				...
				if (count($strnum)==1) {
					...
					switch (strlen($num)) {
						...
						case 2:
							...
							break;
						case 3:
							$w .=  ($isone ? "Seratus" : self::terbilang(substr($num,0,1)) .
								" Ratus").
								" ".(intval(substr($num,1))==0 ? "" : self::terbilang(substr($num,1)));
							break;
						case 4:
							$w .=  ($isone ? "Seribu" : self::terbilang(substr($num,0,1)) .
								" Ribu").
								" ".(intval(substr($num,1))==0 ? "" : self::terbilang(substr($num,1)));
							break;
						default:
					...
					$w = ($isone && strlen($strnum[0])==1 && count($strnum) <=3? "Se".strtolower($text) : self::terbilang($strnum[0]).' '.$text);
					array_shift($strnum);
					$i =count($strnum)-2;
					foreach ($strnum as $k=>$v) {
						if (intval($v)) {
							$w.= ' '.self::terbilang($v).' '.($i >=0 ? $say[$i] : "");
						}
						$i--;
					}
				}
