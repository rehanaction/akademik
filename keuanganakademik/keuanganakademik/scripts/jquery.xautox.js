(function($){
	...
	$.fn.xautox = function(options) {
		var tdpeak, tdmore;
		var settings = $.extend({
			ajaxpage: ajaxpage, strpost: "", targetid: "", postid: "", imgchkid: "", imgavail: false, acdivpos: 1, posset: 1, minlength: 0
		}, options);
	...
	function showAutoComplete(jqtbox,jqkode,jqimgc,jqimgu,settings) {
		...
		var jqtboxlen = 0;
		var jqtboxval = jqtbox.val();
		if(jqtboxval)
			jqtboxlen = jqtbox.val().length;
		
		if(jqtboxlen >= minlength) {
			actdnum = 0;
			acx = $.getJSON(ajaxpage,strpost+"&"+getc+"="+jqtboxval,function(data) {
				// alert('DATA: '+data);
