			...
			<form name="pageform" id="pageform" method="post"<?= $isupload ? ' enctype="multipart/form-data"' : '' ?>>
				...
				<?	/********/
                    ...
                    <tr>
                    	...
                    	<td>
                        <span id="namasistem"><?=$data['namasistem']?></span>
						<input type="hidden" id="sistemkuliah" name="sistemkuliah" value="<?= $data['sistemkuliah'] ?>">
                        </td>
                    </tr>
                    <tr>
                        ...
                        <input type="hidden" id="mhstransfer" name="mhstransfer" value="<?= $data['mhstransfer'] ?>">
                        </td>
                    </tr>
                    <tr>
...
function getDatamhs() {
	...
	jqxhr.done(function(data) {
		...
		$("#sistemkuliah").val(datas.sistemkuliah);
		$("#namajalur").html(datas.jalurpenerimaan);
		$("#namaunit").html(datas.namaunit);
		$("#mhstransfer").val(datas.mhstransfer);
		if(datas.nama)
		$("#cek").val('1');
		loadJenisTagihan();
    });
...
function loadJenisTagihan() {
	var param = new Array();
	param[0] = $("#mhstransfer").val();
	param[1] = $("#sistemkuliah").val();
	var jqxhr = $.ajax({
					url: ajaxpage,
					timeout: ajaxtimeout,
					data: { f: "optjenistagihan", q: param }
				});
	
	jqxhr.done(function(data) {
		$("#jenistagihan").html(data);
    });
    jqxhr.fail(function(xhr,status) {
		alert(status);
	});
