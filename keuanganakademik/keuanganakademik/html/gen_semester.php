<?php
	...
	// properti halaman
	$p_title = 'Generate Tagihan Mahasiswa Per Semester';
	$p_tbwidth = 600;
	...
	if($r_act == 'generate' and $c_edit){
			...
			list($err, $daftartagihan) = mTagihan::generateTagihansemester($conn, $r_nim, $r_periode, $datamhs);

			if ($err <> 0){
				$p_posterr=true;
				$p_postmsg= 'Generate Tagihan Gagal<br>'.$conn->ErrorMsg();
			}
			else if (!empty($daftartagihan)){
				$p_posterr=false;
				$p_postmsg='Tagihan '.implode(',', $daftartagihan).' Berhasil di Generate';
			}else if (empty($daftartagihan)){
				$p_posterr=true;
				$p_postmsg= 'Generate Tagihan Gagal<br> Tidak ada daftar tagihan atau daftar tagihan tidak ditemukan';
            ...
            <div style="clear:both"></div>
				<div>
					<fieldset style="background:#E0FFF3; border:1px solid #CCC;">
                        ...
                        - Tagihan yang di generate ialah tagihan per semester dan Tahunan dan bulanan
                        <br>
                        - Tagihan Tahunan khusus untuk periode gasal saja
                    </fieldset>
				</div>
		</div>
		</div>
