<?php
	...
	// jenis tagihan
	$a_jenis = mTagihan::getListJenisTagihanGenerate($conn,$r_periode,$r_sistem,false); // hanya mahasiswa lama
	...
	// properti halaman
	$p_title = 'Generate Tagihan Rutin';
	$p_tbwidth = '100%';
	$p_aktivitas = 'Master';
	...
	if (($r_act == 'generate' or $r_act == 'void' or $r_act == 'voidna') and $c_edit) {
		...
		$a_filter['ispendaftar'] = false; // hanya mahasiswa lama
	...
	$l_kodeunit = uCombo::unit($conn,$r_kodeunit,'kodeunit','onchange="goSubmit()"',true,true);
	
	// daftar jurusan
	$arr_unit = mAkademik::getArrayunit($conn,false,'2',$r_kodeunit);
	...
	<div id="wrapper">
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post">
							...
							<tr>
								<td valign="top" width="50%">
									<table width="100%" cellspacing="0" cellpadding="4">
										...
										<?	}
											if($c_edit and !empty($r_periode) and !empty($r_sistem)) { ?>
										<tr>
											<td colspan="2">
												<input type="button" value="Generate Tagihan" onclick="goGenerate()">
												<input type="button" value="Hapus Tagihan" onclick="goVoid()">
												<input type="button" value="Hapus Tagihan Mhs Non Aktif" onclick="goVoidNA()">
											</td>
										</tr>
										<?	} ?>
									</table>
				...
				<div class="Break"></div>
				<?	}
					if(!empty($r_periode) and !empty($r_sistem)) { ?>
				<center>
...
function goVoidNA(elem) {
	...
	var txt = confirm("Apakah anda yakin akan melakukan Hapus Tagihan Mhs Non Aktif?");
	if(txt) {
		document.getElementById("act").value = "voidna";
		document.getElementById("key").value = key;
		goSubmit();
	}
}
