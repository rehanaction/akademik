				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						<?	foreach($rowc as $j => $rowcc) {
								...
								if($j == 9 and empty($row['ish2h']) and empty($row['flagbatal'])) {
						?>
						<td<?= $t_align ?>><img id="<?= $t_key ?>" title="Set Batal" src="images/checkb.png" onClick="goBatal(this)" style="cursor:pointer"></td>
						<?
								}
								else {
						?>
						...
						<?	} ?>
                           <td align="center"><img id="<?= $t_key ?>" title="Print Kwitansi" src="images/print.png" style="width:23px" onClick="goPrintdetail('<?=$t_key?>','idpembayaran','<?= Route::navAddress($p_printpagedet) ?>')" style="cursor:pointer"></td>
					</tr>
					<?	}
						if($i == 0) {
					?>
...
function goPrintdetail(nilai,id,file) {
	document.getElementById(id).value = nilai;
	goSubmitBlank(file);
}

<? if($c_edit) { ?>
function goBatal(elem) {
	var batal = confirm("Apakah anda yakin akan membatalkan pembayaran ini?");
	if(batal) {
		$("#act").val("batal");
		$("#key").val(elem.id);
		goSubmit();
