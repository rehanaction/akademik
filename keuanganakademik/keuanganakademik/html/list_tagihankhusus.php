<?php
	...
	$p_detailpage = "data_tagihankhusus";
	// include
	...
	if ($r_act =='set' or $r_act=='unset'){
		...
		$valid = '-1';
				...
				$ok = $conn->Execute("update h2h.ke_tagihan set isvalid = '".$valid."' where idtagihan = '".$key."' ");
				
			if (!$ok)
				$p_postmsg = 'Gagal Melakukan Persetujuan Pembayaran';
			else
				$p_postmsg = 'Berhasil Melakukan Persetujuan Pembayaran';
		
		
	}
	// mendapatkan data
	if(!empty($r_unit)) $a_filter[] = $p_model::getListFilter('p.kodeunit',$r_unit);
	...
	$a_filter[] = $p_model::getListFilter('flaglunas',array('S', 'F'));
	...
	require_once($conf['view_dir'].'v_list_tagihankhusus.php');
?>
