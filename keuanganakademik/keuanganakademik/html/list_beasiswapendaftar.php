<?php
	...
	$a_kolom[] = array('kolom' => 'jalurpenerimaan', 'label' => 'Jalur Penerimaan');
	$a_kolom[] = array('kolom' => 'periodedaftar', 'label' => 'Periode');
	$a_kolom[] = array('kolom' => 'nimpendaftar', 'label' => 'NIM');
	$a_kolom[] = array('kolom' => 'pilihanditerima', 'label' => 'Prodi diterima', 'readonly'=>true,'option' => mCombo::jurusan($conn),'empty'=>true);
	$a_kolom[] = array('label' => 'Kontak');
	$a_kolom[] = array('kolom' => 'isadministrasi','label'=>'Keterangan');
	$a_kolom[] = array('kolom' => 'lulusujian','label'=>'Lulus Ujian', 'readonly'=>true,'option' => array(-1=>'Lulus', 0=>'Tidak Lulus'));
...
function validasi(elem){
	if(elem.checked){
		...
		var posted = "f=validasi&q[]="+elem.id;
		...
		var posted = "f=bukaValidasi&q[]="+elem.id;
		$.post("<?= Route::navAddress('ajax'); ?>",posted,function(text) {
			var msg=text.split('|');
			if(msg[0]==''){
				sukses(msg[1]);
