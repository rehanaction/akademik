<?php
	...
	class uForm {
		...
		function getLabel($data,$value) {
			...
			if(!empty($data['link']))
				$value = '<a href="'.Route::navAddress($data['link'],'key='.$value).'" class="ALink" target="_blank">'.$value.'</a>';
			if(!empty($data['bold']))
...
<?php
		...
		function getPostRecord($data,$apost,$pref='',$postf='',$remspc=true) {
			$post = array();
			$record = array();
