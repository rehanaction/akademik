<?php
	...
	function listQuery() {
			...
			if(empty($ispmb)) {
				$sql .= "(select 
					p.idpembayaran,
					p.tglbayar,
					coalesce(p.nip,p.companycode) as nip ,
					p.ish2h ,
					p.refno ,
					p.idcurrency ,
					p.flagrekon ,
			...
			$sql .= "(select 
					...
					p.keterangan ,
					p.flagbatal,
					coalesce(p.nopendaftar,p.nim) as nim ,
					p.jumlahbayar ,
					t.nama,t.jalurpenerimaan,t.sistemkuliah,t.pilihanditerima as kodeunit,u.namaunit from h2h.ke_pembayaran p
					JOIN pendaftaran.pd_pendaftar t ON coalesce(p.nopendaftar,p.nim)=t.nopendaftar
	...
	function getTagihanMaba($conn,$nim){
		$sql = "select string_agg(flaglunas,'') as flaglunas,count(flaglunas) as tottagihan from h2h.ke_tagihan where nopendaftar='$nim'";
		return $conn->getOne($sql);
	}
	...
	function laporanbytanggal($conn,$tglmulai,$tglakhir,$jenis,$void = '0',$infounit,$sistem,$angkatan,$bank){
			...
			$sql = "select * from (
					...
					from ".static::table()." b
					".($ispmb ? '' : 'left ')."JOIN pendaftaran.pd_pendaftar p ON p.nopendaftar=coalesce(b.nopendaftar,b.nim) 
		...
		function laporanbytanggalDetail($conn,$tglmulai,$tglakhir,$jenis,$void = '0',$infounit,$sistem,$angkatan,$bank,$jenistagihan){
			if($tglmulai== '')
				$tglmulai = date('Y-m-d');
			if($tglakhir == '')
				$tglakhir = date('Y-m-d');

			$sql = "select * from h2h.v_pembayarandetail where (1=1) and to_char(tglbayar,'YYYY-MM-DD') between '$tglmulai' and '$tglakhir' ";
			 if(is_array($jenis))
			 	$sql .= " and ish2h in ('".implode("','",$jenis)."')";
			 else
				if($jenis)
			...
			if(!empty($jenistagihan)){
				$sql .= "and jenistagihan = ".Query::escape($jenistagihan);
			}
