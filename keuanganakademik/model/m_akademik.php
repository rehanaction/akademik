<?php
	...
	class mAkademik {
		...
		function generatePerwalian($conn,$periode,$data) {
			...
			select m.nim, $eperiode, case statusmahasiswa when 'TT' then 'K' else 'T' end, 0, ".Query::logInsert().", '".Query::getAct('generate')."' from akademik.ms_mahasiswa m
			left join akademik.ak_perwalian w on w.nim = m.nim and w.periode = $eperiode left join akademik.v_statusmhsperiode vs on vs.nim=m.nim
