<?php
	...
	if($r_act == 'inquiry' and $r_kodefrm and $c_inquiry){
			...
			$tarif = mTarifformulir::getTarifbykode($conn,$r_kodefrm);
			if(!$tarif)
			{
				$p_postmsg = " Maaf, Tarif Formulir dengan kode tersebut tidak ada";
				$p_posterr = true;
			}
			elseif($tarif['periodedaftar'] <> $settingdetail['FRM']['periodesekarang'])
			{
	...
	$arr_tarif = mTarifformulir::getArraytarif($conn,$settingdetail['FRM']['periodesekarang']);
	if($arr_tarif)
