...
if ($r_act == 'save' and $c_edit) {
    ...
    $r_isditerima = mPengajuanBeasiswaPd::getValidByPd($conn, $r_key);
	if(empty($r_isditerima))
		list($p_posterr, $p_postmsg) = mPengajuanBeasiswaPd::delete($conn, $r_idpengajuan);
	else{
		list($p_posterr, $p_postmsg) = array(true,'Beasiswa sudah tervalidasi.');
	}
						...
						<table width="100%" cellpadding="4" cellspacing="2" class="table table-striped">
							<tr>
								...
								<td>
									...
									?>
									  <button id="pengajuan" class="btn btn-danger" type="button" name="pengajuan" 
										<? if(mPengajuanBeasiswaPd::getValidByPd($conn, $r_key))
											echo 
												'onclick="batalBeasiswa()"'; 
											else  
												echo 'disabled'; ?>
