...
<?php
		...
		if(empty($q[0])) {
			...
			echo UI::createOption($a_prodi,'',true,'-- Pilih Sistem Kuliah Dahulu--');
		}
		else {
			$a_prodi = mCombo::getProdiBuka($conn,$q[0],$q[2]);
			$t_prodi = $q[1];
			
			echo UI::createOption($a_prodi,$t_prodi);
		}
	}
	else if ($f=='sistemkuliah'){ 
		 
		require_once(Route::getModelPath('sistemkuliah'));
		//$conn->debug = true;
		if(empty($q[0])) {
			$a_sistemkuliah = array();
			
			echo UI::createOption($a_sistemkuliah,'',true,'-- Pilih Kampus Terlebih Dahulu--');
		}
		else {
			$a_sistemkuliah = array(''=>'-- Pilih Sistem Kuliah --')+mSistemkuliah::getIdByKampus($conn,$q[0],$q[2]);
			$t_sistemkuliah = $q[1];
			
			echo UI::createOption($a_sistemkuliah,$t_sistemkuliah);
		}
	}
