<?php
	...
	// properti halaman
	$p_title = 'Kelulusan Tes Bidang Mata Pelajaran';
	$p_tbwidth = 550;
	$p_aktivitas = 'DAFTAR';
	$p_model = mPendaftar;
	...
	$l_jalur 	= uCombo::jalurMapel($conn,$r_jalur,'','jalur');
	...
	if(isset($_POST['save'])){
		...
		if($lulus==-1)
		$record['lulustespelajaran']		=$lulus;
		else
		$record['lulustespelajaran']		=0;
				    ...
				    <?
						...
						while($pendaftar = $data->FetchRow()){
							...
							<tr align="center">
								...
								<td><input type="checkbox" value="-1" id=<?=$pendaftar['lulustespelajaran']==true?"checked":"checklist"?> name="<? echo "check".$pendaftar['nopendaftar']; ?>" <? if($pendaftar['lulustespelajaran']==true) echo ' checked="checked" '; ?>/></td>
								<td><?= CStr::formatNumber(mPendaftar::getTotalNilai($conn, $pendaftar['nopendaftar']),2,true);?></td>
								<td><img src="images/edit.png" title="edit nilai" onclick="goTo('<?= $url ?>')"></td>
							</tr>
							<?
						}
					}
