<?php
	...
	//combo
	$l_jalur 	= uCombo::jalurpenerimaan($conn,$r_jalur,'','jalur','onchange="goSubmit()"');
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'kodelokasiujian', 'label' => 'Kode Ruang');
	$a_kolom[] = array('kolom' => 'namalokasi', 'label' => 'Lokasi', 'type'=>'S', 'option' => $ruang);
	$a_kolom[] = array('kolom' => 'kapasitaslokasi', 'label' => 'Kapasitas');
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						<td align="center" width="50px;"><?= $rowc[$j++] ?></td>
						<td><?= $rowc[$j++] ?></td>
						<td align="center"><?= $rowc[$j++] ?></td>
						<td align="center"><?= $rowc[$j++] ?></td>
						<td align="center">
                                                    <?
                                                        if(!empty($row['jalurpenerimaan'])){
                                                            $tglujian=$p_model::getWaktuUjian($conn,$row['jalurpenerimaan']);
                                                            $tglujian=$tglujian['tglujian'];
                                                            $peserta=$p_model::getjumlahpeserta($conn, $row['jalurpenerimaan'], $t_key);
                                                            $peserta=$peserta['count'];
                                                        }else{
                                                            $tglujian="tanggal belum ditentukan";
                                                        }
                                                    ?>
						    <img id="imgcontact" src="images/calendernotice.png" width="20px" title="<?= Date::indoDate($tglujian) ?>">
                                                    <img id="imgcontact" src="images/peps.png" width="20px" title="<?= $peserta ?>">
						</td>
						<?	if($c_edit or $c_delete) { ?>
