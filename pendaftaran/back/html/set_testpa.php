<?php
	...
	//$l_onedayservice  =uCombo::getOnedayservice($conn,$r_service,'','onedayservice');
	$l_periode 	= uCombo::periode($conn,$r_periode,'','periode');
	$l_jalur 	= uCombo::jalurTPA($conn,$r_jalur,'','jalur');
	...
	if(isset($_POST['save'])){
		...
		$nilai		=$_POST["n_".$datapendaftar['nopendaftar']];	
		if($lulus==-1)
		$record['lulustpa']		=$lulus;
		else
		$record['lulustpa']		=0;
		$record['nilaitpa']		=$nilai;
	...
	if($r_act == 'upxls' and $c_edit) {	
		...
		if(empty($_FILES['xls']['name'])){
			...
			$import = mPendaftar::uploadTPA($conn,$r_file);
			if($import){
				$p_posterr=false;
				$p_postmsg = 'berhasil disimpan';
			}else{
		...
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
							...
							<tr>
								<td width="60%">
									<table width="100%" cellspacing="0" cellpadding="4">
										...
										<tr>		
											<td width="100" style="white-space:nowrap"><strong><?= $t_filter['label'] ?> </strong></td>
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle">
					<tr class="DataBG">
						<td align="center" colspan="3">Impor Nilai TPA dari Format Excel</td>
					</tr>
				...
				*/ ?>
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="" cellspacing="0" class="GridStyle" align="center">
				    ...
				    <?
						...
						while($pendaftar = $data->FetchRow()){
							...
							<tr align="center">
								...
								<td><input type="checkbox" value="-1" id=<?=$pendaftar['lulustpa']==true?"checked":"checklist"?> name="<? echo "check".$pendaftar['nopendaftar']; ?>" <? if($pendaftar['lulustpa']==true) echo ' checked="checked" '; ?>/></td>
								<td align="center"><?= UI::createTextBox('n_'.$pendaftar['nopendaftar'],$pendaftar['nilaitpa'],'ControlStyle XCell',6,3,$c_edit) ?></td>
							</tr>
							<?
						}
					}
					?>
					<tr>
						<td colspan="3" align="right"><b><i> Check / Uncheck All </i></b></td>
...
function goDownXLS() {
	location.href = '<?=$conf['download_dir']?>template_tpa.xls';
}
