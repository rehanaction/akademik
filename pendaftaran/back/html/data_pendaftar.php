	...
	if($r_act == 'save' and $c_edit) {
		...
		list($post,$record_quisioner) = uForm::getPostRecord($a_input_quisioner,$_POST);
			...
			if(empty($r_key)){
				...
				$record['pilihanditerima'] = $record['pilihanditerima'];
				...
				<center>
					...
					<? if(!empty($r_key)){?>
						...
						<td align="center" valign="top" rowspan="<?= 8+(empty($a_evaluasi) ? 0 : 1) ?>">
							...
							<input type="button" style="padding:4px" value="Capture" class="ControlStyle" onClick="popup('index.php?page=capture_cam&nopendaftar=<?=$r_key?>&jalur=<?=$r_jalur?>&gelombang=<?=$r_gel?>&periode=<?=$r_periode?>',420,670);">
							<?php }?>
						</td>
					<? } ?>
					</tr>
					...
					*/ ?>
					...
					<tr>
	                	<td style="background: #EDEDED;">Referensi INABA</td>
	                    <td><?= Page::getDataInputOnly($row_kuisioner,'jawab_1')?> | <?= Page::getDataValue($row_kuisioner,'jawabketerangan_1') ?></td>
	                </tr>
