<?php
	...
	// properti halaman
	$p_title = 'Kelulusan Tes Kesehatan';
	$p_tbwidth = 550;
	$p_aktivitas = 'DAFTAR';
	$p_model = mPendaftar;
	...
	$l_jalur 	= uCombo::jalurTesKesehatan($conn,$r_jalur,'','jalur');
	...
	if(isset($_POST['save'])){
		...
		if($lulus==-1)
		$record['lulusteskesehatan']		=$lulus;
		else
		$record['lulusteskesehatan']		=0;
				    ...
				    <?
						...
						while($pendaftar = $data->FetchRow()){
							...
							<tr align="center">
								...
								<td><input type="checkbox" value="-1" id=<?=$pendaftar['lulusteskesehatan']==true?"checked":"checklist"?> name="<? echo "check".$pendaftar['nopendaftar']; ?>" <? if($pendaftar['lulusteskesehatan']==true) echo ' checked="checked" '; ?>/></td>
