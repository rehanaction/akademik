<?php
	...
	// properti halaman
	$p_title = 'Kelulusan Tes Kompetensi';
	$p_tbwidth = 550;
	$p_aktivitas = 'DAFTAR';
	$p_model = mPendaftar;
	...
	$l_jalur 	= uCombo::jalurKompetensi($conn,$r_jalur,'','jalur');
	...
	if(isset($_POST['save'])){
		...
		if($lulus==-1)
		$record['luluskompetensi']		=$lulus;
		else
		$record['luluskompetensi']		=0;
		
		$record['nilaikompetensi']		=$nilai;
				    ...
				    <?
						...
						while($pendaftar = $data->FetchRow()){
							...
							<tr align="center">
								...
								<td><input type="checkbox" value="-1" id=<?=$pendaftar['luluskompetensi']==true?"checked":"checklist"?> name="<? echo "check".$pendaftar['nopendaftar']; ?>" <? if($pendaftar['luluskompetensi']==true) echo ' checked="checked" '; ?>/></td>
								<td align="center"><?= UI::createTextBox('n_'.$pendaftar['nopendaftar'],CStr::formatNumber($pendaftar['nilaikompetensi'],2,true),'ControlStyle XCell',5,3,$c_edit) ?></td>
							</tr>
							<?
						}
					}
