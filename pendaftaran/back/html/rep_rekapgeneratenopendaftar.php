...
// cek akses halaman
   ...
   require_once(Route::getModelPath('combo'));
	...
	$p_model=mLaporan;
	$p_tbwidth = 950;
	$p_namafile = 'Rekapitulasi Generte Nomor Pendaftar';
	
	$r_data = mLaporan:: getRekapgeneratenopendaftar($conn, $r_tglawal,$r_tglakhir,$r_periode,$r_sistemkuliah);
	$arrUnit = mCombo::unitProdi($conn);
	$arrSistemkuliah = mCombo::sistemKuliah($conn);
 
	foreach ($r_data as $key => $val){
		$data[$val['pilihanditerima']] = $val['jumlah'];
		}
...
<body style="font-size:11pt" onLoad="window.print()">
	...
	<table style="width:<?= $p_tbwidth?>px; border-collapse:collapse; text-align:left" border="1" cellpadding="3">
		...
		<tr>
			...
			<td><?= $val['kodenim']?></td>
			<td><?= $val['namaunit']?></td>
			<td align="right"><?= cStr::formatNumber($data[$val['kodeunit']]);?></td>
		</tr>
