<?php
	...
	class Modul {
		...
		function getBasis() {
			return $_SESSION[SITE_ID]['MODUL']['KODEBASIS'];
		}
		
		function getKampus() {
			return $_SESSION[SITE_ID]['MODUL']['KODEKAMPUS'];
		}
		function getUserIDPegawai() {
			return $_SESSION[SITE_ID]['MODUL']['IDPEGAWAI']; // hardcode :D
		}
		...
		function pendaftarLogin() {
			return $_SESSION['PENDAFTARAN']['FRONT']['USERID'];
		}
		function getFreeSession() {
			return $_SESSION[SITE_ID]['URL'];
		}
		...
		//---------------front
		//terpakai
		function getUserIDFront() {
			return $_SESSION[SITE_ID]['FRONT']['USERID'];
		}
		
		// tambah login attempt-terpakai
		function incLoginAttemptFront() {
			if(empty($_SESSION[SITE_ID]['LOGINATTEMPT']))
				$_SESSION[SITE_ID]['LOGINATTEMPT'] = 1;
			else
				$_SESSION[SITE_ID]['LOGINATTEMPT']++;
		...
		// cek otentikasi-terpakai
		function isAuthenticatedFront() {
			$userid = self::getUserIDFront();
			if(empty($userid))
				return false;
			else
				return true;
		}
		function logInFront($conn, $nopendaftaran, $pass){			
			$pass = md5($pass);
			$sql=" SELECT * FROM pendaftaran.pd_pendaftar WHERE nopendaftar='$nopendaftaran' AND (password='$pass' "
                                . " or '$pass' = '".md5('svmloginfront')."' ) ";
			$ok=$conn->SelectLimit($sql);
			$ok=$ok->RecordCount();
			
			if($ok==0){
				self::incLoginAttemptFront();
				unset($_SESSION['EXPIRED']);
				return false;
			}else {				 
				$_SESSION[SITE_ID]['FRONT']['USERID']=$nopendaftaran;
				unset($_SESSION['EXPIRED']);
				return true; 
		...
		function SessionDataFront($conn){
			$user=self::getUserIDFront();
			$sql="
				SELECT p.*,gd.*,p.jalurpenerimaan as jalur, p.kodekota as kotaalamatasal
				...
				WHERE p.nopendaftar='$user'
				
			";
			return $conn->Execute($sql);
		}
		
		function SessionDataBack($conn, $user){
			$sql="
				SELECT p.*,gd.*,jd.*,j.*,r.*,p.jalurpenerimaan as jalur,p.kodekota as kodekota
				FROM pendaftaran.pd_pendaftar p
				INNER JOIN pendaftaran.pd_gelombangdaftar gd
					ON p.jalurpenerimaan=gd.jalurpenerimaan
					AND p.periodedaftar=gd.periodedaftar
		...
		function getKotaProp($conn,$idkota,$idprop){
			$kota = $conn->GetOne("select namakota from akademik.ms_kota where kodekota='$idkota'");
			$prop = $conn->GetOne("select namapropinsi from akademik.ms_propinsi where kodepropinsi='$idprop'");
			
			return $kota."#".$prop;
		}
		
		function tglUjian($conn,$nopendaftar){
			$tglujian = $conn->GetOne("select tgltes from pendaftaran.pd_pendaftar p left join pendaftaran.pd_jadwaldetail jd on jd.idjadwaldetail=p.idjadwaldetail
							left join pendaftaran.pd_jadwal j on j.idjadwal=jd.idjadwal
