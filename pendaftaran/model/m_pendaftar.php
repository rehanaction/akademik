<?php
	...
	class mPendaftar extends mModel{
		...
		*/
		...
		function exportPendaftar($conn, $periode, $fakultas, $nim=''){
          ...
          //eman (ganti tmplahir menjadi kotakota. Koq lebar banget charvar 50?)
					$record['tmplahir']=$conn->GetOne("select namakota from akademik.ms_kota where kodekota='".$row['kodekotalahir']."'");
					...
					$record['kodeagama']=cstr::cStrNull($row['kodeagama']);
					$record['kodewn']=cstr::cStrNull($row['kodewn']);
					$record['namaayah']=$row['namaayah'];
					$record['namaibu']=$row['namaibu'];
					$record['alamatortu']=$row['alamatayah'];
					$record['kodepekerjaanayah']=cstr::cStrNull($row['kodepekerjaanayah']);
					$record['kodepekerjaanibu']=cstr::cStrNull($row['kodepekerjaanibu']);
					$record['kodependidikanayah']=cstr::cStrNull($row['kodependidikanayah']);
					$record['kodependidikanibu']=cstr::cStrNull($row['kodependidikanibu']);
          //eman tambah namasmu
					$record['asalsmu']=$conn->GetOne("select namasmu from pendaftaran.lv_smu where idsmu='".$row['asalsmu']."'");
					$record['alamatsmu']=$row['alamatsmu'];
					$record['kodekotasmu']=cstr::cStrNull($row['kodekotasmu']);
					...
					// $record['kodepropinsiortu']=cstr::cStrNull($row['kodepropinsiayah']);
					/* informasi transfer */
					$record['mhstransfer']=$row['mhstransfer'];
					$record['ptasal']=$row['ptasal'];
					// $record['kodepropinsipt']=cstr::cStrNull($row['propinsiptasal']);
					$record['kodekotapt']=cstr::cStrNull($row['kodekotapt']);
					$record['ptjurusan']=$row['ptjurusan'];
					$record['ptipk']=$row['ptipk'];
					$record['ptthnlulus']=$row['ptthnlulus'];
					$record['sksasal']=$row['sksasal'];
					$record['sksdiakui']=$row['sksdiakui'];
		...
		function generateNIM($conn, $jalur, $jurusan, $periode, $urutreg, $asing=''){
			...
			$kodejalur	= self::getkodenim($conn, "akademik.lv_jalurpenerimaan", "jalurpenerimaan='$jalur'","");
		...
		function getMaxNim($conn,$periode,$kodeunit){
			$sql="select coalesce(max(right(nimpendaftar,4)),'0') from pendaftaran.pd_pendaftar where pilihanditerima = '$kodeunit' 
			and substring(periodedaftar,1,2) = substring('$periode',1,2)";
			$nim=$conn->GetOne($sql);
			$nourut=substr($nim,-4);
			$nourut++;
			return str_pad($nourut,4,"0",STR_PAD_LEFT);
		}


		function getNim($conn,$nopendaftar){
			$sql="select pr.kodenim, p.nopendaftar, p.jalurpenerimaan, p.idgelombang,p.sistemkuliah, p.mhstransfer, p.periodedaftar,p.pilihanditerima from pendaftaran.pd_pendaftar p
					...
					where nopendaftar='$nopendaftar'";
			$data=$conn->GetRow($sql);
			$kodekampus = self::getKodeKampus($conn);
			$kodesistem = self::getKodeSistem($conn, $data['sistemkuliah']);
			$kodestatus = self::getKodeStatus($conn, $data['mhstransfer']);
			$urutreg=self::getMaxNim($conn,$data['periodedaftar'],$data['pilihanditerima']);
			$nim=$kodekampus.str_pad($data['kodenim'],2,"0",STR_PAD_LEFT).substr($data['periodedaftar'],2,2).$kodestatus.$kodesistem.$urutreg;
			return $nim;
		}

		function getKodeKampus($conn){
			return $conn->getOne("select kodenim from gate.ms_unit where kodeunit = '4302100000'");
		}

		function getKodeSistem($conn, $kodesistem){
			if ($kodesistem == "R"){
				$kodesistem = 1;
			}elseif($kodesistem == "RS"){
				$kodesistem = 2;
			}
			return $kodesistem;
		}

		function getKodeStatus($conn, $kodestatus){
			if ($kodestatus == 0){
				$kodestatus = 1;
			}elseif($kodestatus == -1){
				$kodestatus = 2;  //laNJUTAN
			}else{
				$kodestatus = 3; //PINDAHAN
			}
			return $kodestatus;
		}
		...
		function generateOneNim($conn,$nopendaftar){
			...
			if(strlen($nim)==12)
			{
				...
				list($p_posterr,$p_postmsg)=array(true,'Nim Yang Dihasilkan Kurang Dari 12 digit '.$nim.' ');
	...
	function inputColumn($conn,$key='',$r_jalur='', $r_periode='', $r_gel='',$beasiswa='',$registrasi='',$semesterpendek=''){
		...
		$arrSistemkuliah = array(''=>'--- Pilih Sistem Kuliah ---')+mCombo::sistemKuliah($conn);
		$arrPendapatan = mCombo::pendapatan($conn);
		$arrKampus = array(''=>'--- Pilih Kampus ---')+mCombo::getKampus($conn);
		$a_input = array();
		$a_input[] = array('kolom' => 'nama',	'label' => 'Nama Pendaftar', 'maxlength' => 50, 'size' => 30, 'notnull' => true);
		$a_input[] = array('kolom' => 'kodekampus', 'label' => 'Kampus', 'type' => 'S','option' => $arrKampus,'empty'=>false,'readonly'=>$readonly,'add'=>'onChange="loadSistemkuliah()"', 'notnull' => true);
		$a_input[] = array('kolom' => 'sistemkuliah', 'label' => 'Basis', 'type' => 'S','option' => $arrSistemkuliah,'empty'=>false,'readonly'=>$p_edit,'add'=>'onChange="loadProdiBuka()"', 'notnull' => true);
		...
		$a_input[] = array('kolom' => 'kodepropinsilahir', 'label' => '(Kelahiran): Propinsi', 'type' => 'S', 'option' => $arrPropinsi, 'empty' => true, 'add' => 'onchange="loadKotaLahir()"', 'notnull' => true);
		$a_input[] = array('kolom' => 'kodekotalahir', 'label' => 'Kota Lahir', 'type' => 'S', 'option' => "", 'notnull' => true, 'empty'=>true,'text'=>'kodekotalahir_text');

		$a_input[] = array('kolom' => 'pilihan1', 'label' => 'Prodi Dipilih', 'type' => 'S', 'notnull' => true, 'option' => $arrJurusan, 'empty' => true,'add' => 'onchange="loadProdiBuka2()"','readonly'=>$readonly);
		...
		$a_input[] = array('kolom' => 'jeniswali', 'label' => 'Hubungan', 'type' => 'S', 'option' => array(1=>'Ayah',2=>'Wali'), 'empty' => false);
		...
		//perguruan tinggi
		$a_input[] = array('kolom' => 'mhstransfer', 'label' => 'Status Pendidikan', 'type' => 'R', 'option' => array('0' => 'Lulusan SMU/SMK/ dan sejenisnya','-1' => 'Melanjutkan/(D3/D4->S1)/(S1->S2)'), 'add' => 'onChange="disabledMhst();"', 'default'=>'0','notnull'=>true);
		$a_input[] = array('kolom' => 'ptasal', 'label' => 'Perguruan Tinggi (PT) Asal', 'maxlength' => 50, 'size' => 40);
		$a_input[] = array('kolom' => 'propinsiptasal', 'label' => 'Propinsi PT  ', 'type' => 'S', 'option' => $arrPropinsi, 'add' => 'onchange="loadKotaPTAsal()"', 'empty'=>true);
		$a_input[] = array('kolom' => 'kodekotapt', 'label' => 'Kota PT', 'type' => 'S', 'option' =>"",'empty'=>true,'text'=>'kodekotapt_text');
	...
	function getUrl($url,$post_data){
		...
		function getIdPegawai($conn,$namalengkap)
		{
			$namalengkap = strtolower(self::cleanSS($namalengkap));
			$sql = "SELECT idpegawai from sdm.ms_pegawai where regexp_replace(lower(sdm.f_namalengkap('',ms_pegawai.namadepan,ms_pegawai .namatengah,ms_pegawai.namabelakang,'')),'[^a-zA-Z]', '', 'g')='$namalengkap'";
			
			return $conn->GetOne($sql);
		}

		function cleanSS($string) {
			$string = str_replace(' ', '', $string); // Replaces all spaces with hyphens.
			return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
		 }
		 

		 function insertDataKunjungan($conn,$data){
			...
			$sql = "insert into humas.hm_pendaftar ($kolom) values($values)";
			$ok = $conn->Execute($sql);
			if($ok){
				return true;
			}else{
		...
		function getIdPegawaiLogin($conn,$key)
		{
			$sql = "select idpegawai from gate.sc_user where username='$key'";
			return $conn->GetOne($sql);
		}


		function viewDatabymarketing($conn,$idpegawai)
		{
			if(!empty($idpegawai) || $idpegawai<>''){
				$sql = "select * from humas.v_datakunjunganpermarketing where idpegawai='$idpegawai'";
			}else{
				$sql = "select * from humas.v_datakunjunganpermarketing";
			}
			

			return $conn->GetArray($sql);
		}
		function viewDatabymarketingDetail($conn,$data)
		{
			$kolom = implode(',',array_keys($data));
			$valuesArrays = array();
			$i = 0;
			...
			$values = implode(' and ',$valuesArrays);
			$sql = "select * from humas.hm_pendaftar where $values";
			return $conn->GetArray($sql);
		}

		function UpdateDataKunjungan($conn,$data,$where){
			...
			foreach($data as $key=>$values)
			{
				if(is_int($values))
				{
					if($key=='statusfollowup'){
						$valuesArrays[$i] = $key.'=statusfollowup::integer+1';
					}else{
						$valuesArrays[$i] = $key.'='.$values;
					}
				}else{
			...
			foreach($where as $key2=>$values2)
			{
				if(is_int($values2))
				{
					$valuesArrayswhere[$i] = $key2.'='.$values2;
				}else{
					$valuesArrayswhere[$i]= $key2."='".$values2."'";
				}
				$ii++;
			...
			$sql = "update humas.hm_pendaftar set $values  where $values2";
			$ok = $conn->Execute($sql);
			if($ok){
				return true;
			}else{
		...
		function cekNoPendaftarHumas($conn,$pendaftar)
		{
			$sql = "SELECT nopendaftar from humas.hm_pendaftar where nopendaftar='$pendaftar'";
			return $conn->GetOne($sql);
		}
		function getPeriodeDaftar($conn)
		{
			$sql = "select periodedaftar from pendaftaran.pd_pendaftar group by periodedaftar order by periodedaftar desc limit 4";
			return $conn->GetArray($sql);
		}
		function getPencapaian($conn)
		{
			$sql = "select distinct * from humas.v_pencapaian2";
			return $conn->GetArray($sql);

		} 
		function getPencapaianSemester($conn)
		{
			$sql = "select periodedaftar,sum(pendaftar) as total from humas.v_pencapaian group by periodedaftar";
			$data = $conn->GetArray($sql);
			$newdata = array();
			foreach($data as $row){
				$newdata[$row['periodedaftar']]=$row['total'];
			}
			return $newdata;
		}
		function getPencapaianTahun($conn)
		{
			$sql = "select periodedaftar,sum(pendaftar) as total from humas.v_pencapaian group by periodedaftar";
			$data = $conn->GetArray($sql);
			$newdata = array();
			...
			$i=0;
			foreach($data as $row){
				if(!empty($data[$i+1]['periodedaftar'])){ 
					if(Pendaftaran::getTahun($row['periodedaftar'])==Pendaftaran::getTahun($data[$i+1]['periodedaftar'])){
							$total = $row['total']+$data[$i+1]['total'];
							$newdata[Pendaftaran::getTahun($row['periodedaftar'])] = $total;
					}
				}
				$i++;
			}
		...
		function getPencapaianDetail($conn)
		{
			$sql = "select distinct * from humas.v_pencapaian";
			$data = $conn->GetArray($sql);
			$pcp = array();
			foreach($data as $values){
				$pcp[$values['periodedaftar'].$values['idpegawai']]= $values['pendaftar'];
