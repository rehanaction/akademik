<?php
	...
	class mUserSession extends mModel {
		const schema = 'gate';
		...
		function getHomeGraphData($conn) {
			...
			$sql = "select t_logintime as time, t_osname from gate.sc_usersession where to_date(t_logintime, 'yy-mm-dd') >= current_date-$hari";
			$rs = $conn->Execute($sql);
