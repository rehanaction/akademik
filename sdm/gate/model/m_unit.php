	...
	class mUnit extends mModel {
		...
		function insertCRecord($conn,$kolom,$record,&$key) {
			...
			if($record['parentunit'] != 'null') {
				$a_parent = $conn->GetRow("select level, inforight, kodeurutan from ".static::table()." where kodeunit = '".$record['parentunit']."'");
				$t_urutan = $conn->GetOne("select max(kodeurutan) from ".static::table()." where parentunit = '".$record['parentunit']."'");
