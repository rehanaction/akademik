<?php
	...
	$t_data = current($a_data);
    
   $isldap = $conn->GetOne("select isuseldap from gate.sc_user where userid = '".Modul::getUserID()."'");
?>
<!DOCTYPE html>
...
<body onLoad="openContent('<?= $r_act == 'changepass' ? 'password' : $t_data['kode'] ?>')">
	...
	<div id="main">
		...
		<div id="<?= $t_data['kode'] ?>" class="panel">
			<div class="content">
				...
				<div class="role_box">
					...
					<span class="role_container" onClick="goAccess('<?= $t_modul ?>','<?= $t_kolom['koderole'] ?>','<?= trim($t_kolom['kodeunit']) ?>')" style="float:left;width:100%;">
		...
		<div id="password" class="panel">
			<div class="content">
				...
				<div class="clr" style="margin-bottom:20px;"></div>
				<div class="role_box">
				<table width="100%" class="role_container" id="msgChangePass">
					<? if(!empty($p_postmsg)) { ?>
					<tr>
						<td align="center" colspan="2" class="<?= $p_posterr ? 'error' : 'success' ?>"><?= $p_postmsg ?></td>
					</tr>
					<?	} ?>
				</table>
				<table width="100%" class="role_container" id="frmChangePass">
					<tr>
...
<script type="text/javascript">
	...
	function openContent(id) {
		if(id != "") {
			//pengecekan untuk ganti password
			if(id == 'password'){
				$("#msgChangePass").hide();
				$("#frmChangePass").show();
				<?if($r_act == 'changepass'){?>
