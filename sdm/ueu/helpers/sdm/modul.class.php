<?php
	...
	class Modul {
		...
		function getSessionData($data) {
			...
			$modul['USEREMAIL'] = $data[$idx['USEREMAIL'][0]]['value'];
		...
		function getUserEmail() {
			return $_SESSION[SITE_ID]['MODUL']['USEREMAIL'];
		}
		...
		function resetSession($key){
			unset($_SESSION[SITE_ID]['VAR'][$key]);
		}
		...
		function setSIMSession($conn,$kodemodul,$koderole,$kodeunit) {
			...
			// include
			require_once($conf['modelgate_dir'].'m_user.php');
		...
		//change right
		function changeRight($conn,$kodeunit,$koderole,$userid){
			// include
			require_once(Route::getModelPath('mastkepegawaian'));
			
			$_SESSION[SITE_ID]['MODUL']['RIGHT'] = mMastKepegawaian::infoRight($conn,$kodeunit,$koderole,$userid);
		}
	}
?>