<?php
	...
	?>
			<td <?= $bgcolor ?><?= empty($notgl) ? '&nbsp;' : $add; ?> valign="top">
				<table width="100%">
					<tr>
						<td>
							<div style="background-color: #999999;border-radius: 3px 3px 3px 3px; color: #FFFFFF;display: block;float: left;padding: 2px 4px 3px;font-weight: bold;">
								<?= empty($notgl) ? '&nbsp;' : $notgl ?>
							</div>
						</td>
					</tr>
					<tr>
						<td>
							<table cellpadding="0" cellspacing="0"<?= $subbgcolor ?><?= $substyle ?>>
								<tr>
									<td width="<?= $subborder ? 30 : 120 ?>" height="<?= $subborder ? 24 : 40 ?>" valign="top">
										<?
											if(empty($notgl))
												echo '&nbsp;';
											else{
												if(count($arrkeg)>0){
													foreach($arrkeg as $keg => $ashift) {
														$label = '';
														foreach($ashift as $id => $data) {
															$waktu = substr($data['jamdatang'],0,2).':'.substr($data['jamdatang'],2,4).' - '.substr($data['jampulang'],0,2).':'.substr($data['jampulang'],2,4);
															$nama = $data['namalengkap'];
															
															$label = (!empty($waktu) and !empty($nama)) ? '<font size="-2"><strong>'.$nama.'</strong> <br>'.$waktu.'</font>' : '';
															echo $label.'<br>';
														}
													}
												}
