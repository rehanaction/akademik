	...
	class Query {
		...
		function connect($pref='') {
			...
			if($pref == 'keu'){
				...
				$conn = ADONewConnection($conf[$pref.'db_driver']);
				$conn->Connect($conf[$pref.'db_host'],$conf[$pref.'db_username'],$conf[$pref.'db_password'],$conf[$pref.'db_dbname']);
		...
		function recInsert($conn,$record,$table) {
			Query::setLogColumn($record,'insert');
		...
		function recUpdate($conn,$record,$table,$condition,$force=false) {
			Query::setLogColumn($record,'update');
		...
		// Simpan dari array
		function recSave($conn,$record,$table,$condition,$force=false) {
		...
		function qDelete($conn,$table,$condition) {
			...
			//$act = substr('d-'.$i_page,0,30);
			//$sql = "update $table set ".Query::logUpdate().", t_updateact = '$act' where $condition";
			//$conn->Execute($sql);
		...
		function setLogColumn(&$record, $act='update') {
		    if($act == 'update'){
			    $record['t_updateuser'] = Modul::getUserName();
			    $record['t_updatetime'] = date('Y-m-d H:i:s');
			    $record['t_updateip'] = $_SERVER['REMOTE_ADDR'];
		    }else if($act == 'insert'){
			    $record['t_insertuser'] = Modul::getUserName();
			    $record['t_inserttime'] = date('Y-m-d H:i:s');
			    $record['t_insertip'] = $_SERVER['REMOTE_ADDR'];
		    }
		}
		
		// menambahkan pengisian kolom act
		function setAct(&$record,$act) {
			if(empty($record['t_updateact'])) {
				global $i_page;
				
				$act = substr($act.'-'.$i_page,0,30);
				$record['t_updateact'] = $act;
			}
		}
		...
		function colFilter($col,$str) {
			return "lower(cast(".$col." as varchar)) like '%".strtolower($str)."%'";
		}
