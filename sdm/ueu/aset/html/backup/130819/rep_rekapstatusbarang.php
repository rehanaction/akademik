<?php
    ...
    if(!empty($r_cabang)) 
        $sql .= "and substring(s.idlokasi,1,1) = '$r_cabang' ";
    if(!empty($r_gedung)) 
        $sql .= "and substring(s.idlokasi,2,1) = '$r_gedung' ";
    if(!empty($r_lantai)) 
	...
	$sql ="select s.idbarang1,s.idstatus,count(s.idseri) as total
        from aset.as_seri s 
        left join aset.ms_lokasi l on l.idlokasi = s.idlokasi ";
    if($r_showchild) 
        $sql .= "join aset.ms_unit u on u.idunit = s.idunit ";
    ...
    if(!empty($r_lantai)) 
        $sql .= "and substring(s.idlokasi,3,1) = '$r_lantai' ";
    $sql .= "group by s.idbarang1,s.idstatus";
