<?php
    ...
    $sql = "select u.kodeunit, u.namaunit, pg.nip+' - '+pg.namalengkap as pemakai, pg.namalengkap
	...
	and pd.iddetperolehan = '$r_detail' ";

	$data =  $conn->GetRow($sql);

	$sql = "select right('000000' + cast(s.noseri as varchar(6)), 6) noseri, 
			case when s.tglperolehan < '2016-06-01' then p.idbarang+' - '+b.namabarang else p.idbarang1+' - '+bb.namabarang END AS barang,
			  p.merk, p.spesifikasi, pd.idlokasi+' - '+l.namalokasi as lokasi, pd.idlokasi, pg.nip+' - '+pg.namalengkap as pemakai, p.tglperolehan, k.kondisi
			  from aset.as_perolehan p
			  join aset.as_perolehandetail pd on pd.idperolehan = p.idperolehan
			  ...
			  left join sdm.v_biodatapegawai pg on pg.idpegawai = pd.idpegawai
			 where pd.iddetperolehan = '$r_detail' 
