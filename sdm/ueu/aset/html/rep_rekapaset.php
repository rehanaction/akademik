...
$conn->debug = true;
    ...
    $sql ="select a.idbarang1, a.namabarang FROM (
            SELECT
            CASE WHEN s.tglperolehan < '2016-06-01'
                THEN substring(s.idbarang,1,10)
                ELSE substring(s.idbarang1,1,16)
            END as idbarang1,
            CASE WHEN s.tglperolehan < '2016-06-01'
                THEN b.namabarang
                ELSE bb.namabarang
    ...
    if(!empty($r_jenisruang)) 
        $sql .= "and l.idjenislokasi = '$r_jenisruang' ";
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,16) = substring(s.idbarang1,1,16) ";
    $sql .= " ) a group by a.idbarang1, a.namabarang order by a.idbarang1 ";
    ...
    if(!empty($r_level)) 
        ...
        // $sql .= "and (s.idbarang = '3080102001' or s.idbarang1 = '3010501001000003') ";
    $sql .= " ) a group by a.idbarang1,a.namabarang order by a.idbarang1 ";
    ...
    $sql ="select a.idbarang1, a.total, a.nilai FROM (
            SELECT
            CASE WHEN s.tglperolehan < '2016-06-01'
                THEN substring(s.idbarang,1,10)
                ELSE substring(s.idbarang1,1,16)
            END as idbarang1,
            count(s.idseri) as total,
            sum(s.nilaiaset) as nilai
            from aset.as_seri s 
            left join aset.ms_barang b on b.idbarang = s.idbarang 
            left join aset.ms_barang1 bb on bb.idbarang1 = s.idbarang1  
            left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,16) = substring(s.idbarang1,1,16) ";
    $sql .= "group by substring(s.idbarang,1,10), substring(s.idbarang1,1,16), s.tglperolehan ) a order by substring(a.idbarang1,1,16)";
    ...
    $sql .= "and s.idbarang1 = s.idbarang1 ";
    // $sql .= "and (s.idbarang = '3080102001' or s.idbarang1 = '3010501001000003') ";
    $sql .= " group by s.tglperolehan, s.idbarang, s.idbarang1) a order by a.idbarang1 ";
