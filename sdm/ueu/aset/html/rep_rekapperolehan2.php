<?php
	...
	$conn->debug = true;
	// hak akses
	Modul::getFileAuth('repp_rekapperolehan2');
	...
	$sql = "select namalengkap, jabatanstruktural from sdm.v_asetpegawai where jabatanstruktural like '%Pengadaan%' ";
	
	$data = $conn->GetRow($sql);
	
	$sql ="select p.nopo, p.idunit, u.namaunit, 
			case when p.tglperolehan < '2016-06-01' then b.namabarang else bb.namabarang END AS namabarang,
			sp.namasupplier, p.tglperolehan, p.qty,COUNT(p.qty) as jml, p.harga
			from aset.as_perolehan p 
			left join aset.as_perolehandetail pd on pd.idperolehan=p.idperolehan 
			left join aset.as_seri se on se.iddetperolehan = pd.iddetperolehan 
			left join aset.ms_barang b on b.idbarang=p.idbarang 
			left join aset.ms_barang1 bb on bb.idbarang1=p.idbarang1 
	...
	$sql .= " group by p.nopo, b.namabarang,bb.namabarang,p.merk, p.tglperolehan, sp.namasupplier, p.idunit, u.namaunit, p.harga,p.qty
...
?>
<div class="div_head">
    Laporan Rekap Pengadaan Barang<br/>
...
<table class="tb_data" width="<?= $p_tbwidth ?>">
	...
	while ($row = $rs->FetchRow ()){
	    ...
	    $tot = (float)$qty * (float)$row['harga'];

		$harga += (float)$row['harga'];
		$total = bcadd($tot, $total);
	?>
	<tr valign="top">
	    ...
	    ?>
	    <!--<td align="right"><?= CStr::formatNumber($row['qty']) ? CStr::formatNumber($row['qty']) : CStr::formatNumber($row['qtyb']) ?></td>-->
	    <td align="right"><?= CStr::formatNumber($qty)?></td>
	    <td align="right"><?= CStr::formatNumber($row['harga'],2) ?></td>
		<td align="right"><?= CStr::formatNumber($tot,2) ?></td>
	</tr>
	<?
	}
    if($i == 0) {
	...
	<tr>
		...
		<td align="right"><b><?= CStr::formatNumberRep($r_format,$harga,2) ?></b></td>
