<?php
	...
	// hak akses
	Modul::getFileAuth('repp_mutasiperiode');
	...
	$p_title = '.: Laporan Mutasi Aset (Per Periode) :.';	
	$p_tbwidth = 1250;
	$p_ncol = 9;
	$p_namafile = 'mutasi_periode_'.$r_unit;
	
	$a_unit = mLaporan::getDataUnit($conn, $r_unit);
	$a_param = array('unit' => $a_unit);
	//$a_jnsrawat = mLaporan::getJenisRawat($conn,$r_jnsrawat);
    //$rs = mLaporan::getMutasiPeriode($conn, $a_param);
    
	$sql ="select m.tglmutasi, m.tglpengajuan, s.noseri, b.namabarang, u1.namaunit as unitasal, u2.namaunit as unittujuan, 
...
<table class="tb_data" width="<?= $p_tbwidth ?>">
	...
	<tr valign="top">
	    ...
	    <td align="center"><?= CStr::formatDateInd($row['tglpengajuan'],false) ?></td>
