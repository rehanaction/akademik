<?php
	...
	$a_unittujuan = mCombo::unitSave($conn,false,true);
	...
	$a_input[] = array('kolom' => 'pegawaiasal', 'label' => 'Pemakai Asal', 'size' => 30, 'readonly' => $isro, 'notnull' => true);
	$a_input[] = array('kolom' => 'idpegawaiasal', 'type' => 'H', 'issave' => true);
	...
	$a_input[] = array('kolom' => 'idunittujuan', 'label' => 'Unit Tujuan', 'type' => 'S', 'option' => $a_unittujuan, 'add' => 'style="width:250px"', 'notnull' => true, 'readonly' => $isro);
	//$a_input[] = array('kolom' => 'idlokasitujuan', 'label' => 'Lokasi Tujuan', 'type' => 'S', 'option' => $a_lokasitujuan, 'add' => 'style="width:250px"', 'notnull' => true, 'readonly' => $isro);
	$a_input[] = array('kolom' => 'lokasitujuan', 'label' => 'Lokasi Tujuan', 'size' => 30, 'notnull' => true, 'readonly' => $isro);
	$a_input[] = array('kolom' => 'idlokasitujuan', 'type' => 'H', 'issave' => true);
	$a_input[] = array('kolom' => 'pegawaitujuan', 'label' => 'Pemakai Tujuan', 'size' => 30, 'notnull' => true, 'readonly' => $isro);
	$a_input[] = array('kolom' => 'idpegawaitujuan', 'type' => 'H', 'issave' => true);
					...
					<div class="box-content" style="width:<?= $p_tbwidth-22 ?>px">
					    <table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
	                        ...
	                        <tr>
		                        ...
		                        <td class="LeftColumnBG"><?= Page::getDataLabel($row,'lokasitujuan') ?></td>
		                        <td class="RightColumnBG">
		                            <?= Page::getDataInput($row,'lokasitujuan') ?>
		                            <?= Page::getDataInput($row,'idlokasitujuan') ?>
		                        </td>
	                        </tr>	                        				        
	                        <tr>
								...
								<td class="LeftColumnBG"><?= Page::getDataLabel($row,'pegawaitujuan') ?></td>
		                        <td class="RightColumnBG">
		                            <?= Page::getDataInput($row,'pegawaitujuan') ?>
					    ...
					    <table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center" class="GridStyle">
				            <tr>
					            <td colspan="<?= ($r_isverify == '1') ? '5' : '6' ?>" class="DataBG">Detail Mutasi</td>
				            </tr>
					        <tr>
					        ...
					        <tr valign="top">
					            ...
					            */?>
						        ...
						        <?	if($r_isverify != '1') { ?>
						        <td align="center">
							        <img title="Hapus Detail" src="images/delete.png" onclick="goDeleteDet('<?= $val['iddetmutasi'] ?>')" style="cursor:pointer">
						        </td>
						        <?	} ?>
					        </tr>
					        <?  }
					        ...
					        ?>
					        <tr>
					            <td colspan="<?= ($r_isverify == '1') ? '5' : '6' ?>" align="center"><b>-- Data tidak ditemukan --</b></td>
					        </tr>
					        <?
					        }
					        ?>
...
$(document).ready(function() {
	...
	// autocomplete
	$("#lokasitujuan").xautox({strpost: "f=acxlokasi", targetid: "idlokasitujuan"})
        .blur(function(){
            if($('#idlokasitujuan').val() == '')
                $(this).val('');
        });

	loadCBLokasiAsal();
	//loadCBLokasiTujuan();
    setAutoPegawaiTujuan();
    setAutoPegawaiAsal();
});
...
function loadCBLokasiAsal(){
    $.post(ajaxpage, 
        {f: 'optlokasibrg', idunit: $('#idunitasal').val(), idlokasi: $('#idlokasiasal').val()}, 
        function(data){
...
// autocomplete
function setAutoPegawaiTujuan(){
    <? /*$("#pegawaitujuan").xautox({strpost: "f=acxpegawai&idunit="+$('#idunittujuan').val(), targetid: "idpegawaitujuan"}) */?>
    $("#pegawaitujuan").xautox({strpost: "f=acxpegawai", targetid: "idpegawaitujuan"})
        .blur(function(){
            if($('#idpegawaitujuan').val() == '')
                $(this).val('');
        });
}

$('#idunittujuan').change(function(){
    $('#pegawaitujuan').unbind();
    $('#pegawaitujuan').val('');
    $('#idpegawaitujuan').val('');
...
function setAutoPegawaiAsal(){
    $("#pegawaiasal").xautox({strpost: "f=acxpemakai&idunit="+$('#idunitasal').val(), targetid: "idpegawaiasal"})
        .blur(function(){
            if($('#idpegawaiasal').val() == '')
                $(this).val('');
        });
}
