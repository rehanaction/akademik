...
<table border="0" width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="2" class="GridStyle" align="center">
	...
	<tr>
		<td class="LeftColumnBG" width="125">ID.</td>
		<td class="RightColumnBG"><?= $r_key ?></td>
	</tr>
	<tr>
...
<input type="hidden" name="id" id="id" value="<?= $r_key ?>">
...
var ajax_saldoawal = "<?= Route::navAddress('ajax_saldoawal') ?>";
...
function goSaveDet(){
	...
	if(pass) {
        ...
        $.post(ajax_saldoawal,
            ...
            function(err){
                if(err == '0'){
                    $.post(ajax_saldoawal,{ f: 'getRow', id: i_id }, function(row){
                        $('#tr_'+i_id).html(row);
                        $('.close').click();
                    });
                }else
