<?php
	...
	$sql ="select substring(s.idbarang1,1,1) as idbarang1, count(s.idseri) as total,s.idstatus
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,1) = substring(s.idbarang1,1,1) ";
    $sql .= "group by substring(s.idbarang1,1,1),s.idstatus order by substring(s.idbarang1,1,1) ";
    
    $rs = $conn->Execute($sql);
	while($row = $rs->FetchRow()){
        $a_barang[$row['idbarang1']][$row['idstatus']] = (int)$row['total'];
    }
	...
	$sql ="select substring(s.idbarang1,1,3) as idbarang1, count(s.idseri) as total,s.idstatus
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,3) = substring(s.idbarang1,1,3) ";
    $sql .= "group by substring(s.idbarang1,1,3),s.idstatus order by substring(s.idbarang1,1,3) ";
	...
	$sql ="select substring(s.idbarang1,1,5) as idbarang1, count(s.idseri) as total,s.idstatus
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,5) = substring(s.idbarang1,1,5) ";
    $sql .= "group by substring(s.idbarang1,1,5),s.idstatus order by substring(s.idbarang1,1,5) ";
	...
	$sql ="select substring(s.idbarang1,1,7) as idbarang1, count(s.idseri) as total,s.idstatus
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,7) = substring(s.idbarang1,1,7) ";
    $sql .= "group by substring(s.idbarang1,1,7),s.idstatus order by substring(s.idbarang1,1,7) ";
    ...
    $sql .= "group by s.idbarang1,b.namabarang order by substring(s.idbarang1,1,10)";
    ...
    $sql ="select substring(s.idbarang1,1,10) as idbarang1, count(s.idseri) as total,s.idstatus
            from aset.as_seri s 
            left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
            left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang1,1,10) = substring(s.idbarang1,1,10) ";
    $sql .= "group by substring(s.idbarang1,1,10),s.idstatus order by substring(s.idbarang1,1,10) ";
	...
	$sql ="select s.idbarang1 as idbarang1, count(s.idseri) as total,s.idstatus
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and s.idbarang1 = s.idbarang1 ";
    $sql .= "group by s.idbarang1,s.idstatus order by s.idbarang1 ";
...
<table class="tb_data" width="<?= $p_tbwidth ?>">
	...
	<tr valign="top">
	    ...
	    <?  foreach($a_status as $idstatus => $namastatus){
			$status[$idstatus] += $a_barang[$idbarang1][$idstatus];
		?>
	    <td align="right"><?= CStr::formatNumber($a_barang[$idbarang1][$idstatus]) ?></td>
	    <?  } ?>
	</tr>
	<?
	}
