...
<?php
		...
		function getImageInput() {
			...
			// iframe
			$input .= '<iframe name="foto_iframe" style="display:none"></iframe>';
		...
		function getImageFoto($conn,$key,$folder='fotopeg',$edit=true) {
			if($edit) {
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImageFoto($conn,$key,$folder,true).'" style="cursor:pointer">';
				$img .= '<br><em><font size="1px">Klik gambar untuk upload/hapus foto</font></em>';
				$img .= self::getImageInput();
			}
			else
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImageFoto($conn,$key,$folder,true).'">';
			
			return $img;
		}
		
		function getImageFotoRep($conn,$key,$folder='fotopeg') {
			global $conf;
			...
			$imgfoto = '<img id="imgfoto" border="1" src="'.$img.'">';
...
<?php
		...
		// foto Pelamar
		function getPathImagePelamar($conn,$key,$show=false) {
			global $conf;
		...
		function getImagePelamar($conn,$key,$edit=true) {
			if($edit) {
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImagePelamar($conn,$key,true).'" style="cursor:pointer">';
				$img .= '<br><em><font size="1px">Klik gambar untuk upload/hapus foto</font></em>';
				$img .= self::getImageInput();
			}
			else
				$img = '<img id="imgfoto" border="1" src="'.self::getPathImagePelamar($conn,$key,true).'">';
