<?php
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'kodetariflembur', 'label' => 'Kode', 'align' => 'center','width' => '60px');
	$a_kolom[] = array('kolom' => 'pendidikan', 'label' => 'Pendidikan','width' => '200px');
	$a_kolom[] = array('kolom' => 'jenishari', 'label' => 'Jenis Hari');
	$a_kolom[] = array('kolom' => 'harilibur', 'label' => 'is Hari Libur?', 'align' => 'center','width' => '80px');
	$a_kolom[] = array('kolom' => 'tariflembur', 'label' => 'Tarif Lembur','type'=>'N','align'=>'right');
	
	// properti halaman
	$p_title = 'Daftar Tarif Lembur';
	$p_tbwidth = 800;
	$p_aktivitas = 'ANGGARAN';
	$p_detailpage = Route::getDetailPage();
	$p_dbtable = "ms_tariflembur";
	...
	if($r_act == 'delete' and $c_delete) {
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_key,'ga_tariflembur',$p_key);
		...
		list($p_posterr,$p_postmsg) = $p_model::saveSalinTarifLembur($conn,$r_periode,$f_keyPeriode,$prosentase);
	...
	if (empty($r_sort)) $r_sort = 'kodetariflembur desc';
	...
	$sql = $p_model::listQueryTarifLembur();
					...
					<div class="filterTable" style="width:<?= $p_tbwidth-12 ?>px;">
						<table cellpadding="5" cellspacing="0" width="<?= $p_tbwidth-12 ?>px";>
							<tr>
								...
								<td width="70px"><?= UI::createSelect('speriodetarif',$a_salinperiode,'', 'ControlStyle',$c_edit,'',true) ?>
								...
								<td><input type="button" value="Salin Lembur" id="be_copy" class="ControlStyle" onClick="goCopy()"></td>
							</tr>
						</table>
					</div>
					<br>
...
function goCopy(){
	...
	var retval = confirm("Apakah anda yakin akan menyalin tarif lembur dari periode tarif '" + valPerTarif + "' ?");
	if(retval) {
