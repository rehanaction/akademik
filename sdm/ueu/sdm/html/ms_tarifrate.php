<?php
	...
	$a_kolom[] = array('kolom' => 'tarifgapok', 'label' => 'Gapok Rate','size' => 14, 'maxlength' => 14, 'type' => 'N');
	$a_kolom[] = array('kolom' => 'tarifkehadiran', 'label' => 'Kehadiran Rate','size' => 14, 'maxlength' => 14, 'type' => 'N');
	
	// properti halaman
	$p_title = 'Daftar Tarif Gapok dan Kehadiran Rate Mengajar';
	$p_tbwidth = 600;
	$p_aktivitas = 'ANGGARAN';
	$p_dbtable = 'ms_tarifrate';
	$p_key = 'idpendidikan';
	...
	if($r_act == 'insert' and $c_insert) {
		...
		if(empty($p_posterr))			
			list($p_posterr,$p_postmsg) = $p_model::setRateHonor($conn,'','','','null',$_POST['i_idpendidikan']);
	}
		...
		if(empty($p_posterr))			
			list($p_posterr,$p_postmsg) = $p_model::setRateHonor($conn,'','','','null',$_POST['u_idpendidikan']);
	}
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<?	/********/
						...
						foreach($a_data as $row) {
							...
							$t_key = $p_model::getKeyRow($row,'real_idpendidikan');
