<?php
	...
	// properti halaman
	$p_title = 'Data Riwayat Pangkat';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	$p_listpage = Route::getListPage();
	
	$p_model = mRiwayat;
	$p_dbtable = "pe_rwtpangkat";
	$where = 'nourutrp';
	
	//struktur view
	$a_input = array();
	//$a_input[] = array('kolom' => 'idkenaikanpangkat', 'label' => 'Jenis Kenaikan', 'type' => 'S', 'empty' => true, 'option' => $p_model::jenisKenaikanPangkat($conn));
	$a_input[] = array('kolom' => 'jenissk', 'label' => 'Jenis SK', 'type' => 'S', 'empty' => true, 'notnull' => true, 'option' => $p_model::jenisSK($conn));
	$a_input[] = array('kolom' => 'jenispangkat', 'label' => 'Jenis Pangkat', 'type' => 'S', 'empty' => true, 'notnull' => true, 'option' => $p_model::jenisPangkat());
	$a_input[] = array('kolom' => 'idpangkat', 'label' => 'Nama Pangkat', 'type' => 'S', 'option' => $p_model::namaPangkat($conn));
	$a_input[] = array('kolom' => 'tmtpangkat', 'label' => 'TMT. Pangkat', 'type' => 'D', 'notnull' => true);
	...
	$a_input[] = array('kolom' => 'flagacuan', 'label' => 'Acuan Masa Kerja', 'type' => 'C', 'option' => array('Y' => ''), 'add' => 'title="Centang sebagai acuan perhitungan masa kerja golongan"');
	$a_input[] = array('kolom' => 'isvalid', 'label' => 'Valid', 'type' => 'R', 'option' => SDM::getValid());
	$a_input[] = array('kolom' => 'filepangkat', 'label' => 'File Pangkat', 'type' => 'U', 'uptype' => 'filepangkat', 'size' => 40);
	...
	if($r_act == 'save' and $c_edit) {
		...
		$record['isefektif'] = $record['tmtpangkat'] <= date('Y-m-d') ? 'Y' : 'null';
		$conn->BeginTrans();
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,$p_dbtable,$where,'','filepangkat');
		
		if(!$p_posterr) Route::navListpage($p_listpage,$r_key);
	}
	else if($r_act == 'deletefile' and $c_delete) {
		list($p_posterr,$p_postmsg) = $p_model::deleteFile($conn,$r_subkey,$p_dbtable,'filepangkat',$where);
	...
	$sql = $p_model::getDataEditPangkat($r_subkey);
	...
	//pengecekan apakah sudah ada acuan masa kerja
	$acuan = $p_model::isExistAcuan($conn,$r_key,$r_subkey);
			...
			<form name="pageform" id="pageform" method="post" action="<?= Route::navAddress(Route::thisPage()) ?>" enctype="multipart/form-data">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td class="LeftColumnBG" width="20%" style="white-space:nowrap"><?= Page::getDataLabel($row,'jenissk') ?></td>
							<td  class="RightColumnBG" width="40%"><?= Page::getDataInput($row,'jenissk') ?></td>
							<td class="LeftColumnBG" width="20%" style="white-space:nowrap"><?= Page::getDataLabel($row,'jenispangkat') ?></td>
							<td  class="RightColumnBG" width="20%"><?= Page::getDataInput($row,'jenispangkat') ?></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td  class="RightColumnBG" <?= $acuan == true ? 'coslpan="3"' : ''?>><?= Page::getDataInput($row,'namapejabat') ?></td>
							<?if(!$acuan){?>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'flagacuan') ?></td>
							<td  class="RightColumnBG"><?= Page::getDataInput($row,'flagacuan') ?></td>
							<?}?>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'filepangkat') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'filepangkat') ?></td>
						</tr>
					</table>
					</div>
				</center>
