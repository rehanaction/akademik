...
?>
<div style="border:1px solid #CCC;width:200px;margin-right:15px;float:left;border-radius:5px">
	<center>
	...
	<font size="1">
		...
		<?		foreach($a_ctfilter as $t_filter) {
					list($t_id,$t_kode) = explode(':',$t_filter);
		?>
			<u class="ULink" title="Hapus filter ini" id="<?= $t_filter ?>"><?= $a_filtertree[$t_id]['label'] ?>: <?= getTreeLabel($t_kode,$a_filtertree[$t_id]['data']) ?></u><br>
		<?		} ?>
		</div>
		<?	} ?>
		<input type="button" value="Reset Filter" title="Hapus semua filter pada tree" onclick="goRemoveFilterTree()">
	</font>
	</div>
</div>
<?php
