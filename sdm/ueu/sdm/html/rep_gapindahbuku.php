<?php
	...
	$p_tbwidth = 1000;
	$p_file = 'gajipemindahanbuku_penunjang_'.$r_kodeunit;
	$p_model = 'mGaji';
	$p_window = 'Rekapitulasi Gaji Penunjang';
	$p_col = 8;
    ...
    $a_laporan = $p_model::repLapPindahBuku($conn,$r_periode,$r_kodeunit, $sqljenis);
...
<body>
	<div align="center">
		...
		<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" style="border-collapse:collapse;border:1px solid black">
			...
			<tbody>
			<? 	
				$totgajibruto=0;$totalbuku=0;$totalsaldo=0;$pph=0;$isunit=0;
				if (count($a_data) >0) {
					$i=0;
					foreach($a_data as $row){ 
						$totgajibruto += $row['gajibruto'];
						if($row['norekening'] != '' and ($row['istunda'] != 'Y' or $row['istunda'] ==''))
							$totalbuku += $row['gajiditerima'];
						else
							$totalsaldo += $row['gajiditerima'];
						$pph += $row['pph'];
						
						if($jenis != $row['idjenispegawai']){
							$isunit++;
							if($isunit > 1){
			?>
				<tr>
					<td style="border:1px solid black;" colspan="4" align="center"><strong>Sub Total</strong></td>
			...
			?>
				<tr>
					...
					<td style="border-right:1px solid black;"></td>
					<td style="border-right:1px solid black;"><u><b><?= $row['jenispegawai']?></b></u></td>
					<td style="border-right:1px solid black;"></td>
					<td style="border-right:1px solid black;"></td>
					<?
						if(count($a_jns)>0){
				...
				<tr>
					...
					<td style="border-right:1px solid black;" align="center"><?= $row['nik']; ?></td>
					<td style="border-right:1px solid black;"><?= $row['namalengkap']; ?></td>
					<td style="border-right:1px solid black;"><?= $row['jabatanstruktural']; ?></td>
				...
				?>
				<tr>
					<td style="border-right:1px solid black;" colspan="<?= $p_col+count($a_jns)?>" align="center">Data tidak ditemukan</td>
				</tr>
				<? } ?>
				<tr>
					<td style="border:1px solid black;" colspan="4" align="center"><strong>Sub Total</strong></td>
				...
				<tr>
					<td style="border-right:1px solid black;border-top:1px solid black;" colspan="4" align="center"><strong>Total</strong></td>
					<td style="border-right:1px solid black;border-top:1px solid black;" align="right"><strong><?= CStr::formatNumber($totgajibruto,0,$dot); ?><strong></td>
					<?
						if(count($a_jns)>0){
							foreach($a_jns as $key=>$val){
					?>
					<td style="border-right:1px solid black;border-top:1px solid black;" align="right"><strong><?= CStr::formatNumber($totpotongan[$key],0,$dot)?></strong></td>
					<?}}?>
					<td style="border-right:1px solid black;border-top:1px solid black;" align="right"><strong><?= CStr::formatNumber($totalbuku,0,$dot); ?><strong></td>
					<td style="border-right:1px solid black;border-top:1px solid black;" align="right"><strong><?= CStr::formatNumber($totalsaldo,0,$dot); ?><strong></td>
					<td style="border-right:1px solid black;border-top:1px solid black;" align="right"><strong><?= CStr::formatNumber($pph,0,$dot); ?><strong></td>
				</tr>
			</tbody>
		</table>
		<br />
