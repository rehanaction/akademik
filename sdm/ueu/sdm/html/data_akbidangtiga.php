<?php
	...
	// properti halaman
	$p_title = 'Data Bidang III (Pengabdian Masyarakat)';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	$p_listpage = Route::getListPage();
	
	$p_model = mAngkaKredit;
	$p_dbtable = "ak_bidang3";
	$where = 'nobidangiii';
	
	//struktur view
	$a_input = array();
	$a_input[] = array('kolom' => 'namakegiatan', 'label' => 'Pengabdian Masyarakat', 'type' => 'A', 'rows' => 2, 'cols' => 50, 'notnull' => true, 'class' => 'ControlRead');
	$a_input[] = array('kolom' => 'idpkm', 'type' => 'H');
	$a_input[] = array('kolom' => 'namapkm', 'label' => 'Nama Pengabdian', 'maxlength' => 100, 'size' => 70, 'class' => 'ControlRead');
	$a_input[] = array('kolom' => 'tempatkegiatan', 'label' => 'Lokasi', 'type' => 'A', 'rows' => 2, 'cols' => 50, 'class' => 'ControlRead');
	$a_input[] = array('kolom' => 'tgl', 'label' => 'Tgl. Kegiatan','type' => 'D', 'maxlength' => 10, 'size' => 10, 'class' => 'ControlRead');
	...
	$a_input[] = array('kolom' => 'filebidangtiga', 'label' => 'File Bidang III', 'type' => 'U', 'uptype' => 'filebidangtiga', 'size' => 40);
	...
	if($r_act == 'save' and $c_edit) {
		...
		$nilaikredit = $p_model::getAKPKM($conn,$record['idpkm'],$record['idkegiatan']);
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,$p_dbtable,$where,'','filebidangtiga');
		
		if(!$p_posterr) Route::navListpage($p_listpage,$r_key);
	}
	else if($r_act == 'deletefile' and $c_delete) {
		list($p_posterr,$p_postmsg) = $p_model::deleteFile($conn,$r_subkey,$p_dbtable,'filebidangtiga',$where);				
		?>
	...
	$sql = $p_model::getDataEditBidang3($r_subkey);	
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							...
							<td  class="RightColumnBG" colspan="3">
								...
								<?= Page::getDataInput($row,'idpkm') ?>
								<span id="edit" style="display:none;"><img src="images/magnify.png" title="Pilih dari daftar pengabdian" style="cursor:pointer" onclick="showPengabdian()"></span>
							</td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'namapkm') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'namapkm') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'filebidangtiga') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'filebidangtiga') ?></td>
						</tr>
					</table>
					</div>
				</center>
...
function showPengabdian(){
	win = window.open("<?= Route::navAddress('pop_pengabdian').'&key='.$r_key?>","popup_pengabdian","width=950,height=600,scrollbars=1");
	win.focus();
}

function showIndeks(){
	win = window.open("<?= Route::navAddress('pop_penilaian').'&m=2&b=III'?>","popup_penilaian","width=650,height=500,scrollbars=1");
	win.focus();
}
</script>
</body>
