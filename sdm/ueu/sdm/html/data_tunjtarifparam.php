<? 
	...
	$r_tunjangan = !empty($_REQUEST['tunjangan']) ? CStr::removeSpecial($_REQUEST['tunjangan']) : CStr::removeSpecial($_REQUEST['kodetunjangan']);
	...
	$a_input = array();	
	$a_input[] = array('kolom' => 'kodetunjangan', 'label' => 'Jenis Tunjangan', 'type' => 'S', 'option' => $p_model::getCTunjParameter($conn), 'notnull' => true, 'add' => 'onchange="goCek(this.value)"', 'default' => $r_tunjangan);
	...
	$a_input[] = array('kolom' => 'tmtmulai', 'label' => 'Tgl. Berlaku', 'type' => 'D', 'notnull' => true);	
	$a_input[] = array('kolom' => 'nominal', 'label' => 'Tarif Tunjangan', 'maxlength' => 12, 'size' => 15, 'type' => 'N');	
	...
	$sql = $p_model::getDataEditTunjTarifParam($r_key);
	...
	foreach($row as $t_row) {
		if($t_row['notnull'])
			$a_required[] = $t_row['id'];
		if($t_row['id'] == 'kodetunjangan' and empty($r_tunjangan))
			$r_tunjangan = $t_row['realvalue'];
	}
