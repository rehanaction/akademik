<?php
	...
	// properti halaman
	$p_title = 'Daftar Sanksi Pelanggaran';
	$p_tbwidth = 800;
	$p_aktivitas = 'HISTORY';
	...
	$a_kolom[] = array('kolom' => 'tgltmt', 'label' => 'Tgl. Mulai', 'type' => 'D', 'align' => 'center', 'width' => '150px');
	$a_kolom[] = array('kolom' => 'tglselesai', 'label' => 'Tgl. Selesai', 'type' => 'D', 'align' => 'center', 'width' => '150px');
	$a_kolom[] = array('kolom' => 'jenispelanggaran', 'label' => 'Pelanggaran', 'filter' => 'j.jenispelanggaran');
	...
	if(empty($r_sort)) $r_sort = 'tgltmt desc';
	$a_filter = Page::setFilter($_POST['filter']);
	$a_datafilter = Page::getFilter($a_kolom);
	
	$sql = $p_model::listQuerySanksi($r_key);
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<?
						if(count($sk[$row['idjenispelanggaran']]) > 0){
							foreach($sk[$row['idjenispelanggaran']] as $keysk => $valsk){
								echo '<li>'.$valsk.'</li>';
							}
						}else
