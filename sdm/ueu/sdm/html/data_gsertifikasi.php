<?php
	...
	// properti halaman
	$p_title = 'Data Sertifikasi';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	$p_listpage = Route::getListPage();
	
	$p_model = mPengembangan;
	$p_dbtable = "pe_sertifikasi";
	$where = 'idsertifikasi';
	
	//struktur view
	$a_input = array();
	$a_input[] = array('kolom' => 'bulan', 'label' => 'Periode Sertifikasi', 'empty' => true, 'type' => 'S', 'option' => Date::arrayMonth());
	$a_input[] = array('kolom' => 'tahun', 'maxlength' => 4, 'size' => 4);
	$a_input[] = array('kolom' => 'kodesertifikasi', 'label' => 'Jenis Sertifikasi', 'type' => 'S', 'empty' => true, 'notnull' => true, 'option' => $p_model::jenisSertifikasi($conn));
	$a_input[] = array('kolom' => 'tglsertifikasi', 'label' => 'Tgl. Sertifikasi', 'type' => 'D', 'notnull' => true);
	$a_input[] = array('kolom' => 'nosertifikasi', 'label' => 'No. Sertifikasi', 'maxlength' => 50, 'size' => 50);
	$a_input[] = array('kolom' => 'keterangan', 'label' => 'Keterangan', 'type' => 'A', 'rows' => 2, 'cols' => 50, 'maxlength' => 255);
	$a_input[] = array('kolom' => 'statussertifikasi', 'label' => 'Lingkup', 'type' => 'S', 'empty' => true, 'option' => $p_model::lingkup());
	...
	$a_input[] = array('kolom' => 'filesertifikasi', 'label' => 'File Sertifikasi', 'type' => 'U', 'uptype' => 'filesertifikasi', 'size' => 40);
	...
	if($r_act == 'save' and $c_edit) {
		...
		$record['isefektif'] = $record['tglsertifikasi'] <= date('Y-m-d') ? 'Y' : 'null';
		if($record['bulan'] != 'null' and $record['tahun'] != 'null')
			$record['periodesertifikasi'] = $record['tahun'].str_pad($record['bulan'],2,'0', STR_PAD_LEFT);
		$conn->BeginTrans();
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,$p_dbtable,$where,'','filesertifikasi');
		
		if(!$p_posterr) Route::navListpage($p_listpage,$r_key);
	}
	else if($r_act == 'deletefile' and $c_delete) {
		list($p_posterr,$p_postmsg) = $p_model::deleteFile($conn,$r_subkey,$p_dbtable,'filesertifikasi',$where);				
		?>
	...
	$sql = $p_model::getDataEditSertifikasi($r_subkey);
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							...
							<td class="LeftColumnBG" width="20%" style="white-space:nowrap"><?= Page::getDataLabel($row,'kodesertifikasi') ?></td>
							<td  class="RightColumnBG" width="20%"><?= Page::getDataInput($row,'kodesertifikasi') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'tglsertifikasi') ?></td>
							<td  class="RightColumnBG"><?= Page::getDataInput($row,'tglsertifikasi') ?></td>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'nosertifikasi') ?></td>
							<td  class="RightColumnBG"><?= Page::getDataInput($row,'nosertifikasi') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'statussertifikasi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'statussertifikasi') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'filesertifikasi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'filesertifikasi') ?></td>
						</tr>
					</table>
					</div>
				</center>
