<?php
	...
	$a_kolom[] = array('kolom' => 'tgllembur', 'label' => 'Tgl. Lembur', 'type' => 'D', 'align' => 'center', 'width' => '150px');
	$a_kolom[] = array('kolom' => 'jamlembur', 'label' => 'Jam Lembur','filter' => "substring(jamawal,1,2)+':'+substring(jamawal,3,2)+' - '+substring(jamakhir,1,2)+':'+substring(jamakhir,3,2)");
	$a_kolom[] = array('kolom' => 'namapejabat', 'label' => 'Pejabat Pemberi Tugas','filter' => "coalesce(p.nik+' - ','')+sdm.f_namalengkap(gelardepan,namadepan,namatengah,namabelakang,gelarbelakang)+' - '+st.jabatanstruktural");
	...
	if(empty($r_sort)) $r_sort = 'tglpenugasan desc';
	$a_filter = Page::setFilter($_POST['filter']);
	$a_datafilter = Page::getFilter($a_kolom);
	
	$sql = $p_model::listQueryRSuratLembur($r_key);
	
	$a_data = $p_model::getPagerData($conn,$a_kolom,$r_row,$r_page,$r_sort,$a_filter,$sql);
	$p_lastpage = Page::getLastPage();
		
	$isPejabat = $p_model::isPejabat($conn, $r_key);
	if ($isPejabat){
		$c_insert = false;
		$p_posterr = true;
		$p_postmsg = 'Pejabat Struktural tidak bisa mengajukan permohonan lembur';
	}
