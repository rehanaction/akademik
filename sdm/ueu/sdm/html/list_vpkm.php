<?php
	...
	// properti halaman
	$p_title = 'Daftar Riwayat Pengabdian Masyarakat';
	$p_tbwidth = 1050;
	$p_aktivitas = 'DAFTAR';
	$p_key = 'idpkm';
	$p_dbtable = 'pe_pkm';
	...
	$a_kolom[] = array('kolom' => 'tglawal', 'label' => 'Tgl. Mulai', 'type' => 'D', 'align' => 'center');
	$a_kolom[] = array('kolom' => 'namakegiatan', 'label' => 'Nama Kegiatan');
	$a_kolom[] = array('kolom' => 'namapkm', 'label' => 'Jenis PKM');
	$a_kolom[] = array('kolom' => 'filepkm', 'label' => 'File PKM');
	...
	if($r_act == 'save' and $c_edit) {
		for($i=0;$i<count($_POST['kode']);$i++){
			...
			if(!empty($is)){
				...
				if(!$err)
					$p_model::updateRecord($conn,$rec,$r_ida,false,$p_dbtable,'refpkm');
			}
		}
	}
	...
	if(empty($r_sort)) $r_sort = 'nik,tglawal desc';
	...
	$sql = $p_model::listQueryValidasiPKM();
	$a_data = $p_model::getPagerData($conn,$a_kolom,$r_row,$r_page,$r_sort,$a_filter,$sql);
	$p_lastpage = Page::getLastPage();
	
	//mendapatkan tim penelitian
	$a_tim = mPengembangan::listTimPKMAll($conn);
	...
	<div id="wrapper" style="width:<?= $p_tbwidth+72 ?>px">
		<div class="SideItem" id="SideItem" style="width:<?= $p_tbwidth+50 ?>px">
			<form name="pageform" id="pageform" method="post">
				...
				<input type="hidden" name="link" id="link" value="5">
			</form>
		</div>
	</div>
</div>
