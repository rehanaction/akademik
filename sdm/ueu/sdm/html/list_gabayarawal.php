<?php
	...
	$r_bayar = Modul::setRequest($_POST['bayar'],'BAYARTUNJ');
	...
	// properti halaman
	$p_title = 'Daftar Pembayaran Gaji Awal';
	$p_tbwidth = 1100;
	...
	if(!empty($r_bayar)) $a_filter[] = $p_model::getListFilter('bayartunj',$r_bayar);
		
	$sql = $p_model::listQueryGajiBayarAwal();
	if(count($a_datafilter) > 0)
		$a_sql = $p_model::getListQuery($r_sort,$a_filter,$sql,$table,$r_row);
	
	// ada aksi
	$r_act = $_POST['act'];
	if($r_act == 'bayar' and $c_insert) {
		$conn->BeginTrans();
		
		list($p_posterr,$p_postmsg) = $p_model::bayarTunjangan($conn,$r_periode,$a_sql);
		...
		list($p_posterr,$p_postmsg) = $p_model::tundaBayarTunjangan($conn,$r_periode,$a_sql);
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td align="center">
							<img id="<?= $t_key.'::list_gabayarawal' ?>" title="Tampilkan Slip Gaji" src="images/edit.png" onclick="goDetail(this)" style="cursor:pointer">
							<?if($row['isbayarawal'] == 'Y'){?>
...
function goBayar() {
	var hitung = confirm("Apakah anda ingin membayarkan gaji awal sekarang?");
	if(hitung) {
