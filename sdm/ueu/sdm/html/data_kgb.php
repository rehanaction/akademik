<?php
	...
	$r_pegawai = CStr::removeSpecial($_POST['pegawai']);
	...
	// properti halaman
	$p_title = 'Data Kenaikan Gaji Bekala';
	$p_tbwidth = 650;
	$p_aktivitas = 'DATA';
	$p_dbtable = 'pe_kgb';
	$p_key = 'nokgb';
	...
	$a_input = array();
	if(empty($r_key)){
		$a_input[] = array('kolom' => 'tglkgb', 'label' => 'Tgl. Kenaikan', 'type' => 'D', 'notnull' => true, 'default' => date('Y-m-d'));
		$a_input[] = array('kolom' => 'pegawai', 'label' => 'Nama Pegawai', 'maxlength' => 100, 'size' => 50, 'notnull' => true,'default'=>$r_pegawai);
	}else{
		$a_input[] = array('kolom' => 'tglkgb', 'label' => 'Tgl. Kenaikan', 'type' => 'D', 'readonly' => true);
	...
	$a_input[] = array('kolom' => 'golonganlama', 'label' => 'Pangkat Lama', 'readonly' => true);
	...
	$a_input[] = array('kolom' => 'pangkatbaru', 'label' => 'Pangkat Baru', 'type' => 'S', 'option' => mRiwayat::namaPangkat($conn));
	...
	if($r_act == 'save' and $c_edit) {
		...
		if(count($pktold) > 0){
			$pktold['pangkatlama'] = $pktold['idpangkatlama'];
			$record = array_merge($record,$pktold);
		}
			
		if($record['mkthn'] != 'null' and $record['mkbln'] != 'null')
			$record['mkgbaru'] = str_pad($record['mkthn'],2,'0',STR_PAD_LEFT).str_pad($record['mkbln'],2,'0',STR_PAD_LEFT);
		...
		$r_tgl = CStr::removeSpecial($_POST['tglkgb']);
	
		$a_naik = $p_model::isNaikGaji($conn,$r_peg,$r_tgl);
		
		if(!empty($a_naik['kgb'])){
			$p_posterr = true;
			$p_postmsg = 'Pegawai tersebut sudah dilakukan proses kenaikan gaji berkala untuk bulan ini';
		}
		if(empty($a_naik['row'])){
			$p_posterr = true;
			$p_postmsg = 'Pegawai tersebut belum waktunya untuk naik gaji';
		}
		if($a_naik['isnaik']){
			$isnaik = true;
			$sql = $a_naik['sql'];
			...
			<form name="pageform" id="pageform" method="post">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td class="LeftColumnBG" width="150" style="white-space:nowrap"><?= Page::getDataLabel($row,'tglkgb') ?></td>
							<td class="RightColumnBG"><?= Page::getDataInput($row,'tglkgb') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'golonganlama') ?></td>
							<td class="RightColumnBG"><?= Page::getDataInput($row,'golonganlama') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'pangkatbaru') ?></td>
							<td class="RightColumnBG"><?= Page::getDataInput($row,'pangkatbaru') ?></td>
						</tr>
						<tr>
