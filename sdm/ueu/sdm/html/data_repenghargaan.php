<?php
	...
	// properti halaman
	$p_title = 'Data Penghargaan';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	$p_listpage = Route::getListPage();
	
	$p_model = mPenghargaan;
	$p_dbtable = "pe_penghargaan";
	$where = 'nourutpenghargaan';
	
	//struktur view
	$a_input = array();
	$a_input[] = array('kolom' => 'tglpenghargaan', 'label' => 'Tgl. Penghargaan', 'type' => 'D', 'notnull' => true);
	$a_input[] = array('kolom' => 'bentukreward', 'label' => 'Nama Penghargaan', 'maxlength' => 100, 'size' => 70, 'notnull' => true);
	...
	$a_input[] = array('kolom' => 'filepenghargaan', 'label' => 'File Penghargaan', 'type' => 'U', 'uptype' => 'filepenghargaan', 'size' => 40);
	...
	if($r_act == 'save' and $c_edit) {
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,$p_dbtable,$where,'','filepenghargaan');
		
		if(!$p_posterr) Route::navListpage($p_listpage,$r_key);
	}
	else if($r_act == 'deletefile' and $c_delete) {
		list($p_posterr,$p_postmsg) = $p_model::deleteFile($conn,$r_subkey,$p_dbtable,'filepenghargaan',$where);				
		?>
			...
			<form name="pageform" id="pageform" method="post" action="<?= Route::navAddress(Route::thisPage()) ?>" enctype="multipart/form-data">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td class="LeftColumnBG" width="20%" style="white-space:nowrap"><?= Page::getDataLabel($row,'tglpenghargaan') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'tglpenghargaan') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'bentukreward') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'bentukreward') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'nosk') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'nosk') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'tglsk') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'tglsk') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'filepenghargaan') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'filepenghargaan') ?></td>
						</tr>
					</table>
					</div>
				</center>
