<?php
	...
	// properti halaman
	$p_title = 'Daftar Bidang II (Penelitian)';
	$p_tbwidth = 800;
	$p_aktivitas = 'HISTORY';
	...
	$a_kolom[] = array('kolom' => 'tgl', 'label' => 'Tgl. Kegiatan', 'type' => 'D', 'align' => 'center', 'width' => '100px');
	$a_kolom[] = array('kolom' => 'judulpenelitian', 'label' => 'Judul Penelitian', 'filter' => 'p.judulpenelitian');
	...
	if($r_act == 'delete' and $c_delete) {
		...
		$where = 'idpegawai,nobidangii';
		
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$a_key,$p_dbtable,$where,'','filebidangdua');
	}
	...
	if(empty($r_sort)) $r_sort = 'tgl desc';
	$a_filter = Page::setFilter($_POST['filter']);
	$a_datafilter = Page::getFilter($a_kolom);
	
	$sql = $p_model::listQueryBidang2($r_key);
