<?php
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'kodetunjangan', 'label' => 'Kode');
	$a_kolom[] = array('kolom' => 'namatunjangan', 'label' => 'Tunjangan');
	$a_kolom[] = array('kolom' => 'carahitung', 'label' => 'Cara Hitung', 'align' => 'center');
	
	// properti halaman
	$p_title = 'Daftar Jenis Tunjangan';
	$p_tbwidth = 800;
	...
	if (empty($r_sort)) $r_sort = 'kodetunjangan';
	$a_filter = Page::setFilter($_POST['filter']);
	$a_datafilter = Page::getFilter($a_kolom);
	
	$sql = $p_model::listQueryTunjangan();
	...
	//cara hitung
	$a_cara = $p_model::aCaraHitungTunj();
?>
<html>
<head>
	<title><?= $p_title ?></title>
		...
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<?	} ?>
						<th>Termasuk Gaji Tetap</th>
						<th>Dibayar Bersama Gaji</th>
						<th>Dikalikan Jam Kerja</th>
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td<?= $t_align ?>><?= $rowcc ?></td>
						<?	} ?>
						<td align="center"><?= $row['isgajitetap'] == 'Y' ? '<img src="images/check.png">' : ''; ?></td>
						<td align="center"><?= $row['isbayargaji'] == 'Y' ? '<img src="images/check.png">' : ''; ?></td>
						<td align="center"><?= $row['iskeldosen'] == 'Y' ? '<img src="images/check.png">' : ''; ?></td>
						<?	if($c_edit or $c_delete) { ?>
						<td align="center">
						<?		if($c_edit) { ?>
			...
			<div class="filterTable" style="width:<?= $p_tbwidth-12 ?>px;">
				<table cellspacing="0" cellpadding="0" align="center" width="100%">
					<tr>
						<td colspan="2"><strong>Keterangan Cara Hitung :</strong></td>
					</tr>
					<tr>
					<? 
					$i=0;
					foreach ($a_cara as $kode => $label){
					?>
						<td width="20"><?= $kode; ?></td>
						<td>: <?= $label; ?></td>
					<? 
