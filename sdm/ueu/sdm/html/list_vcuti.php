<?php
	...
	$a_kolom[] = array('kolom' => 'tglpengajuan', 'label' => 'Tgl. Pengajuan', 'type' => 'D', 'align' => 'center');
	$a_kolom[] = array('kolom' => 'jeniscuti', 'label' => 'Jenis Cuti');
	$a_kolom[] = array('kolom' => 'alasancuti', 'label' => 'Keperluan');
	$a_kolom[] = array('kolom' => 'lama', 'label' => 'Lama Cuti', 'align' => 'center');
	...
	if($r_act == 'save' and $c_edit) {
		for($i=0;$i<count($_POST['kode']);$i++){
			...
			$r_tgl = CStr::formatDate($_POST['tglp'][$i]);
			$is = CStr::cAlphaNum($_POST['check'.$r_ida]);
			if(!empty($is)){
				$rec['isvalid'] = 'Y';
				$nosurat = mCuti::getNoSuratCuti($conn,$r_ida,$r_tgl);
				if($nosurat != 'null')
					$rec['nosurat'] = $nosurat;
	...
	if(empty($r_sort)) $r_sort = 'nik,tglpengajuan desc';
	...
	$sql = $p_model::listQueryValidasiCuti();
	...
	<div id="wrapper">
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td align="center">
							...
							<input type="hidden" name="tglp[]" id="tglp[]" value="<?= CStr::formatDate($row['real_tglpengajuan']) ?>">
						</td>
						<?	} ?>
					</tr>
					<?	}
				...
				<input type="hidden" name="idx" id="idx" value="6">
