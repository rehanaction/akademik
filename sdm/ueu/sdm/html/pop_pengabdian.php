<?php
	...
	// properti halaman
	$p_title = 'Daftar Pengabdian Masyarakat';
	$p_tbwidth = 900;
	$p_col = 5;
	
	$p_model = mAngkaKredit;
	
	$a_data = $p_model::getPengabdian($conn,$r_key);
	...
	<?							
		...
		if(count($a_data)>0){
			foreach($a_data as $row) {
				...
				$send = $row['idpkm'].'|'.preg_replace("/[^a-z0-9_\-\.]/i"," ",$row['namakegiatan']).'|'.preg_replace("/[^a-z0-9_\-\.]/i"," ",$row['namapkm']).'|'.preg_replace("/[^a-z0-9_\-\.]/i"," ",$row['tempatkegiatan']).'|'.CStr::formatDate($row['tglawal']).'|'.$row['idkegiatan'].'|'.$row['kodekegiatan'].'|'.$row['kegiatan'].'|'.number_format($row['stdkredit'],2);
	?>
	<tr valign="top" class="<?= $rowstyle ?>">
		<td align="center">
			<u onClick="goSend('<?= $send ?>');" style="cursor:pointer;color:#3300FF;" title="Pilih untuk angka kredit"><?= $i; ?></u>
		</td>
		<td><?= CStr::formatDateInd($row['tglawal'])?></td>
		<td><?= CStr::formatDateInd($row['tglakhir'])?></td>
...
function goSend(send) {
	sendto = send.split('|');
	window.opener.document.getElementById("idpkm").value = sendto[0];
	window.opener.document.getElementById("namakegiatan").value = sendto[1];
	window.opener.document.getElementById("namapkm").value = sendto[2];
	window.opener.document.getElementById("tempatkegiatan").value = sendto[3];
