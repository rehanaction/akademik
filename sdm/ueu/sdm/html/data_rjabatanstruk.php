<?php
	...
	// properti halaman
	$p_title = 'Data Riwayat Jabatan Struktural';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	...
	//struktur view
	$a_input = array();
	$a_input[] = array('kolom' => 'idjnspejabat', 'label' => 'Status Pejabat', 'type' => 'S', 'option' => $p_model::jenisPejabat($conn));
	$a_input[] = array('kolom' => 'idjstruktural', 'label' => 'Nama Jabatan', 'type' => 'S', 'option' => mCombo::strukturalSave($conn,false));
	$a_input[] = array('kolom' => 'tmtmulai', 'label' => 'Tgl. Mulai', 'type' => 'D', 'notnull' => true);
	$a_input[] = array('kolom' => 'tmtselesai', 'label' => 'Tgl. Selesai', 'type' => 'D');
	...
	$a_input[] = array('kolom' => 'isutama', 'label' => 'Utama', 'type' => 'R', 'option' => $p_model::isUtama());
	$a_input[] = array('kolom' => 'isaktif', 'label' => 'Aktif', 'type' => 'R', 'option' => mCombo::isAktif());
	$a_input[] = array('kolom' => 'isvalid', 'label' => 'Valid', 'type' => 'R', 'option' => SDM::getValid());
	$a_input[] = array('kolom' => 'isgaji', 'label' => 'Gaji', 'type' => 'R', 'option' => SDM::getValid());
	$a_input[] = array('kolom' => 'filestruktural', 'label' => 'File Jabatan', 'type' => 'U', 'uptype' => 'filestruktural', 'size' => 40);
	...
	if($r_act == 'save' and $c_edit) {
		...
		if($record['tmtmulai'] <= date('Y-m-d')){
			$record['isefektif'] = 'Y';
			if($record['tmtselesai'] == 'null' or $record['tmtselesai'] >= date('Y-m-d'))
				$record['isaktif'] = 'Y';
			else
				$record['isaktif'] = 'T';
		}
		else
			$record['isefektif'] = 'null';
		...
		if(!$p_posterr){
			$p_posterr = mIntegrasi::savePejabatRole($conn,$r_key);
			if($p_posterr)
				$p_postmsg = 'Penyimpanan Pejabat Role ke Gate gagal';
		}
		...
		if(!$p_posterr){
			...
			if($p_posterr)
				$p_postmsg = 'Penyimpanan Kepala Unit gagal';
		}
		...
		if(!$p_posterr)
			list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,$p_dbtable,$where,'','filestruktural');
		...
		list($p_posterr,$p_postmsg) = $p_model::deleteFile($conn,$r_subkey,$p_dbtable,'filestruktural',$where);				
		?>
	...
	$sql = $p_model::getDataEditStruktural($r_subkey);
			...
			<form name="pageform" id="pageform" method="post" action="<?= Route::navAddress(Route::thisPage()) ?>" enctype="multipart/form-data">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap" width="200px"><?= Page::getDataLabel($row,'idjnspejabat') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'idjnspejabat') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'idjstruktural') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'idjstruktural') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap" width="20%"><?= Page::getDataLabel($row,'tmtmulai') ?></td>
							<td  class="RightColumnBG" width="40%"><?= Page::getDataInput($row,'tmtmulai') ?></td>
							<td class="LeftColumnBG" style="white-space:nowrap" width="20%"><?= Page::getDataLabel($row,'tmtselesai') ?></td>
							<td  class="RightColumnBG" width="20%"><?= Page::getDataInput($row,'tmtselesai') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'isutama') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'isutama') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'isgaji') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'isgaji') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'filestruktural') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'filestruktural') ?></td>
						</tr>
					</table>
					</div>
				</center>
