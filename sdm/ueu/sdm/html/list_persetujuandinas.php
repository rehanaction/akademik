<?php
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'tglusulan', 'label' => 'Tgl. Pengajuan', 'type' => 'D', 'align' => 'center', 'width' => '150px');
	$a_kolom[] = array('kolom' => 'nik', 'label' => 'NIP');
	$a_kolom[] = array('kolom' => 'namalengkap', 'label' => 'Nama', 'filter' => 'sdm.f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang)');
	$a_kolom[] = array('kolom' => 'namajenisdinas', 'label' => 'Jenis Kedinasan');
	$a_kolom[] = array('kolom' => 'dalamrangka', 'label' => 'Dalam Rangka');
	
	// properti halaman
	$p_title = 'Daftar Persetujuan Tugas Kedinasan';
	$p_tbwidth = 900;
	$p_aktivitas = 'DAFTAR';
	$p_key = 'nodinas';
	...
	// mendapatkan data
	if(!empty($r_unit)) $a_filter[] = $p_model::getListFilter('unitjab',$r_unit);
	if(!empty($r_month)) $a_filter[] = $p_model::getListFilter('bulandinas',str_pad($r_month, 2, "0", STR_PAD_LEFT));
	if(!empty($r_tahun)) $a_filter[] = $p_model::getListFilter('tahundinas',$r_tahun);
	
	$a_persetujuan = SDM::statusPersetujuan();
	$sql = $p_model::listQueryPersetujuanDinas();
		...
		<div class="SideItem" id="SideItem">
			...
			<div class="filterTable" style="width:<?= $p_tbwidth-12 ?>px;">
				<table cellspacing="0" cellpadding="0" align="center" width="890">
					<tr>
						<td colspan="2"><strong>Keterangan Status Persetujuan :</strong></td>
					</tr>
					<? foreach ($a_persetujuan as $kode => $label){?>
					<tr>
						<td width="20"><?= $kode; ?></td>
						<td>: <?= $label; ?></td>
					</tr>
