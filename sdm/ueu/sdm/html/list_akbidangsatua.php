<?php
	...
	// properti halaman
	$p_title = 'Daftar Bidang IA (Pendidikan)';
	$p_tbwidth = 800;
	$p_aktivitas = 'HISTORY';
	...
	$a_kolom[] = array('kolom' => 'tglijazah', 'label' => 'Tgl. Kegiatan', 'type' => 'D', 'align' => 'center', 'width' => '100px');
	$a_kolom[] = array('kolom' => 'namapendidikan', 'label' => 'Pendidikan', 'filter' => 'j.namapendidikan');
	...
	if($r_act == 'delete' and $c_delete) {
		...
		$where = 'idpegawai,nobidangia';
		
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$a_key,$p_dbtable,$where,'','filebidangsatua');
	}
	...
	if(empty($r_sort)) $r_sort = 'tglijazah desc';
	$a_filter = Page::setFilter($_POST['filter']);
	$a_datafilter = Page::getFilter($a_kolom);
	
	$sql = $p_model::listQueryBidangIA($r_key);
