<?php
	...
	// properti halaman
	$p_title = 'Daftar Range Kategori Nilai';
	$p_tbwidth = 800;
	$p_aktivitas = 'NILAI';
	...
	$p_dbtable = 'pa_indeksnilaiobyektif';
	
	// struktur view
	$a_kolom = array();

	$a_kolom[] = array('kolom' => 'indeksnilai', 'label' => 'Indeks');
	$a_kolom[] = array('kolom' => 'batasatas', 'label' => 'Batas Atas', 'type' => 'N,2');
	$a_kolom[] = array('kolom' => 'batasbawah', 'label' => 'Batas Bawah', 'type' => 'N,2');
	...
	if($r_act == 'delete' and $c_delete) {
		...
		$where = 'kodeperiodebobot,indeksnilai';
	...
	if (empty($r_sort)) $r_sort = 'indeksnilai';
	$a_datafilter = Page::getFilter($a_kolom);
	
	$sql = $p_model::listQueryIndeksObj($r_key);
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td align="center">
						<?		if($c_edit) { ?>
							<img id="<?= $row['kodeperiodebobot'].'|'.$row['indeksnilai'] ?>" title="Tampilkan Detail" src="images/edit.png" onclick="goDetail(this)" style="cursor:pointer">
						<?		}
								if($c_delete and (Modul::getRole() == 'A' or $t_valid != 'Y')) { ?>
							<img id="<?= $row['kodeperiodebobot'].'|'.$row['indeksnilai'] ?>" title="Hapus Data" src="images/delete.png" onclick="goDelete(this)" style="cursor:pointer">
						<?		} ?>
						</td>
						<?	} ?>
					</tr>
