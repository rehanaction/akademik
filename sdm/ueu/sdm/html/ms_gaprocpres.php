<?php
	...
	$p_key = 'kodeperiode,kategorinilai';
	
	// variabel request
	$r_periode = Modul::setRequest($_POST['kodeperiode'],'PERIODEPRES');
	if(empty($r_periode))
		$r_periode = mPa::getLastPeriode($conn);
		
	$a_periode = mPa::getCPeriode($conn);
		
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'periodegaji', 'label' => 'Acuan Periode Gaji', 'type' => 'S', 'option' => $p_model::getCPeriodeGaji($conn), 'notnull' => true);
	$a_kolom[] = array('kolom' => 'kategorinilai', 'label' => 'Kategori', 'type' => 'S', 'align' => 'center', 'option' => mPa::getCKategoriNilai($conn, $r_periode), 'notnull' => true);
	$a_kolom[] = array('kolom' => 'procnilai', 'label' => '% Gaji', 'maxlength' => 2, 'size' => 2, 'type' => 'N', 'align' => 'center', 'notnull' => true);
	
	// properti halaman
	$p_title = 'Daftar Prosentase Gaji Tunj. Prestasi';
	$p_tbwidth = 500;
	...
	if($r_act == 'insert' and $c_insert) {
		$a_kolom[] = array('kolom' => 'kodeperiode');
		$_POST['i_kodeperiode'] = $r_periode;
	...
	// combo
	$l_periode = UI::createSelect('kodeperiode',$a_periode,$r_periode,'ControlStyle',true,'onchange="goSubmit()"');
	
	// mendapatkan data ex
	$r_sort = Page::setSort($_POST['sort']);
	if(empty($r_sort)) $r_sort = 'kategorinilai';
	
	// mendapatkan data
	if(!empty($r_periode)) $a_filter[] = $p_model::getListFilter('periodepenilaian',$r_periode);
	
	$a_data = $p_model::getListData($conn,$a_kolom,$r_sort,$a_filter,'',$p_dbtable);
	
	// membuat filter
	$a_filtercombo = array();
	$a_filtercombo[] = array('label' => 'Periode Penilaian', 'combo' => $l_periode);
?>
