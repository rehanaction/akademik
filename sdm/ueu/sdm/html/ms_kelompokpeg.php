<?php
	...
	// variabel request
	$r_tipe = Modul::setRequest($_POST['idjenispegawai'],'JENISPEG');	
	// combo
	$l_tipe = uCombo::jenispegawaibaru($conn,$r_tipe,'idjenispegawai','onchange="goSubmit()"',false);
	
	// properti halaman
	$p_title = 'Daftar Kelompok Pegawai';
	$p_tbwidth = 500;
	$p_aktivitas = 'BIODATA';
	$p_dbtable = 'ms_kelompokpeg';
	$p_key = 'idkelompok';
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'idkelompok', 'label' => 'Kode', 'size' => 5, 'maxlength' => 5, 'align' => 'center', 'notnull' => true);
	$a_kolom[] = array('kolom' => 'namakelompok', 'label' => 'Nama Kelompok', 'size' => 40, 'maxlength' => 100, 'notnull' => true);
	...
	if($r_act == 'insert' and $c_insert) {
		...
		$a_kolom[] = array('kolom' => 'idjenispegawai', 'value' => $r_tipe);
		...
		if( empty($p_posterr))
			$p_posterr = mIntegrasi::saveDataIntegrasi($connsia, $p_dbtable, $keyintegrasi, $p_key, $dataset);
	}
		...
		if( empty($p_posterr))
			$p_posterr = mIntegrasi::deleteDataIntegrasi($connsia, $p_dbtable, $keyintegrasi, $p_key); 
	}
	...
	if(empty($r_sort)) $r_sort = 'namakelompok';
	
	// mendapatkan data
	if(!empty($r_tipe)){
		$a_filter[] = $p_model::getListFilter('idjenispegawai',$r_tipe);
	}
	...
	// membuat filter
	$a_filtercombo = array();
	$a_filtercombo[] = array('label' => 'Jenis Pegawai', 'combo' => $l_tipe);
