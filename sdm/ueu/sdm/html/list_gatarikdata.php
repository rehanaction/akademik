<?php
	...
	if($r_act == 'tarik' and $c_insert) {			
		...
		list($p_posterr,$p_postmsg) = $p_model::tarikData($conn,$r_periode,$r_unit);
		...
		list($p_posterr,$p_postmsg) = $p_model::deleteTarikData($conn,$r_key);
	...
	$sql = $p_model::listQueryHistoryGaji();
			...
			<form name="pageform" id="pageform" method="post">
						...
						<div class="inner">
							...
							<div class="right">
								<div class="TDButton" style="padding:7px 0px 7px;width:95px;position:relative;left:-5px;top:5px;" onClick="goTarik()">
									...
									<span style="position:relative;left:15px"> Tarik Data</span>
								</div>
							</div>
							<? } ?>
						</div>
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<?	} ?>
						<td align="center">
							<img id="<?= $t_key.'::list_gatarikdata' ?>" title="Tampilkan Detail" src="images/edit.png" onclick="goDetail(this)" style="cursor:pointer">
						<?	if($c_delete and !in_array($row['idpeg'],$a_byrpeg)) { ?>
							<img id="<?= $t_key ?>" title="Hapus Data Penguncian Pegawai" src="images/delete.png" onclick="goDelete(this)" style="cursor:pointer">
						<?	} ?>
						</td>
					</tr>
					<?	}
...
function goTarik() {
	var tarik = confirm("Apakah anda yakin menarik dan mengunci data pegawai untuk perhitungan gaji?");
	if(tarik) {
