<?php
	...
	// properti halaman
	$p_title = 'Daftar Permohonan Cuti';
	$p_tbwidth = 800;
	$p_aktivitas = 'HISTORY';
	...
	$a_kolom[] = array('kolom' => 'jeniscuti', 'label' => 'Jenis Cuti', 'filter' => 'c.jeniscuti');
	$a_kolom[] = array('kolom' => 'alasancuti', 'label' => 'Keperluan');
	$a_kolom[] = array('kolom' => 'lama', 'label' => 'Lama', 'align' => 'center', 'filter' => "cast(r.lamacuti as varchar)+' hari'");
	$a_kolom[] = array('kolom' => 'status', 'label' => 'Status', 'align' => 'center', 'filter' => "case when r.statususulan = 'A' then 'Diajukan' when r.statususulan = 'S' then 'Disetujui' when r.statususulan = 'T' then 'Ditolak' else 'Belum Diajukan' end");
	...
	if($r_act == 'delete' and $c_delete) {
		...
		if(!$p_posterr)
			list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,'pe_rwtcutidet','nourutcuti');
	...
	$sql = $p_model::listQueryPermohonanCuti($r_key);
	...
	//cek pegawai apakah tetap
	$istetap = $p_model::isPegTetap($conn,$r_key);
	if(empty($istetap)){
		$c_insert = false;
		list($p_posterr,$p_postmsg) = array(true,"Mohon ma'af, anda belum bisa mengajukan cuti");		
	}
?>
<html>
<head>
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<?
								if($c_delete and (Modul::getRole() == 'A' or Modul::getRole() == 'admhrm' or (empty($row['statususulan']) and $row['isvalid'] != 'Y'))) { ?>
