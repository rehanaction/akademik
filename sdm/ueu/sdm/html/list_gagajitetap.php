<?php
	...
	$r_jenispeg = Modul::setRequest($_POST['tipepegawai'],'JENISPEGAWAI');
	...
	$a_jenispeg = $p_model::getCTipePegawai($conn);
	$l_jenispeg = UI::createSelect('tipepegawai',$a_jenispeg,$r_jenispeg,'ControlStyle',true,'onchange="goSubmit()"');
	...
	if(!empty($r_jenispeg)) $a_filter[] = $p_model::getListFilter('tipepegawai',$r_jenispeg);
	...
	if($r_act == 'hitung' and $c_insert) {		
		...
		list($p_posterr,$p_postmsg) = $p_model::hitGajiTetap($conn,$r_periode,$a_sql,true);
		if(!$p_posterr){
			list($p_posterr,$p_postmsg) = $p_model::hitGajiTdkTetap($conn,$r_periode,$a_sql,true);
	...
	$a_filtercombo[] = array('label' => 'Tipe Pegawai', 'combo' => $l_jenispeg);
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<?	} ?>
						<td align="center">
							<img id="<?= $t_key.'::list_gagajitetap' ?>" title="Tampilkan Slip Gaji" src="images/edit.png" onclick="goDetail(this)" style="cursor:pointer">
						</td>
					</tr>
					<?	}
						if($i == 0) {
