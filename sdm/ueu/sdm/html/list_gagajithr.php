<?php
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'nik', 'label' => 'NIK', 'align' => 'center');
	...
	$a_kolom[] = array('kolom' => 'golongan', 'label' => 'Golongan','align' => 'center');
	$a_kolom[] = array('kolom' => 'masakerja', 'label' => 'Masa Kerja','filter' => "cast(p.masakerjathngol as varchar)+' tahun ' + cast(p.masakerjablngol as varchar)+' bulan'");
	$a_kolom[] = array('kolom' => 'gajitetap', 'label' => 'Gaji THR','type' => 'N','align' => 'right');
	...
	if($r_act == 'hitung' and $c_insert) {		
		...
		list($p_posterr,$p_postmsg) = $p_model::hitGajiTetap($conn,$r_periode,$a_sql);
		if(!$p_posterr){
			list($p_posterr,$p_postmsg) = $p_model::hitGajiTdkTetap($conn,$r_periode,$a_sql);
