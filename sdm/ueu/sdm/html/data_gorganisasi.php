<?php
	...
	// properti halaman
	$p_title = 'Data Pengalaman Organisasi';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	$p_listpage = Route::getListPage();
	
	$p_model = mPengembangan;
	$p_dbtable = "pe_organisasi";
	$where = 'nourutpo';
	...
	$a_input[] = array('kolom' => 'namaorganisasi', 'label' => 'Nama Organisasi', 'maxlength' => 100, 'size' => 50, 'notnull' => true);
	$a_input[] = array('kolom' => 'lokasiorganisasi', 'label' => 'Alamat Organisasi', 'maxlength' => 100, 'size' => 70);
	$a_input[] = array('kolom' => 'lingkuporganisasi', 'label' => 'Lingkup', 'type' => 'S', 'empty' => true, 'option' => $p_model::lingkup());
	$a_input[] = array('kolom' => 'jabatan', 'label' => 'Jabatan', 'maxlength' => 50, 'size' => 50);
	$a_input[] = array('kolom' => 'website', 'label' => 'Website', 'maxlength' => 50, 'size' => 50);
	$a_input[] = array('kolom' => 'refleksi', 'label' => 'Refleksi', 'type' => 'A', 'rows' => 2, 'cols' => 50, 'maxlength' => 255);
	...
	$a_input[] = array('kolom' => 'fileorganisasi', 'label' => 'File Organisasi', 'type' => 'U', 'uptype' => 'fileorganisasi', 'size' => 40);
	...
	if($r_act == 'save' and $c_edit) {
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,$p_dbtable,$where,'','fileorganisasi');
		
		if(!$p_posterr) Route::navListpage($p_listpage,$r_key);
	}
	else if($r_act == 'deletefile' and $c_delete) {
		list($p_posterr,$p_postmsg) = $p_model::deleteFile($conn,$r_subkey,$p_dbtable,'fileorganisasi',$where);				
		?>
	...
	$sql = $p_model::getDataEditOrganisasi($r_subkey);
			...
			<form name="pageform" id="pageform" method="post" action="<?= Route::navAddress(Route::thisPage()) ?>" enctype="multipart/form-data">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'namaorganisasi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'namaorganisasi') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'lokasiorganisasi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'lokasiorganisasi') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'lingkuporganisasi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'lingkuporganisasi') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'website') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'website') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'refleksi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'refleksi') ?></td>
						</tr>
						<tr>
						...
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'fileorganisasi') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'fileorganisasi') ?></td>
						</tr>
					</table>
					</div>
				</center>
