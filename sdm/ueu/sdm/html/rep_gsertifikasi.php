<?php
	...
	$r_jenissertifikasi = CStr::removeSpecial($_REQUEST['jenissertifikasi']);
	...
	$p_tbwidth = 1000;
	$p_col = 10;
	$p_file = 'riwayatsertifikasi_'.$r_kodeunit;
	$p_model = 'mPengembangan';
	$p_window = 'Daftar Sertifikasi';
    ...
    $a_data = $p_model::repRiwayatSertifikasi($conn,$r_kodeunit,$r_tglmulai,$r_tglselesai,$r_jenissertifikasi);
	$lingkup = $p_model::lingkup();
	
	$rs = $a_data['list'];
	
	$p_title = 'Daftar Riwayat Sertifikasi <br />
...
<div align="center">
		...
		<table width="<?= $p_tbwidth ?>" border="1" cellpadding="4" cellspacing="0">
			<tr bgcolor = "gray">
				...
				<th><b style = "color:#FFFFFF">Tgl. Sertifikasi</b></th>		
				<th><b style = "color:#FFFFFF">Jenis Sertifikasi</b></th>
				<th><b style = "color:#FFFFFF">No. Sertifikasi</b></th>
			...
			<tr>
				...
				<td><?= $row['namaunit']; ?></td>
				<td><?= $row['tipepeg']; ?></td>
				<td align="center"><?= CStr::formatDateInd($row['tglsertifikasi']); ?></td>
				<td><?= $row['jenissertifikasi']; ?></td>
				<td><?= $row['nosertifikasi']; ?></td>
				<td><?= $lingkup[$row['statussertifikasi']]; ?></td>
