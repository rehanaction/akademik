<?php
	...
	$r_tunjangan = Modul::setRequest($_POST['tunjangan'],'TUNJANGANAWAL');
	
	if(empty($r_tahun))
		$r_tahun = $p_model::getLastTahunTunjTarifParam($conn);
	
	// combo
	$l_unit = uCombo::unit($conn,$r_unit,'unit','onchange="goSubmit()" style="width:300px"',false);
	$a_tahun = $p_model::getTahunTunjTarifParam($conn);
	$l_tahun = UI::createSelect('tahun',$a_tahun,$r_tahun,'ControlStyle',true,'onchange="goSubmit()"');
	
	$a_tunjangan = $p_model::getCTunjParameter($conn,true);
	$l_tunjangan = UI::createSelect('tunjangan',$a_tunjangan,$r_tunjangan,'ControlStyle',true,'onchange="goSubmit()"');
		
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'namatunjangan', 'label' => 'Jenis Tunjangan');
	$a_kolom[] = array('kolom' => 'tmtmulai', 'label' => 'Tgl. Berlaku', 'type' => 'D');
	...
	// properti halaman
	$p_title = 'Penentuan Pegawai yang Mendapatkan Tunjangan Tarif Parameter';
	$p_tbwidth = 900;
	...
	$p_colnum = $r_tunjangan == 'T00022' ? $p_colnum+1 : $p_colnum;
	...
	if(!empty($r_tunjangan)) $a_filter[] = $p_model::getListFilter('tunjangan',$r_tunjangan);
		
	$sql = $p_model::listQueryTunjTarifParam($r_key);
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td><?= $rowc[$j++] ?></td>
						<?if($r_tunjangan == 'T00022'){?>
						<td align="right"><?= CStr::formatNumber($row['nominal'])?></td>
						<? }?>
