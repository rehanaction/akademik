<?php
	...
	// properti halaman
	$p_title = 'Data Riwayat Jabatan Akademik';
	$p_tbwidth = 800;
	$p_aktivitas = 'DATA';
	...
	if($r_act == 'save' and $c_edit) {
		...
		$record['jenisjabatan'] = 'L'; //untuk jabatan akademik
		$record['isefektif'] = $record['tmtmulai'] <= date('Y-m-d') ? 'Y' : 'null';
		$conn->BeginTrans();
	...
	$sql = $p_model::getDataEditFungsional($r_subkey,'L'); //untuk jabatan akademik
			...
			<form name="pageform" id="pageform" method="post" action="<?= Route::navAddress(Route::thisPage()) ?>" enctype="multipart/form-data">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap" width="200px"><?= Page::getDataLabel($row,'idjfungsional') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'idjfungsional') ?></td>
						</tr>
						<tr>
							<td class="LeftColumnBG" style="white-space:nowrap"><?= Page::getDataLabel($row,'tmtmulai') ?></td>
							<td  class="RightColumnBG" colspan="3"><?= Page::getDataInput($row,'tmtmulai') ?></td>
						</tr>
						<tr>
