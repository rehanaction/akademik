<?php
	...
	$r_potongan = CStr::removeSpecial($_POST['potongan']);
	
	if(empty($r_tahun))
		$r_tahun = $p_model::getLastTahunPotonganParam($conn);
	
	// combo
	$l_unit = uCombo::unit($conn,$r_unit,'unit','onchange="goSubmit()" style="width:300px"',false);
	$a_tahun = $p_model::getTahunPotonganParam($conn,$r_potongan);
	$l_tahun = UI::createSelect('tahun',$a_tahun,$r_tahun,'ControlStyle',true,'onchange="goSubmit()"');
	
	$a_potongan = $p_model::getCPotParam($conn,true);
	...
	$a_kolom[] = array('kolom' => 'tglmulai', 'label' => 'Tgl. Berlaku', 'type' => 'D','width'=>'100px');
	...
	$a_kolom[] = array('kolom' => 'nominal', 'label' => 'Tarif Potongan');
	
	// properti halaman
	$p_title = 'Daftar Potongan Parameter';
	$p_tbwidth = 850;
	...
	if(!empty($r_potongan)) $a_filter[] = $p_model::getListFilter('potonganparam',$r_potongan);
	if(!empty($r_tahun)) $a_filter[] = $p_model::getListFilter('tahunpot',$r_tahun);
		
	$sql = $p_model::listQueryPotParam($r_key);
