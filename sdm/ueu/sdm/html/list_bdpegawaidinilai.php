<?php
	...
	// properti halaman
	$p_title = 'Daftar Dosen Yang Dinilai';
	$p_tbwidth = 600;
	$p_aktivitas = 'NILAI';
	...
	// ada aksi
	$r_key = Modul::setRequest($_POST['key'],'KEYMONEV');	
		
	$r_act = $_POST['act'];
	if($r_act == 'savemonev' and $c_edit) {
		$r_pegawai = CStr::removeSpecial($_REQUEST['idpegawai']);
		list($p_posterr,$p_postmsg) = $p_model::saveMonevDet($conn,$r_key,$r_pegawai);
	}
	else if($r_act == 'delmonev' and $c_delete) {
		$r_subkey = CStr::removeSpecial($_REQUEST['subkey']);
		
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_subkey,'bd_monevdet',$p_key);
	}
	...
	$sql = $p_model::listBDPegawaiDinilai($r_key);
			...
			<form name="pageform" id="pageform" method="post">
				<center>
					<header style="width:<?= $p_tbwidth ?>px">
						<div class="inner">
							<div class="left title">
								<img id="img_workflow" width="24px" src="images/aktivitas/<?= $p_aktivitas ?>.png" onerror="loadDefaultActImg(this)"> <h1>Data Monev</h1>
							</div>
						</div>
					</header>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td class="LeftColumnBG" width="150px">Nama Periode</td>
