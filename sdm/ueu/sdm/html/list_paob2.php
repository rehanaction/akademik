<?php
	...
	$a_kolom[] = array('kolom' => 'kategori', 'label' => 'Kategori Penilaian');
	$a_kolom[] = array('kolom' => 'nilaiob2', 'label' => 'Nilai Kehadiran', 'align' => 'right', 'type' => 'N,2');
	
	// properti halaman
	$p_title = 'Daftar Penilaian Objektif (OB-2)';
	$p_tbwidth = 900;
	$p_aktivitas = 'NILAI';
	...
	if($r_act == 'hitung' and $c_edit) {
		$p_postmsg = $p_model::hitungKehadiran($conn, $r_periode);
	...
	$sql = $p_model::listQueryTimKehadiran();
	...
	$a_filtercombo[] = array('label' => 'Periode', 'combo' => UI::createSelect('periode',mPa::getCPeriode($conn), $r_periode, 'ControlStyle',$c_edit,'onChange="goSubmit()"').'&nbsp;&nbsp;'.(($c_edit) ? '<input type="button" value="Hiung OB-2" class="ControlStyle" onClick="goHitung()">' : ''));
?>
<html>
<head>
	<title><?= $p_title ?></title>
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td align="center">
							<img id="<?= $row['kodeperiode'].'|'.$row['idpegawai'] ?>" title="Tampilkan Detail" src="images/edit.png" onclick="goDetail(this)" style="cursor:pointer">
						</td>
					</tr>
					<?	}}
						if($i == 0) {
...
function goHitung(){
	var set = confirm("Anda yakin untuk mengitung nilai OB-2 "+ $("#periode option:selected").text() + " ?");
	if (set){
