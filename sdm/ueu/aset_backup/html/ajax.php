<?php
	...
	if($f == 'acbaranginv') {
	    ...
	    $sql .= "and lower(s.idbarang+'.'+right('000000'+convert(varchar(6), s.noseri), 6)+' - '+b.namabarang) like '%$q%' 
		...
		$sql = "select top 20 b.idbarang,b.namabarang,b.idsatuan,b.idbarang+' '+b.namabarang as barang 
		    from aset.ms_barang b join aset.as_stockhp s on s.idbarang = b.idbarang 
		    where s.jmlstock > 0 and b.idsatuan is not null 
		    and lower(b.idbarang+' '+b.namabarang) like '%$q%' 
		    order by b.idbarang";
        $res = $conn->GetArray($sql);
		...
		$sql  = "select top 20 s.idbarang,s.idbarang+' - '+b.namabarang as barang 
		    from aset.as_seri s left join aset.ms_barang b on b.idbarang = s.idbarang 
	    ...
	    $r_idbarang = $_REQUEST['idbarang'];
	    echo $conn->GetOne("select idsatuan from aset.ms_barang where idbarang = '$r_idbarang'");
	}
	else if($f == 'acunit') {
		$q = strtolower($q);
