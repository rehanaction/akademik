<?php
	...
	// properti halaman
	$p_title = 'Data Detail Penghapusan';
	$p_tbwidth = 500;
	$p_aktivitas = 'Detail Penghapusan';
	
	$p_model = mOpnameHPDetail;
	
	//init
	$isro = true;
	if(!empty($r_key)){
	    $a_mdata = mOpnameHP::getMData($conn,$r_key);
...
<table border="0" width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="2" class="GridStyle" align="center">
	...
	<tr>
		<td class="LeftColumnBG"><?= Page::getDataLabel($row,'qtyawal') ?></td>
		<td class="RightColumnBG"><?= Page::getDataInput($row,'qtyawal') ?></td>
	</tr>
	<tr>
		<td class="LeftColumnBG"><?= Page::getDataLabel($row,'qtyakhir') ?></td>
		<td class="RightColumnBG"><?= Page::getDataInput($row,'qtyakhir') ?></td>
	</tr>
</table>
    ...
    $('#labelbarang').autocomplete(
	    ajaxpage, 
	    {
		    ...
		    formatItem: function(data,i,max){
			    ...
			    str += data.idbarang + '.' + data.noseri + ' - ' + data.namabarang +'<br>';
			    str += data.merk +'<br>';
