		...
		if(empty($_FILES['hpexcel']['error'])) {
            ...
            for ($i = 2; $i <= $numRows; $i++) {
                if(!empty($data->sheets[0]['cells'][$i][1])){
                    ...
                    unset($record);
                    $record['idbarang'] = CStr::removeSpecial($data->sheets[0]['cells'][$i][1]);
                    ...
                    //$sql .= "insert into aset.as_transhpdetail (idtranshp,idbarang,qty,idsatuan,harga,total) 
                    //    values ($r_key,'$idbarang','$qty','$satuan','$harga','$total');";
                        
                    if(empty($record['idbarang']) or empty($record['qty']) or empty($record['idsatuan']) or empty($record['harga'])){
                        $sql = '';
			            $p_posterr = true;
