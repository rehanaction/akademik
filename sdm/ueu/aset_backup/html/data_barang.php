<?php
    ...
    $a_parent = array();
	if(empty($r_pkey)){
		$a_parent = array('idbarang' => null, 'level' => 0);
	}else{
		$a_parent = $p_model::getData($conn,$r_pkey);
	}
	
	if(!empty($r_pkey))
		$pcat = $p_model::getLast($conn,$r_pkey);
	...
	//struktur view
	$a_input = array();
	$a_input[] = array('kolom' => 'idbarang', 'label' => 'ID. Barang', 'maxlength' => 16, 'size' => 16, 'notnull' => true, 'default' => $pcat+1);
	...
	$a_input[] = array('kolom' => 'idparent', 'label' => 'ID. Parent', 'readonly' => $isreadonly, 'default' => $a_parent['idbarang']);
	...
	if($r_act == 'save' and $c_edit) {
		...
		if(empty($r_key)){
		    ...
		    $record['idparent'] = $a_parent['idbarang'];
