<?php
	...
	if(!empty($r_key)){
	    ...
	    if($r_isverify == '1' and $c_change){
			$is_mutasi = $p_model::getCheckMutasi($conn, $r_keydet);
				if($is_mutasi){
					$c_update = false;
					$c_delete = false;
					$c_edit = false;
...
<div class="Break"></div>
<table align="left" width="<?= $p_tbwidth ?>" >
	<? if ($is_mutasi) { ?>
		<tr>
			<td align="center"><font color="red"><b>Aset tersebut pernah dilakukan transaksi Mutasi</b></span></td>
		</tr>
	<? } ?>
</table>
<table border="0" width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="2" class="GridStyle" align="center">
	...
	<tr>
		<td class="LeftColumnBG" width="125"><?= Page::getDataLabel($row,'lokasi') ?></td>
...
$(document).ready(function() {
	...
	// autocomplete
	$("#lokasi").xautox({strpost: "f=acxlokasi", targetid: "idlokasi"}).blur(function(){
	    if($('#idlokasi').val() == '') $(this).val('');
	});
	$("#pegawai").xautox({strpost: "f=acxpegawai", targetid: "idpegawai"}).blur(function(){
	    if($('#idpegawai').val() == '') $(this).val('');
	});
