<?php
	...
	$sql ="select substring(s.idbarang,1,1) as idbarang, count(s.idseri) as total,s.idkondisi
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,1) = substring(s.idbarang,1,1) ";
    $sql .= "group by substring(s.idbarang,1,1),s.idkondisi order by substring(s.idbarang,1,1) ";
    
    $rs = $conn->Execute($sql);
	while($row = $rs->FetchRow()){
        $a_barang[$row['idbarang']][$row['idkondisi']] = (int)$row['total'];
    }
	...
	$sql ="select substring(s.idbarang,1,3) as idbarang, count(s.idseri) as total,s.idkondisi
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,3) = substring(s.idbarang,1,3) ";
    $sql .= "group by substring(s.idbarang,1,3),s.idkondisi order by substring(s.idbarang,1,3) ";
	...
	$sql ="select substring(s.idbarang,1,5) as idbarang, count(s.idseri) as total,s.idkondisi
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,5) = substring(s.idbarang,1,5) ";
    $sql .= "group by substring(s.idbarang,1,5),s.idkondisi order by substring(s.idbarang,1,5) ";
	...
	$sql ="select substring(s.idbarang,1,7) as idbarang, count(s.idseri) as total,s.idkondisi
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,7) = substring(s.idbarang,1,7) ";
    $sql .= "group by substring(s.idbarang,1,7),s.idkondisi order by substring(s.idbarang,1,7) ";
	...
	$sql ="select s.idbarang as idbarang, count(s.idseri) as total,s.idkondisi
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and s.idbarang = s.idbarang ";
    $sql .= "group by s.idbarang,s.idkondisi order by s.idbarang ";
...
<table class="tb_data" width="<?= $p_tbwidth ?>">
	...
	<tr valign="top">
	    ...
	    <?  foreach($a_kondisi as $idkondisi => $kondisi){ 
			$kondisi_tot[$idkondisi] += $a_barang[$idbarang][$idkondisi];		
		?>
	    <td align="right"><?= CStr::formatNumber($a_barang[$idbarang][$idkondisi]) ?></td>
	    <?  } ?>
	</tr>
	<?
	}
