<?php
    ...
    //barang saja
	$sql = "select td.idbarang, b.namabarang, b.idsatuan
        from aset.as_transhp t 
		join aset.as_transhpdetail td on td.idtranshp = t.idtranshp
		join aset.ms_barang b on b.idbarang = td.idbarang
		where year(t.tgltransaksi) <= '$r_tahun' and month(t.tgltransaksi) <= '$r_bulan' ";

    $sql .= " group by td.idbarang, b.namabarang, b.idsatuan order by b.namabarang";

	$rs = $conn->Execute($sql);
	while($row = $rs->FetchRow()){
	    $a_barang[$row['idbarang']]['namabarang'] = $row['namabarang'];
	    $a_barang[$row['idbarang']]['idsatuan'] = $row['idsatuan'];
	    $a_barang[$row['idbarang']]['jmla'] = 0;
	    $a_barang[$row['idbarang']]['nilaia'] = 0;
	    $a_barang[$row['idbarang']]['jmlt'] = 0;
	    $a_barang[$row['idbarang']]['jmlk'] = 0;
	    $a_barang[$row['idbarang']]['nilait'] = 0;
	    $a_barang[$row['idbarang']]['nilaik'] = 0;
	}

	//awal
	$sql = "select td.idbarang, t.tok, sum(td.qty) as jumlah, sum(td.total) as total
        from aset.as_transhp t 
		join aset.as_transhpdetail td on td.idtranshp = t.idtranshp
		join aset.ms_barang b on b.idbarang = td.idbarang
		where year(t.tgltransaksi) <= '$r_tahun' and month(t.tgltransaksi) < '$r_bulan' ";

    $sql .= " group by td.idbarang, t.tok order by td.idbarang";

	$rs = $conn->Execute($sql);
	while($row = $rs->FetchRow()){
		if($row['tok'] == 'T'){
			$a_barang[$row['idbarang']]['jmla'] += (float)$row['jumlah'];
			$a_barang[$row['idbarang']]['nilaia'] += (float)$row['total'];
		}else{
			$a_barang[$row['idbarang']]['jmla'] -= (float)$row['jumlah'];
			$a_barang[$row['idbarang']]['nilaia'] -= (float)$row['total'];
		}
	...
	while($row = $rs->FetchRow()){
		if($row['tok'] == 'T'){
			$a_barang[$row['idbarang']]['jmlt'] += (float)$row['jumlah'];
			$a_barang[$row['idbarang']]['nilait'] += (float)$row['total'];
		}else{
			$a_barang[$row['idbarang']]['jmlk'] += (float)$row['jumlah'];
			$a_barang[$row['idbarang']]['nilaik'] += (float)$row['total'];
		}
	}
