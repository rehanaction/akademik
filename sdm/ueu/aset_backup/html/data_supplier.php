<?php
	...
	$a_input[] = array('kolom' => 'isblacklist', 'label' => 'Blacklist ?', 'type' => 'C', 'option' => $a_blacklist);
	...
	if($r_act == 'save' and $c_edit) {
		...
		if(empty($r_key)){
			...
			list($p_posterr,$p_postmsg) = $p_model::saveSyncKeu($conn,$connkeu,$r_key);
		}
		
		$key = $p_model::getKey($conn);
		
		list($p_posterr,$p_postmsg) = $p_model::saveSyncKeu($conn,$connkeu,$key);
		
		if(!$p_posterr) unset($post);
	}
	else if($r_act == 'delete' and $c_delete) {
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_key);
		list($p_posterr,$p_postmsg) = $p_model::deleteSyncKeu($conn,$connkeu,$r_key);
