<?php
	...
	$sql ="select substring(s.idbarang,1,1) as idbarang, b.namabarang
			from aset.as_seri s 
	...
	while($row = $rs->FetchRow()){
		...
		$a_barang[$row['idbarang']]['total'] = 0;
		$a_barang[$row['idbarang']]['nilai'] = 0;
	}	}
	else if($r_level == '2'){	/*	Pengecekan Barang Saja Level 2	*/
	$sql ="select substring(s.idbarang,1,3) as idbarang, b.namabarang
			from aset.as_seri s 
	...
	$sql ="select substring(s.idbarang,1,5) as idbarang, b.namabarang
			from aset.as_seri s 
	...
	$sql ="select substring(s.idbarang,1,7) as idbarang, b.namabarang
			from aset.as_seri s 
	...
	else if($r_level == '5' or empty($r_level)){	/*	Pengecekan Barang Saja Level 5	*/
	$sql ="select s.idbarang as idbarang, b.namabarang
			from aset.as_seri s 
	...
	$sql ="select substring(s.idbarang,1,1) as idbarang, count(s.idseri) as total,sum(s.nilaiaset) as nilai
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,1) = substring(s.idbarang,1,1) ";
    $sql .= "group by substring(s.idbarang,1,1) order by substring(s.idbarang,1,1)";

	$rs = $conn->Execute($sql);		
	while($row = $rs->FetchRow()){
		$a_barang[$row['idbarang']]['total'] += (float)$row['total'];
		$a_barang[$row['idbarang']]['nilai'] += (float)$row['nilai'];
	}	}
	else if($r_level == '2'){	/*	Pengecekan Jumlah Barang Level 2	*/
	$sql ="select substring(s.idbarang,1,3) as idbarang, count(s.idseri) as total,sum(s.nilaiaset) as nilai
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,3) = substring(s.idbarang,1,3) ";
    $sql .= "group by substring(s.idbarang,1,3) order by substring(s.idbarang,1,3)";
	...
	$sql ="select substring(s.idbarang,1,5) as idbarang, count(s.idseri) as total,sum(s.nilaiaset) as nilai
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,5) = substring(s.idbarang,1,5) ";
    $sql .= "group by substring(s.idbarang,1,5) order by substring(s.idbarang,1,5)";
	...
	$sql ="select substring(s.idbarang,1,7) as idbarang, count(s.idseri) as total,sum(s.nilaiaset) as nilai
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and substring(s.idbarang,1,7) = substring(s.idbarang,1,7) ";
    $sql .= "group by substring(s.idbarang,1,7) order by substring(s.idbarang,1,7)";
	...
	else if($r_level == '5' or empty($r_level)){	/*	Pengecekan Jumlah Barang Level 5	*/
	$sql ="select s.idbarang as idbarang, count(s.idseri) as total,sum(s.nilaiaset) as nilai
			from aset.as_seri s 
			left join aset.ms_lokasi l on l.idlokasi = s.idlokasi 
			left join aset.ms_gedung g on g.idgedung = l.idgedung ";
    if($r_showchild) 
    ...
    if(!empty($r_level)) 
        $sql .= "and s.idbarang = s.idbarang ";
    $sql .= "group by s.idbarang order by s.idbarang ";
