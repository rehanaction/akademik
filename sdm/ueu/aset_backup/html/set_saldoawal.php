...
/*
	...
	while($row = $rs->FetchRow()) {
	    ...
	    $sqlp = "insert into aset.as_perolehan (idunit,idjenisperolehan,idbarang,idsaldoawal) values 
	        ($idunit,'100','$idbarang',$idsaldoawal);";
        ...
        if($ok){
            ...
            if($ok){
		        ...
		        $maxSeri = (int)$conn->GetOne("select max(noseri) from aset.as_seri where idbarang = '$idbarang'");
		        //for($i=1;$i<=$jml;$i++){
		            //$noseri = $maxSeri+$i;
		            $noseri = $maxSeri+1;
		            $sqls .= "insert into aset.as_seri (idbarang,noseri,iddetperolehan,idunit,idsaldoawal) values 
		            ('$idbarang',$noseri,'$idd','$idunit',$idsaldoawal);";
		        //}
...
idsupplier,catatan,insertuser,inserttime,isverify,verifyuser,verifytime,idbarang,idsatuan,qty,harga,total,iddasarharga,idkondisi,
...
while($row = $rs->FetchRow()) {
    ...
    for($j=0; $j<(int)$row['jml']; $j++){
        ...
        $sql .= "insert into aset.aaa_saldoawal values 
            ($i,{$row['idunit']},'{$row['idlokasi']}',{$row['idpegawai']},'{$row['idbarang']}',1,'{$row['merk']}',
