<?php
	...
	// properti halaman
	$p_title = 'Data Detail Perolehan';
	$p_tbwidth = 500;
	$p_aktivitas = 'Detail Perolehan';
	...
	//struktur view
	$a_input = $p_model::getInputAttr(array('coa' => $a_coa, 'isro' => $isro));
...
<table border="0" width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="2" class="GridStyle" align="center">
	...
	<tr>
		...
		<td class="RightColumnBG">
            <?//= Page::getDataInputWrap($r_namabarang,	UI::createTextBox('namabarang',$r_namabarang,'ControlAuto',40,40)."\n". '<input type="hidden" id="idbarang" name="idbarang" value="'.$r_idbarang.'">') ?>		    
			<?= Page::getDataInput($row,'labelbarang') ?>
		    <?= Page::getDataInput($row,'idbarang') ?>
<?/*
			<input type="hidden" id="idbarang" name="idbarang" value="<?= $r_idbarang ?>">
		    <input type="text" id="ac_idbarang" size="40" class="ControlStyle" value="<?= Page::getDataValue($row,'namabarang') ?>">&nbsp;&nbsp;
		    <span id="span_idbarang">&nbsp;</span>
...
$(document).ready(function() {
	...
	// autocomplete
	//$("#namabarang").xautox({strpost: "f=acbaranginv", targetid: "idbarang"});
    ...
    $('#labelbarang').autocomplete(
	    ajaxpage, 
	    {
		    ...
		    formatItem: function(data,i,max){
			    ...
			    str += data.idbarang + ' - ' + data.namabarang +'<br>';
			    str += '</div>';
			    return str;
		    },
		    extraParams: {
	    ...
	    .result(
		    function(event,data,formated){
                $('#labelbarang').val(data.idbarang+' - '+data.namabarang).focus();
                $('#idbarang').val(data.idbarang);
                //$('#span_barang').html('<div style="height:10px;"></div>'
                //	+'('+data.idbarang+') - '+data.namabarang
            	//);
		    }
	    ).focus();
...
function goSaveDet(){
	...
	//if($('#idbarang').val() == ''){
	//    alert('Silahkan memilih barang dahulu !');
	//}
