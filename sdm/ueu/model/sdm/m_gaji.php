	...
	class mGaji extends mModel {
		...
		function tarikDataBimbingan($conn,$r_periode){
			...
			$sql = "select p.idpegawai from akademik.ak_bimbingan b
					left join ".static::table('ms_pegawai')." p on p.nik = b.nip
		...
		function listQueryHonor($r_periode) {
			$sql = "select p.nik,".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai,
					...
					left join ".static::table('ms_unit')." u on u.idunit = g.idunit
					left join ".static::table('lv_jenjangpendidikan')." pd on pd.idpendidikan = g.pendidikan
					group by p.nik,p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang,u.namaunit,pd.namapendidikan,
		...
		function repLapRekapGaji($conn, $r_periode, $r_unit, $sqljenis){
			...
			$sql = "select g.*, h.idpeg, p.nik,".static::schema()."f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namalengkap,
		...
		function getRekapJurnalDosen($conn,$r_kodeunit,$r_periodegaji,$r_idpegawai){
			...
			$sql = "select g.idpegawai, v.nik, v.namalengkap, v.namaunit
					from ".static::table('ga_mengajarlog')." g
			...
			$sql .= " group by g.idpegawai, v.nik, v.namalengkap, v.namaunit";
