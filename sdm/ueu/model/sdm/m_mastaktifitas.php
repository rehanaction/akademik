	...
	class mMastAktifitas extends mModel {
		...
		function saveLiburDetail($conn,$post,$r_key,$is){
			...
			while($mulai<=$selesai){
				...
				if(!$err){
					//menghapus status kehadiran yang alpha, karena terlewatkan input hari libur
					$sql = "delete from ".self::table('pe_presensidet')." where kodeabsensi = 'A' and tglpresensi = '".$record['tgllibur']."'";
					$conn->Execute($sql);
