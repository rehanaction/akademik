<?php
	...
	class mPengembangan extends mModel {
		...
		function jenisStudi(){
			$data = array('D' => 'Dalam Negeri', 'L' => 'Luar Negeri');
		...
		function statusUsulan(){
			$data = array('Y' => 'Diterima', 'T' => 'Tidak Diterima');
		...
		function cekList($conn,$r_subkey) {
			if(empty($r_subkey))
				$sql = "select noceklist, ceklist from ".static::schema()."ms_ceklist where isaktif = 'Y' order by ceklist";
			else{
				$sql = "select p.*,c.noceklist as nocek,c.ceklist from ".static::schema()."ms_ceklist c
		...
		function listQueryPenelitian($r_key) {
			$sql = "select r.*, o.outputpenelitian,coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang)||' (Mandiri)' as mandiri
		...
		function getDataEditPenelitian($r_subkey) {
			$sql = "select r.*, o.outputpenelitian,coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as insertuser
					from ".self::table('pe_penelitian')." r 
					left join ".self::table('lv_outputpenelitian')." o on o.kodeoutput = r.kodeoutput
					left join ".self::table('ms_pegawai')." p on p.idpegawai = r.t_insertuser
		...
		function listTimPenelitian($conn,$r_key){
			$sql = "select case when t.statustim = 'P' then coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang)
					when t.statustim = 'L' then t.namatim end as namatim,cast(t.kontributorke as int) as kontributortim,
					r.idpenelitian,coalesce(m.nip||' - ','')||".static::schema.".f_namalengkap(m.gelardepan,m.namadepan,m.namatengah,m.namabelakang,m.gelarbelakang) as namapeg,cast(r.kontributorke as int) as kontributorke
		...
		function listQueryPKM($r_key) {
			$sql = "select r.*, k.namapkm,coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang)||' (Mandiri)' as mandiri
		...
		function getDataEditPKM($r_subkey) {
			$sql = "select r.*, k.namapkm,coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as insertuser
					from ".self::table('pe_pkm')." r 
					left join ".self::table('lv_jenispkm')." k on k.kodepkm = r.kodepkm
					left join ".self::table('ms_pegawai')." p on p.idpegawai = r.t_insertuser
		...
		function listTimPKM($conn,$r_key){
			$sql = "select case when t.statustim = 'P' then coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang)
					when t.statustim = 'L' then t.namatim end as namatim,cast(t.kontributorke as int) as kontributortim,
					r.idpkm,coalesce(m.nip||' - ','')||".static::schema.".f_namalengkap(m.gelardepan,m.namadepan,m.namatengah,m.namabelakang,m.gelarbelakang) as namapeg,cast(r.kontributorke as int) as kontributorke
			...
			if(count($post['P'])>0){
				...
				foreach($post['P'] as $data){
					...
					if(!in_array($ardata[3],$arnik)){
						...
						$r_keyelse = self::getLastValue($conn);
					}else{
		...
		function getTim($conn,$p_dbtable,$a_id,$r_subkey){
			...
			$sql = "select t.*,case when t.idpegawai is not null then coalesce(p.nip||' - ','')||".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) else '' end as namalengkap 
		...
		//jadikan sertifikat
		function setSertifikat($conn,$record,$r_subkey,$bidang='penelitian'){			
			$rec = array();
			if($bidang == 'penelitian'){
				if($record['issertifikat'] == 'Y'){
					$rec['idpegawai'] = $record['idpegawai'];
					$rec['kodesertifikasi'] = $record['lingkuppenelitian'] == 'L' ? 'S2' : ($record['lingkuppenelitian'] == 'N' ? 'S3' : ($record['lingkuppenelitian'] == 'I' ? 'S4' : 'null'));
					$rec['periodesertifikasi'] = substr($record['tglmulai'],0,4).substr($record['tglmulai'],5,2);
					$rec['statussertifikasi'] = $record['lingkuppenelitian'];
					$rec['tglsertifikasi'] = $record['tglmulai'];
					$rec['nosertifikasi'] = $record['nosurat'];
					$rec['keterangan'] = $record['judulpenelitian'];
					$rec['isvalid'] = 'Y';
					$rec['idpenelitian'] = $r_subkey;
					
					$isExist = $conn->GetOne("select 1 from ".self::table('pe_sertifikasi')." where idpenelitian = $r_subkey");
					if(!$isExist)
						list($p_posterr,$p_postmsg) = self::insertRecord($conn,$rec,true,'pe_sertifikasi');
					else
						list($p_posterr,$p_postmsg) = self::updateRecord($conn,$rec,$r_subkey,true,'pe_sertifikasi','idpenelitian');
				}else
					list($p_posterr,$p_postmsg) = self::delete($conn,$r_subkey,'pe_sertifikasi','idpenelitian');
			}
			else if($bidang == 'pkm'){
				if($record['issertifikat'] == 'Y'){
					$rec['idpegawai'] = $record['idpegawai'];
					$rec['kodesertifikasi'] = $record['lingkup'] == 'L' ? 'S2' : ($record['lingkup'] == 'N' ? 'S3' : ($record['lingkup'] == 'I' ? 'S4' : 'null'));
					$rec['periodesertifikasi'] = substr($record['tglawal'],0,4).substr($record['tglawal'],5,2);
					$rec['statussertifikasi'] = $record['lingkup'];
					$rec['tglsertifikasi'] = $record['tglawal'];
					$rec['keterangan'] = $record['namakegiatan'];
					$rec['isvalid'] = 'Y';
					$rec['idpkm'] = $r_subkey;
					
					$isExist = $conn->GetOne("select 1 from ".self::table('pe_sertifikasi')." where idpkm = $r_subkey");
					if(!$isExist)
						list($p_posterr,$p_postmsg) = self::insertRecord($conn,$rec,true,'pe_sertifikasi');
					else
						list($p_posterr,$p_postmsg) = self::updateRecord($conn,$rec,$r_subkey,true,'pe_sertifikasi','idpkm');
				}else
					list($p_posterr,$p_postmsg) = self::delete($conn,$r_subkey,'pe_sertifikasi','idpkm');
			}
		...
		//hapus sertfikat
		function unsetSertifikat($conn,$r_subkey,$bidang='penelitian'){
			if($bidang == 'penelitian')
				list($p_posterr,$p_postmsg) = self::delete($conn,$r_subkey,'pe_sertifikasi','idpenelitian');
			else if($bidang == 'pkm')
				list($p_posterr,$p_postmsg) = self::delete($conn,$r_subkey,'pe_sertifikasi','idpkm');
		...
		function repRiwayatPenelitian($conn,$r_kodeunit,$r_tglmulai,$r_tglselesai,$r_output){
			...
			$sql = "select r.*,p.nik,".static::schema()."f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai,
					...
					left join ".static::schema()."ms_tipepeg t on t.idtipepeg=p.idtipepeg
					where r.isvalid = 'Y'";
			if(!empty($r_tglmulai) and !empty($r_tglselesai))
				$sql .= " and r.tglmulai between '$r_tglmulai' and '$r_tglselesai'";
		...
		function repRiwayatPKM($conn,$r_kodeunit,$r_tglmulai,$r_tglselesai,$r_jenis){
			...
			$sql = "select r.*,p.nik,".static::schema()."f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai,
					...
					where r.isvalid = 'Y'";
			if(!empty($r_tglmulai) and !empty($r_tglselesai))
				$sql .= " and r.tglawal between '$r_tglmulai' and '$r_tglselesai'";
