<?php
	...
	class mGaji extends mModel {
		...
		function listQueryHitTunjangan($r_periode,$r_tunjangan){
			...
			$sql = "select g.*,p.idpegawai,p.nik,".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai,
		...
		function getTunjPendapatanLain($conn){
			$sql = "select kodetunjangan,namatunjangan from ".static::table('ga_tunjangan')." where isbayargaji = 'Y' and (isgajitetap = 'N' or isgajitetap is null) order by kodetunjangan";
			...
			while($row = $rs->FetchRow()){
				...
				if(!$err){
					...
					//T. Transport
						...
						$nominal = $a_tarifTunj[$r_tunjangan][$row['idhubkerja']];
						$record['nominal'] = $nominal;
		...
		//list tunjangan prestasi
		function listQueryTunjanganPrestasi(){
			$sql = "select g.* from ".static::table('ga_patunjprestasi')." g";
		...
		function listQueryHistoryGaji() {
			$sql = "select g.*,p.nik,".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai,
					...
					substring(g.masakerjapend,1,2)||' tahun ' as mkgaji
					from ".static::table('ga_historydatagaji')." g
					left join ".static::table('ms_pegawai')." p on p.idpegawai = g.idpeg
					left join ".static::table('ms_tipepeg')." t on t.idtipepeg = g.idtipepeg
					left join ".static::table('ms_jenispeg')." j on j.idjenispegawai = g.idjenispegawai
		...
		function getDataHistoryGaji($conn,$r_key){
			...
			$sql = "select g.*,pg.namaperiode as namaperiodegaji,pt.namaperiode as namaperiodetarif,
					...
					h.hubkerja,s.namastatusaktif,case when g.statusnikah = 'S' then 'Single' when g.statusnikah = 'N' then 'Nikah' 
					when g.statusnikah = 'D' then 'Duda' when g.statusnikah = 'J' then 'Janda' end as statusnikah,
		...
		function tarikData($conn,$r_periode,$r_unit='',$r_idpegawai=''){
			...
			$sql = "select p.idpegawai,p.jeniskelamin,p.statusnikah,p.jmlanak,p.npwp,js.idjstruktural,js.idjnspejabat,p.idpendidikan,p.idpangkat,
					p.idtipepeg,p.idjenispegawai,p.idhubkerja,jk.jamkerja,p.ispasangankerja,p.idstatusaktif,
		...
		function getTunjTetapGaji($conn,$r_tipepeg){
			$sql = "select g.kodetunjangan,namatunjangan from ".static::table('ga_tunjangan')." g
					...
					where isgajitetap = 'Y' and idjenispegawai like '$r_tipepeg%' group by g.kodetunjangan order by kodetunjangan";
		...
		function tarikDataMengajar($conn,$r_periode){
			...
			$sql = "select a.*,p.idpegawai,sdm.f_diffmenit(a.waktumulai,a.waktuselesai) as jmljam,up.kodefakultas as fakultas
					...
					where a.tglkuliah between '".$pg['tglawalhitung']."' and '".$pg['tglakhirhitung']."'";
			...
			while($row = $rs->FetchRow()){
				...
				$isexist = $conn->GetOne("select 1 from ".static::table('ga_mengajarlog')." where tglkuliah = '".$record['tglkuliah']."',
							perkuliahanke = ".$record['perkuliahanke'].",periode = '".$record['periode']."',thnkurikulum = '".$record['thnkurikulum']."',
							kodeunit = '".$record['kodeunit']."',kodemk = '".$record['kodemk']."',kelasmk = '".$record['kelasmk']."'");
				if(empty($isexist))
		...
		function infoHonor($conn,$r_honor){
			...
			if($r_honor == 'H0001'){//H. Fakultas
				$info = 'Fakultas';
				$filter = 'u.namaunit';
			}else if($r_honor == 'H0002'){//H. Matakuliah
				$info = 'Mata Kuliah';
				$filter = 'mk.namamk';
		...
		function getCHonorTarif($conn){
			$sql = "select kodehonor,namahonor from ".static::table('ga_honor')." order by kodehonor";
		...
		function listQueryTarifHonor($r_honor) {
			if($r_honor == 'H0001'){//H. Fakultas
				...
				$select = ",mk.namamk as namavariabel";
				$leftjoin = "left join akademik.ak_matakuliah mk on mk.kodemk = g.variabel1";
			}
				...
				$select = ",u.namaunit as namavariabel,u.level,u.infoleft";
				$leftjoin = "left join ".static::table('ms_unit')." u on u.idunit = g.variabel1";
			}
			...
			while($row = $rs->FetchRow()){
				...
				if(!$err){
					if(count($a_histajar[$row['idpeg']])>0){
						foreach($a_histajar[$row['idpeg']] as $key => $val){echo $key.' '.$val.'<br>';
							foreach($val as $keys => $jmljam){
								//honor berdasarkan MKDU
								if($key == 'H0002'){
									$jmljam = (int)$jmljam/60;
									$record = array();
									$record['periodegaji'] = $r_periode;
		...
		function listQueryHitungTHR($r_periode,$r_refperiode,$r_hubkerja,$r_jenispeg){
			$sql = "select g.gajiditerima,g.pph,g.isfinish,g.idpegawai,g.periodegaji,gh.*,p.nik,".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai,
		...
		function saveTarifLemburDet($conn,$_POST,$r_key){
			list($r_kodetariflembur,$r_idpendidikan,$r_periodetarif) = explode('|',$r_key);
			
			$record = array();
			$record['kodetariflembur'] = $_POST['kodetariflembur'];
			$record['idpendidikan'] = $_POST['idpendidikan'];
			$record['periodetarif'] = $_POST['periodetarif'];
			$record['isharilibur'] = $_POST['isharilibur'];
			...
			if ($record['isharilibur'] == 'Y'){
				if(count($_POST['ceklist']) > 0){
					...
					foreach($_POST['ceklist'] as $val){
						if(!empty($_POST['ceklist_'.$val])){
		...
		function repLapGapok($conn,$r_periode){
			$sql = "select idpangkat,cast(masakerja as varchar) as mkerja,tarifgapok from ".static::table('ga_tarifgapok')." 
		...
		function repLapRekapGaji($conn, $r_periode, $r_unit, $sqljenis){
			...
			$sql = "select h.idpeg, p.nik,".static::schema()."f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namalengkap,
