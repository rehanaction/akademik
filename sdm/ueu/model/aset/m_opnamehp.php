<?php
	...
	class mOpnameHP extends mModel {
		...
		function prosesOpname($conn,$key){
	        ...
	        $sql = "select o.tglopname,d.iddetopnamehp,d.idbarang1,d.qtyawal,d.qtyakhir,d.idsatuan 
	        ...
	        while($row = $rs->FetchRow()){
	            ...
	            $a_stok[$tok][$row['iddetopnamehp']]['idbarang1'] = $row['idbarang1'];
        ...
        function setSaldoOpname($conn,$tok,$idopnamehp,$a_stok,$tglopname){
            ...
            if($ok){
                foreach($a_stok[$tok] as $val){
                    $sql = "insert into aset.as_transhpdetail (idtranshp,idbarang1,idsatuan,qty,konvidsatuan,konvqty,konvnilai) values 
                        ($r_idtranshp,'{$val['idbarang1']}','{$val['idsatuan']}',{$val['qty']},'{$val['idsatuan']}',{$val['qty']},1)";
                    $ok = $conn->Execute($sql);
                    
                    if($ok){ //hitung saldo avg
                        $ok = mTransHPDetail::setSaldoAvg($conn,$val['idbarang1'],$time,$r_idtranshp);
                    }
                }
            }
