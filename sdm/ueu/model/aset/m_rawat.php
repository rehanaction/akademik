<?php
	...
	class mRawat extends mModel {
		...
		function dataQuery($key){
		    $sql = "select r.*,
		    	case when inserttime < '2016-06-01' then r.idbarang+' - '+b.namabarang else r.idbarang1+' - '+bb.namabarang END AS barang, 
		        p.namasupplier 
		        from ".static::table()." r 
		        left join ".static::schema.".ms_barang b on b.idbarang = r.idbarang 
		        left join ".static::schema.".ms_barang1 bb on bb.idbarang1 = r.idbarang1
		...
		function getMData($conn, $key){
		    return $conn->GetRow("select idunit,idlokasi,isok1,isverify,idbarang1,tglrawat,tglkembali,tglpengajuan,insertuser from ".self::table()." where idrawat = '$key'");
		}
