...
*/
		function getRowByIDP($conn, $idparent){
		    $sql = "select iddettranshp,d.idbarang1,b.namabarang,d.idsatuan,qty,harga,total,idcoa,d.catatan,
		        ...
		        left join ".static::schema.".as_stockhp s on s.idbarang1 = b.idbarang1 
		...
		function dataQuery($key){
		    $sql = "select iddettranshp,d.idbarang1,b.namabarang,d.idsatuan,qty,harga,total,idcoa,d.catatan,
		        ...
		        d.idbarang1+' - '+b.namabarang as barang
		        from ".static::table()." d 
		        left join ".static::schema.".ms_barang1 b on b.idbarang1 = d.idbarang1 
        ...
        function getMDetData($conn, $key){
            return $conn->GetRow("select idbarang1 from ".static::table()." where iddettranshp = '$key'");
        }
		...
		function setDataKonv($conn,&$record){
		    ...
		    $r_defsatuan = mBarang::getSatuanByID($conn,$record['idbarang1']);
		    ...
		    if($r_defsatuan != $record['idsatuan']){
		        ...
		        $r_nilai = mKonversi::getNilaiKonv($conn,$record['idbarang1'],$record['idsatuan']);
		...
		function setSaldoAvg($conn,$idbarang1,$verifytime,$idtranshp=0){
            $ok = true;
...
/*
		    $sql = "select top 1 d.iddettranshp,d.saldoqty,d.saldototal,d.avgprice 
		            ...
		            and d.idbarang1 = '$idbarang1' 
		            order by t.tgltransaksi desc,d.idtranshp desc";
*/
		    $sql = "select top 1 d.iddettranshp,d.saldoqty,d.saldototal,d.avgprice 
		            from aset.as_transhpdetail d join aset.as_transhp t on d.idtranshp = t.idtranshp 
		            where t.isverify = '1' and d.qty > 0 and t.verifytime < '$verifytime' and d.idbarang1 = '$idbarang1' 
            ...
            if(!empty($sql)){
                ...
                if($ok){
                    ...
                    list($p_posterr,$p_postmsg) = mStockHP::setStock($conn, $idbarang1, $p_saldoqty, $p_saldototal, $idsatuan);
