<?php
	...
	class mPerolehan extends mModel {
		...
		function updateCRecord($conn,$a_detail,$record,$key){
		    $row = $conn->GetRow("select isverify,harga,idbarang1 from aset.as_perolehan where idperolehan = '$key'");
            if($row['isverify'] == '1')
		...
		function getRowByIDP($conn, $idparent){
		    $sql = "select idperolehan,
		    	case when tglperolehan < '2016-06-01' then p.idbarang else p.idbarang1 END AS idbarang1, 
		    	case when tglperolehan < '2016-06-01' then b.namabarang else bb.namabarang END AS namabarang, 
		    	--p.idbarang1,b.namabarang,
		    	qty,harga,merk,spesifikasi,idkondisi,idcoa,isverify 
		        from ".static::table()." p 
		        left join ".static::schema.".ms_barang b on b.idbarang = p.idbarang 
		        left join ".static::schema.".ms_barang1 bb on bb.idbarang1 = p.idbarang1 
		...
		function listQuery() {
			$sql = "select idperolehan,namaunit,tglperolehan,jenisperolehan,nobukti,isverify,
			        u.kodeunit+' - '+u.namaunit as unit, p.tglpo, p.nopo, 
			        case when tglperolehan < '2016-06-01' then p.idbarang+' - '+b.namabarang else p.idbarang1+' - '+bb.namabarang END AS barang, 
			        s.namasupplier, p.qty, p.total
					from ".self::table()." p 
		...
		function dataQuery($key) {
			$sql = "select idperolehan,p.idunit,idjenisperolehan,tglperolehan,tglpembukuan,nobukti,tglbukti,nopo,tglpo,nospk,tglspk,
		        ...
		        p.idbarang1,b.namabarang,b.idsatuan,
		        case when tglperolehan < '2016-06-01' then p.idbarang+' - '+b.namabarang else p.idbarang1+' - '+bb.namabarang END AS barang,
		...
		function getMData($conn, $key){
		    return $conn->GetRow("select idunit,isverify,qty,nobukti,tglbukti,idbarang1,harga,tglperolehan from ".self::table()." where idperolehan = '$key'");
		}
		...
		function genSeri($conn,$row){
		    $maxSeri = (int)$conn->GetOne("select max(noseri) from ".static::schema.".as_seri where idbarang1 = '{$row['idbarang1']}'");
		    ...
		    for($i=1;$i<=$jml;$i++){
		        $noseri = $maxSeri+$i;
		        $sql .= "insert into ".static::schema.".as_seri (idbarang1,noseri,iddetperolehan,idunit,idlokasi,idpegawai) 
		            values ('{$row['idbarang1']}',$noseri,'{$row['iddetperolehan']}','{$row['idunit']}','{$row['idlokasi']}','{$row['idpegawai']}');";
		    }
		    $ok = $conn->Execute($sql);
