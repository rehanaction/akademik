<?php
	...
	class mSeri extends mModel {
		...
		function listQuery() {
			// List Barang Lama
			/*$sql = "select s.idseri, s.idbarang1+' - '+b.namabarang as barang, u.namaunit as unit,s.merk,s.spesifikasi,
			...
			$sql = "select s.idseri, 
				case 
					when s.tglperolehan < '2016-06-01' 
						then s.idbarang+' - '+b.namabarang 
					else 
						s.idbarang1+' - '+bb.namabarang 
				END AS barang, bb.namabarang, b.namabarang,  
				u.namaunit as unit,s.merk,s.spesifikasi,
			    s.idlokasi as lokasi, right('000000' + cast(s.noseri as varchar(6)), 6) noseri, 
			    s.idkondisi, s.idstatus, s.tglperolehan, p.namalengkap as pegawai
				from ".self::table()." s
		...
		function dataQuery($key){
			$sql = "select s.*, 
			    ...
			    s.idlokasi+' - '+l.namalokasi as lokasi, s.kmpakai, b1.namabarang 
				from ".self::table()." s
				left join ".static::schema.".ms_barang1 b on b.idbarang1 = s.idbarang1 
				left join ".static::schema.".ms_barang b1 on b1.idbarang = s.idbarang
				left join ".static::schema.".ms_lokasi l on l.idlokasi = s.idlokasi 
				left join ".static::schema.".ms_unit u on u.idunit = s.idunit 
				left join sdm.v_biodatapegawai p on p.idpegawai = s.idpegawai 
		...
		function getSeriByP($conn, $idperolehan){
			$sql = "select s.idseri,s.noseri,s.idbarang1+' - '+b.namabarang as barang,u.namaunit,s.merk,s.spesifikasi,
		...
		function getMData($conn, $key){
		    return $conn->GetRow("select idbarang1,noseri from ".self::table()." where idseri = '$key'");
		}
...
*/
	    ...
	    function getIDBarang($conn, $idseri){
	        return $conn->GetOne("select idbarang1 from ".self::table()." where idseri = '$idseri'");
	    }
