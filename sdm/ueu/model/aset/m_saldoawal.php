	...
	class mSaldoAwal extends mModel {
		...
		function insertCRecord($conn,$kolom,$rec,&$key){
		    ...
		    if(!$err){
		        //perolehan header
		        $rech = array();
		        $rech['idunit'] = $rec['idunit'];
		        $rech['idjenisperolehan'] = '100';
		        $rech['tglperolehan'] = '2012-12-31';
		        $rech['tglpembukuan'] = '2012-12-31';
		        $rech['idsaldoawal'] = $r_key;
		        $rech['listbarang'] = $conn->GetOne("select namabarang from aset.ms_barang1 where idbarang1 = '{$rec['idbarang1']}'");
		        
		        $err = Query::recInsert($conn,$rech,'aset.as_perolehanheader');
                $idh = $conn->Insert_ID();
                ...
                if(!$err){
                    ...
                    //seri
                    if(!$err){
		                $maxSeri = (int)$conn->GetOne("select max(noseri) from aset.as_seri where idbarang1 = '{$rec['idbarang1']}'");
	                    $noseri = $maxSeri+1;

		                $recs = array();
		                $recs['idbarang1'] = $rec['idbarang1'];
		                $recs['noseri'] = $noseri;
		...
		function updateCRecord($conn,$kolom,$rec,&$key){
		    ...
		    if(!$err){
    			...
    			$err = Query::recUpdate($conn,$rech,'aset.as_perolehanheader',"idsaldoawal = '$key'");
    			...
    			if(!$err){
                    ...
                    if(!$err){
                        ...
                        $idbarango = $conn->GetOne("select idbarang1 from aset.as_seri where idsaldoawal = '$key'");
                        if($idbarango != $rec['idbarang1']){
		                    $maxSeri = (int)$conn->GetOne("select max(noseri) from aset.as_seri where idbarang1 = '{$rec['idbarang1']}'");
	                        $noseri = $maxSeri+1;
		...
		function delete($conn,&$key){
            ...
            if(!$err){
                ...
                if(!$err){
	                ...
	                //perolehan header
                    if(!$err){
            			$err = Query::qDelete($conn,'aset.as_perolehanheader',"idsaldoawal = '$key'");
