<?php
	...
	class mLaporan {
		...
		function getSumberdana($conn, $idsumberdana){
		    $sql = "select idsumberdana, sumberdana from aset.ms_sumberdana where idsumberdana = '$idsumberdana'";
			return $conn->GetRow($sql);
		}
		...
		function getJmlAsetKelompok($conn){
		    ...
		    $sql = "select substring(idbarang1,1,1) as brg, namabarang 
		        from aset.ms_barang1 
		        where level = 1 and substring(idbarang1,1,1) != '1' order by idbarang1";
		    $rs = $conn->Execute($sql);
		    while($row = $rs->FetchRow())
		        $a_kelompok[$row['brg']] = $row['namabarang'];
		    
		    $sql = "select substring(idbarang1,1,1) as brg, count(*) as jml 
		        from aset.as_seri s join aset.ms_unit u on u.idunit = s.idunit 
		        where substring(idbarang1,1,1) != '1' and idstatus in ('A','P','R') 
		        and u.infoleft >= ".(int)$a_unit['LEFT']." and inforight <= ".(int)$a_unit['RIGHT']." 
		        group by substring(idbarang1,1,1)";
		    $rs = $conn->Execute($sql);
		    while($row = $rs->FetchRow())
		...
		function getMaxPermintaan($conn){
		    $a_data = array();
		    $a_jenis = array();
		    $a_unit = Modul::getLeftRight();
		    
		    $sql = "select top 3 b.namabarang, sum(td.qty) total
		...
		/*function getPertumbuhanBarang($conn){
			$sql ="select p.idbarang1, b.namabarang, p.idsatuan, sum(p.qty) as jumlah
						from aset.ms_barang1 b
						join aset.as_perolehan p on p.idbarang1=b.idbarang1
						group by p.idbarang1, b.namabarang, p.idsatuan";
		...
		/*function getAsetAkuntansi($conn, $p=''){
			$sql ="select p.idbarang1, b.namabarang, p.idsatuan, pd.qty, p.harga, u.namaunit, convert(varchar(10), p.tglperolehan, 105) as tglperolehan, p.catatan,
		...
		/*function getBASTBNew($conn, $key, $iddetail){
			$sql = "select p.idbarang1, b.namabarang, p.merk, p.spesifikasi, k.kondisi,
		...
		/*function getBASTBRawatSupplier($conn, $key){
		    $sql = "select d.iddetrawat,right('000000'+convert(varchar(6), s.noseri), 6) as noseri,d.idseri, 
					s.idbarang1,b.namabarang,s.merk,s.spesifikasi,s.idbarang1+' - '+b.namabarang as barang,
