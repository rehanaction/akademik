<?php
	...
	class mRiwayat extends mModel {
		...
		function cekPunyaIstriSuami($conn, $r_key){
			$ispunya = $conn->GetOne("select 1 from ".self::table('pe_istrisuami')." where idpegawai = $r_key and isvalid = 'Y' and (statuspasangan is null or statuspasangan = 'H') and (iscerai is null or iscerai = 'T') order by tglkawin desc limit 1");
		...
		function isExistAcuan($conn,$r_key,$r_subkey){
			$sql = "select 1 from ".self::table('pe_rwtpangkat')." where idpegawai = $r_key and isvalid = 'Y' and flagacuan = 'Y'";	
		...
		function cronStruktural($conn,$tgl){
			$conn->Execute("update ".self::table('pe_rwtstruktural')." set isaktif = 'N' where tmtselesai = '$tgl'");
		...
		function repRiwayatFungsional($conn,$r_kodeunit,$r_tglmulai,$r_tglselesai,$r_jenisjab){
			...
			$sql = "select r.*,p.nik,".static::schema()."f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namapegawai, 
					u.namaunit, f.jabatanfungsional, t.tipepeg
