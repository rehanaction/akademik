	...
	class mHistDepresiasi extends mModel {
		...
		function listQuery(){
			$sql = "select idhistdepresiasi,d.periode, b.namabarang as barang, 
                ...
                jp.jenispenyusutan, d.nilaiawal, d.nilaisusut, d.nilaiaset, u.infoleft, u.inforight, u.kodeunit, p.tglbukti, p.nobukti, 
                d.idperolehan, p.idperolehanheader 
			    from ".self::table()." d 
		...
		function setPenyusutan($conn, $idperolehan){
		    ...
		    if($ok){
		        $sql = "select p.nobukti,p.tglbukti,p.idbarang,p.harga,b.idjenispenyusutan,j.tarifsusut,j.lifetime,j.nilaisisa,s.idseri 
		            ...
		            join aset.ms_barang b on b.idbarang = p.idbarang 
		        ...
		        while($row = $rs->FetchRow()){
                    ...
                    $sqli .= "insert into aset.as_histdepresiasi (periode,idseri,idjenispenyusutan,nilaiawal,nilaisusut,nilaiaset,idperolehan,isaktif) values 
                        ('$periode','{$row['idseri']}','{$row['idjenispenyusutan']}','{$row['harga']}','$nilaiaset','1','$idperolehan','$isaktif');";
