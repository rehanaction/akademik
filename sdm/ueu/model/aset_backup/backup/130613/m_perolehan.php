<?php
	...
	class mPerolehan extends mModel {
		...
		function genSeri($conn,$row){
		    $maxSeri = 1+(int)$conn->GetOne("select max(noseri) from ".static::schema.".as_seri where idbarang = '{$row['idbarang']}'");

		    //insert yang perlu saja supaya tidak berat
		    $jml = (int)$row['qty'];
		    $sql = '';
		    for($i=$maxSeri;$i<=$jml;$i++){
		        $sql .= "insert into ".static::schema.".as_seri (idbarang,noseri,iddetperolehan,idunit) 
		            values ('{$row['idbarang']}',$i,'{$row['iddetperolehan']}','{$row['idunit']}');";
		    }
		    $ok = $conn->Execute($sql);
