<?php
	...
	class mLaporan {
		...
		function getPerolehanBarang($conn, $p=''){
			$sql ="select pd.idbarang, b.namabarang, pd.merk, pd.spesifikasi, pd.qty, b.idsatuan
		...
		function getPertumbuhanBarang($conn){
			$sql ="select pd.idbarang, b.namabarang, b.idsatuan, sum(pd.qty) as jumlah
						from aset.ms_barang b
						join aset.as_perolehandetail pd on pd.idbarang=b.idbarang
						group by pd.idbarang, b.namabarang, b.idsatuan";
		...
		function getBukuInventaris($conn, $p=''){
			$sql ="select p.nobukti, convert(varchar(12), p.tglpembukuan, 106) as tglpembukuan, b.namabarang, pd.idbarang, pd.merk, p.idsumberdana, 
		...
		function getBukuInduk($conn, $p=''){
			$sql ="select p.idperolehan, p.idjenisperolehan, p.idunit, u.namaunit, p.idlokasi, l.namalokasi, pd.idbarang, b.namabarang, b.idsatuan, 
		...
		function getAsetAkuntansi($conn, $p=''){
			$sql ="select pd.idbarang, b.namabarang, pd.idsatuan, pd.qty, pd.harga, u.namaunit, convert(varchar(10), p.tglperolehan, 105) as tglperolehan, p.catatan,
		...
		function getPerolehanAset($conn, $p=''){
			$sql ="select pd.idbarang, b.namabarang, pd.merk, pd.spesifikasi, pd.qty, b.idsatuan
							...
							left join aset.ms_satuan s on s.idsatuan=pd.idsatuan
							where u.infoleft >= {$p['unit']['infoleft']} and u.inforight <= {$p['unit']['inforight']} and
							substring(pd.idbarang,0,1) != '1' and p.nobukti is not null
							order by pd.idbarang";
		...
		function getBASTBNew($conn, $key){
		    $sql = "select d.idbarang, b.namabarang, d.merk, d.spesifikasi, d.qty, d.harga 
		        from aset.as_perolehandetail d  
		        join aset.ms_barang b on b.idbarang = d.idbarang 
