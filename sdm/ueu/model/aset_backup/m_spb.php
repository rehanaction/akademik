	...
	class mSpb extends mModel {
		    ...
		    if(!$p_posterr){
		        if(isset($record['isverify']) and $record['isverify'] != $r_isverify){
		            if($record['isverify'] == '1'){
		                ...
		                $sql = "select d.idbarang,t.tgltransaksi from aset.as_transhp t 
		                    join aset.as_transhpdetail d on d.idtranshp = t.idtranshp 
		                    where t.idtranshp = '$r_key'";
	                    $rs = $conn->Execute($sql);
	                    while($row = $rs->FetchRow()){
	                         $ok = mTransHPDetail::setSaldoAvg($conn,$row['idbarang'],$row['tgltransaksi'],$r_key);
	                         if(!$ok){ 
	                            $p_posterr = true;
		...
		function setCreatePO($conn, $record, $r_key){
			...
			if(!empty($a_mdata['isok1'])){
				...
				if($ok){
				$sql = "insert into prcm.pr_podetail (idpo,iddetspb,idbarang,qtypo)
					select $r_po,iddetspb,idbarang,qtysetuju
					from prcm.pr_spb s
					join prcm.pr_spbdetail sd on sd.idspb = s.idspb
					where s.idspb = '$r_key'";
