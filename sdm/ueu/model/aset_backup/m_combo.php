<?php
	...
	class mCombo {
		...
		function barang($conn){
			$sql = "select idbarang, idbarang+' - '+namabarang namabarang from aset.ms_barang where level > 3 and isaktif = 1";
			return Query::arrQuery($conn,$sql);
		}

		function brgstock($conn){
			$sql = "select s.idbarang, b.namabarang as namabarang 
		...
		function satuanTujuan($conn,$key) {
		    if(!empty($key)){
			    $sql = "select idsatuan, satuan 
			        from aset.ms_satuan 
			        where idsatuan not in (select idasal from aset.ms_konversi where idbarang = '$key')
			        order by idsatuan";
			    return Query::arrQuery($conn,$sql);
		    }else return array();
		}
		...
		function level(){
		    return array('5' => 'Level 5', '4' => 'Level 4', '3' => 'Level 3', '2' => 'Level 2', '1' => 'Level 1');
		}

		function listMobil($conn){
			$sql = "select s.idbarang, s.idbarang+' - '+k.nopol+' - '+k.merk+' - '+k.tipe as mobil
				from aset.as_seri s
				left join aset.ms_barang b on b.idbarang = s.idbarang
				left join aset.as_kibkendaraan k on k.idseri = s.idseri
				where s.idbarang like '302010%' ";
			return Query::arrQuery($conn,$sql);
		}
	}
?>
