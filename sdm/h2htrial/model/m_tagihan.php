<?php
	class mTagihan {
		...
		function getJenisTagihanFromKode($conn,$kodetagihan) {
			...
			$sql = "select jenistagihan from h2h.lv_jenistagihan where kodetagihan = ?";
		...
		function getInfoTarifFormulir($conn,$pilihan) {
			$sql = "select t.idgelombang as gelombang, t.jalurpenerimaan, t.programpend||' '||t.sistemkuliah||' '||t.jumlahpilihan||' pilihan' as keterangan
					from h2h.ke_tariffrm t where t.kodeformulir = ?";
		...
		// tagihan
		function getListInquiry($conn,$nim,$jenistagihan,$periode=null,$full=false) {
			...
			$sql = "select t.idtagihan, t.periode, t.flaglunas, t.isvalid, j.namajenistagihan, k.namakelompok,
					...
					where coalesce(t.nim,t.nopendaftar) = ? and j.kodekelompok = ? and not(t.flaglunas = 'F' and t.isvalid <> 0) and t.tgltagihan <= current_date".
					(empty($periode) ? '' : " and (t.periode = ? or ((t.periode is null or t.periode < ?) and t.flaglunas in ('BB','BL','S')))").
					" order by case t.flaglunas when 'BB' then 0 when 'BL' then 0 when 'S' then 1 when 'L' then 2 else 3 end,
					case when t.isvalid = 0 then 1 else 0 end, t.periode";
			
			$param = array($nim,$jenistagihan);
			if(!empty($periode)) {
				$param[] = $periode;
				$param[] = $periode;
			}
		...
		function bayarAllFromRefNo($conn,$refno,$jumlah,$isfrm=false) {
			...
			$rs = $conn->Execute($sql,array($refno));
