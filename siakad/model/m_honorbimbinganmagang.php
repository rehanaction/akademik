<?php
	...
	class mHonorBimbinganMagang extends mModel {
		const schema = 'honorakademik';
		const table = 'hn_bimbinganmagang';
		const order = 'idhonorbimbingan';
		const key = 'idhonorbimbingan';
		const label = 'Honor Pembimbing Terstruktur Magang';
		...
		function genGaji($conn,$record){
			...
			$keyrate='TM|'.$a_mhs['sistemkuliah'];
			$record['honor']=$a_rate[$keyrate]*$record['jumlahbimbingan'];
		...
		function getRowData($conn,$row) {
			...
			$where=" nim='".$row['nim']."' and periode='".$row['periode']."'";
		...
		function getNopengajuan($conn,$kodeunit,$periode,$periodepengajuan){
			...
			where substr(nopengajuan,length(nopengajuan)-7,4)='$tahunpengajuan' and substr(nopengajuan,length(nopengajuan)-10,2)='$bulanpengajuan'";
			$max=$conn->GetOne($sql);
			$urut=(int)$max+1;
			$nourut='TM/'.$periode.'/'.$kodeunit.'/'.$bulanpengajuan.'/'.$tahunpengajuan.'/'.str_pad($urut,3,'0',STR_PAD_LEFT);
