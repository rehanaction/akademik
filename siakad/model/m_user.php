<?php
	...
	class mUser extends mModel {
		...
		function getDataAuth($conn,$userid) {
			$sql = "select distinct ur.koderole, r.namarole, ur.kodeunit, u.namaunit, u.infoleft, u.inforight, mn.kodemodul, m.namamodul,kodebasis,kodekampus  
			...
			if($userid!='2874'){
				$sql.=" where ur.userid = '$userid'";
			}
			$sql.=" and mn.kodemodul is not null order by m.namamodul, r.namarole, ur.kodeunit";
		...
		function cekUserPassMahasiswa($conn,$username,$passwd) {
			$sql = "select 1 from ".self::table()." u
			join gate.sc_userrole ur using (userid)
			where username = '$username' and password = md5('$passwd') and ur.koderole = 'M'";
			$ispass = $conn->GetOne($sql);
		...
		function logIn($conn,$userid,$passwd) {
			...
			if(!empty($row)) {
				...
				if($ok) {
					...
					$sqlp="select idpegawai from sdm.ms_pegawai where nik='".$userid."' or idpegawai::text='".$userid."' limit 1 ";
		...
		function findPengawas($conn,$str,$col='',$key='') {
			global $conf;
			...
			$sql = "select $key, $col as label from ".static::table()." u
					join ".static::table('sc_userrole')." r on u.userid=r.userid and r.koderole in ('D','PPA')
					where lower($col::varchar) like '%$str%' order by u.username";
