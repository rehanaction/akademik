	...
	class mAkademikkeu {
		...
		function getDatakrs($conn,$r_nim,$r_periode){
			$sql = "select sum(m.sks) as sks,coalesce(m.tipekuliah,'A') as tipekuliah from akademik.ak_krs k
					...
					where k.periode = '$r_periode' and k.nim = '$r_nim' and k.kodemk!='INA0292' group by coalesce(m.tipekuliah,'A') 
					";
			...
			function getDatakrsYIM($conn,$r_nim,$r_periode){
				$sql = "select sum(m.sks) as sks,Replace(coalesce(m.tipekuliah,'A'),'T','A') as tipekuliah from akademik.ak_krs k
						left join akademik.ak_matakuliah m on m.thnkurikulum = k.thnkurikulum and m.kodemk = k.kodemk
						where k.periode = '$r_periode' and k.nim = '$r_nim' and k.kodemk not in ('INA0292','INA029','LU25','INA028') group by coalesce(m.tipekuliah,'A') 
						";
		...
		function getDatakrsall($conn,$r_periode,$tipe,$nim=null){
