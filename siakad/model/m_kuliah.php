	...
	class mKuliah extends mModel {
		...
		function listQuery() {
			$sql = "select tglkuliah,
					...
					tglkuliahrealisasi,
					waktumulairealisasi,
					waktuselesairealisasi,
					koderuangrealisasi,
					noharirealisasi,
					nipdosenrealisasi,
					isopen,
					isonline,
					case when validhonorkuliah=-1 then validhonorkuliah else validhonoronline end as validhonorkuliah,
					isdummy,
					nipasisten,
					tglkuliahsemula,
					case when tglvalidhonor is null then case when t_updateuser='rahma' then t_updatetime else tglvalidhonor end else tglvalidhonor end as tglvalidhonor ,
					t_updatetime,
					nim from ".static::table();
		...
		function dataQuery($key) {
			$sql = "select *,case when nim is not null then true else false end as validmhs from ".static::table()." where ".static::getCondition($key);
		...
		function getListPerKelas($conn,$key,$selesai=false) {
			$sql = "select perkuliahanke,tglkuliah,to_char(tglkuliah,'YYYYMMDD') as ftglkuliah,kelompok,jeniskuliah from ".static::table()."
					where ".static::getCondition($key,'thnkurikulum,kodemk,kodeunit,periode,kelasmk,jeniskuliah,kelompok');
			if($selesai)
				$sql .= " and statusperkuliahan = 'S'";
			
			$sql .= " order by ".static::order;
			
			return $conn->GetArray($sql);
		}
		function getListPerKelasOnline($conn_moodle,$key,$selesai=false) {
			$sql = "select distinct perkuliahanke,tglkuliah,to_char(tglkuliah,'YYYYMMDD') as ftglkuliah,kelompok,jeniskuliah from
					moodle.v_absensionline where perkuliahanke is not null and ".static::getCondition($key,'kodemk,kodeunit,periode,kelasmk,jeniskuliah,kelompok');
			$sql .= " order by ".static::order;
			return $conn_moodle->GetArray($sql);
		}
		
		// mendapatkan jumlah jurnal
		function getJumlahPerKelas($conn,$key) {
		...
		function getKehadiran($conn,$thn){
			$sql = "select * from ".self::table('v_kehadiran')." where ".static::getCondition($thn,'periode');
			$row = $conn->GetArray($sql);
			$data = $row;
			return $data;
		}

		function getJumlahMengajar($conn,$key){
			$sql = "select count(kodemk) from ".static::table()." where ".static::getCondition($key,'nipdosen')." group by nipdosen,perkuliahanke order by perkuliahanke asc";
			return (int)$conn->GetOne($sql);
		}
		...
		function getListSelesai($conn,$r_key) {
			...
			//$tglhariini = '2017-03-18';
			$sql = "select tglkuliah, perkuliahanke, tglkuliahrealisasi, koderuangrealisasi, waktumulairealisasi, waktuselesairealisasi
					from ".static::table()."
					where ". static::getCondition($r_key,'thnkurikulum,kodemk,kodeunit,periode,kelasmk,jeniskuliah,kelompok')."
					and statusperkuliahan = 'S'
					and tglkuliahrealisasi = '$tglhariini'									
		...
		function statusKuliah($jenis='') {
			if (($jenis=='P')){
				...
				$data = array('S' => 'Selesai','M'=>'Belum Selesai','B' => 'Batal');			
			}
			else{
				$data =  array( ''=>'Terjadwal', 'J'=>'Terjadwal', 'S' => 'Selesai','M'=>'Belum Selesai','B' => 'Batal');
			}
			return $data;
		}
		...
		function cekIp($conn,$key){
			...
			if($ip==$_SERVER['REMOTE_ADDR'])
				return array(false,'Ruang Kelas Sesuai');
			else
		...
		function cekTgl($conn,$key){
			...
			if($a_data['tglkuliahrealisasi']>date('Y-m-d'))
				return array(true,'Mohon Maaf, Perkuliahan ini dijadwalkan tanggal '.CStr::formatDateInd($a_data['tglkuliahrealisasi']));
			else
				return array(false,'Ini kondisi ketika posterr false');
		}

		function cekTgloL($conn,$key){
			
			$a_data=$conn->GetRow("select tglkuliahrealisasi,waktumulairealisasi,waktuselesairealisasi from ".static::table()." where ".static::getCondition($key));
			if($a_data['tglkuliahrealisasi']!=date('Y-m-d'))
				return array(true,'Mohon Maaf, Perkuliahan OL ini dijadwalkan tanggal '.CStr::formatDateInd($a_data['tglkuliahrealisasi']));
			else
				return array(false,'');
		}
		...
		function getMonitoringRuang($conn,$kodeunit, $periode,$tgl,$waktumin,$waktumax) {
			...
			$sql = "select p.namadepan,p.gelardepan,p.namatengah,p.namabelakang,p.gelarbelakang,mu.namaunit,k.kodemk,m.namamk,k.kelasmk,k.tglkuliahrealisasi,
					...
					where k.periode = '$periode' and k.tglkuliahrealisasi='$tgl' AND am.tugasmengajar = -1 and
		...
		function unsetHonor($conn,$key){
			...
			if(empty($cekvalid))
				$kelas=$conn->Execute("update akademik.ak_kuliah set validhonorkuliah=null, validhonoronline=null where ".static::getCondition($key));
		}
		...
		function getDatajurnalCron($conn_moodle)
		{
			$sql ="SELECT * FROM moodle.status_kehadiran_dosen_summary where tglmulaiperkuliahan>=date_trunc('s',localtimestamp)- interval '8 day' and tglmulaiperkuliahan<=date_trunc('s',localtimestamp) and realisasi is not null and (lower(kodeunit) not like '%mst%' and lower(kodeunit) not like '%pelatihan%' and lower(kodeunit) not like '%contoh%') and kodeunit is not null";
			return $conn_moodle->getArray($sql);
		}
		function updateJurnal($conn,$data,$where)
		{
			//$kolom = implode(',',array_keys($data));
			//$kolomwhere = implode(',',array_keys($where));
			$valuesArrays = array();
			...
			$sql = "update akademik.ak_kuliah set  $values where $values2 ";
			$ok = $conn->Execute($sql);
			if($ok){
				return true;
			}else{
