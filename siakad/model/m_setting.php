<?php
	...
	class mSetting extends mModel {
		...
		function getKartuUtsS1($conn) {
			$sql = "select kartuutssarjana from ".static::table()." where idsetting = 1";
			return $conn->GetOne($sql);
		}
		function getKartuUasS1($conn) {
			$sql = "select kartuuassarjana from ".static::table()." where idsetting = 1";
			return $conn->GetOne($sql);
		}
		function getKartuUtsMM($conn) {
			$sql = "select kartuutsmagister from ".static::table()." where idsetting = 1";
			return $conn->GetOne($sql);
		}
		function getKartuUasMM($conn) {
			$sql = "select kartuuasmagister from ".static::table()." where idsetting = 1";
