<?php
	...
	class mHonorSoal extends mModel {
		const schema = 'honorakademik';
		const table = 'hn_naskahujian';
		const order = 'h.nipdosen, h.kodemk,h.kelasmk';
		const key = 'idnaskahujian';
		const label = 'Honor Naskah Ujian';
		
		// mendapatkan kueri list
		function listQuery() {
			
			$sql="select h.*,m.namamk,
		...
		function genGaji($conn,$unit,$periode,$periodegaji,$jenisujian,$sistemkuliah){
			...
			$q_ujian="select distinct m.nipdosen,k.periode,k.thnkurikulum,k.kodeunit,k.kodemk,k.kelasmk,k.jenisujian,k.jeniskuliah,k.kelompokkul,kl.sistemkuliah 
						...
						where k.kodeunit='$unit' and k.periode='$periode' and k.jenisujian='$jenisujian' and kl.sistemkuliah='$sistemkuliah'";
		...
		function getRowData($conn,$row) {
			...
			$sql="select idnaskahujian,isvalid from ".static::table()." where $where order by isvalid asc";
		...
		function getNopengajuan($conn,$kodeunit,$periode,$periodepengajuan,$jenisujian){
				...
				$max=$conn->GetOne($sql);
				$urut=(int)$max+1;
				$nourut='NU/'.$periode.'/'.$a_jenisujian[$jenisujian].'/'.$kodeunit.'/'.$bulanpengajuan.'/'.$tahunpengajuan.'/'.str_pad($urut,3,'0',STR_PAD_LEFT);
