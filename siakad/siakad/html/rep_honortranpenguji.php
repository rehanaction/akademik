<?php
	...
	// properti halaman
	$p_title = 'Laporan Detail Honor Transport Penguji';
	$p_tbwidth = 650;
	$p_namafile = 'rekap_ajar'.$r_kodeunit;
	
	$a_sistemkuliah=mSistemkuliah::getArray($conn);
	$a_rate=mRateHonor::getJenisRate($conn,'TPS');
	$a_jenispenguji=mHonorPenguji::getJenisSidang();
	
	$a_data = mLaporanHonor::getHonorTransportPenguji($conn,$r_kodeunit,$r_periode,$r_periodegaji,$r_nopengajuan,$r_jenispenguji,$r_sistemkuliah,$r_nip);
...
<tbody>	
	...
	<tr>	
		...
		<td align="left"><?=CStr::formatDateInd($row['tglujian'])?></td>
