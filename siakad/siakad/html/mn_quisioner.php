<?php
	...
	// properti halaman
	$p_title = 'Monitoring Hasil Quisioner';
	$p_tbwidth = 800;
	$p_aktivitas = 'KULIAH';
	...
	$a_kolom[] = array('kolom' => 'namamk', 'label' => 'Nama MK', 'width' => 150);
	...
	$a_data = $p_model::getListMengajar($conn,$a_kolom,$r_sort,$a_filter);
				...
				<center>
					<header style="width:<?= $p_tbwidth ?>px">
						<div class="inner">
							...
							<div class="right">
								<div class="addButton" style="float:left;margin-left:10px">+</div>
							</div>
							<?	}
								if($c_edit) { ?>
							<div class="right">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<?	/********/
						...
						foreach($a_data as $row) {
							...
							$t_key = $p_model::getKeyRow($row)."|".$row['nipdosen'];
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td><?= $row['namadosen'] ?></td>
						<td><?= mMonitoring::getJmlPes($conn,$row['thnkurikulum']."|". $row['kodemk']."|".$row['kodeunit']."|".$row['periode']."|".$row['kelasmk']."|".$row['nipdosen'])?></td>
						<td><?= mMonitoring::getJmlRes($conn,$row['thnkurikulum']."|". $row['kodemk']."|".$row['kodeunit']."|".$row['periode']."|".$row['kelasmk']."|".$row['nipdosen'])?></td>
						<td width="75px">
							<img id="<?= $t_key ?>" title="Isi Quisioner" src="images/link.png" onclick="goPeserta(this.id,'<?= Route::navAddress('list_pesertaquiz') ?>')" style="cursor:pointer">
							<img id="<?= $t_key ?>"  title="Download Saran" src="images/download.png" onclick="goPeserta(this.id,'<?= Route::navAddress('rep_rekapsaranquisioner') ?>')" style="cursor:pointer">
							<img id="<?= $t_key ?>" title="Report /Dosen" src="images/excel.png" width="23" onclick="goPeserta(this.id,'<?= Route::navAddress('rep_quisionerxls2') ?>')" style="cursor:pointer">
...
<table width="130" class="menu-body">
    <tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
        <td onClick="submitPage('key','<?= Route::navAddress('list_pesertaquiz') ?>')">Peserta Kuliah</td>
    </tr>
</table>
</div>
...
function goPeserta(key, url){
	document.getElementById("key").value = key;
	document.getElementById("pageform").action = url;
	goSubmitBlank(url);
}

function goDownloadAll(url){
		document.getElementById("pageform").action = url;
		goSubmit(url);
