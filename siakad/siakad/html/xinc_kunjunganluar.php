<?php
	...
	// properti halaman
	$p_title = 'Pengalaman ke Luar Negeri';
	$p_label = 'kunjungan';
	$p_aktivitas = 'BIODATA';
	$p_model = mPegawai;
	...
	// struktur field
	$a_field = array();
	$a_field[] = array('kolom' => 'nokunjungan', 'label' => 'No', 'type' => 'NP', 'size' => 2, 'maxlength' => 2, 'notnull' => true);
	$a_field[] = array('kolom' => 'negara', 'label' => 'Negara', 'size' => 30, 'maxlength' => 50);
	$a_field[] = array('kolom' => 'tujuan', 'label' => 'Tujuan', 'size' => 30, 'maxlength' => 50);
	$a_field[] = array('kolom' => 'lama', 'label' => 'Lama (Hari)', 'size' => 3, 'maxlength' => 3);
	$a_field[] = array('kolom' => 'sumberbiaya', 'label' => 'Yang Membiayai', 'size' => 30, 'maxlength' => 50);
	...
	if($r_act == 'data' and !empty($r_subkey)) {
		$a_data = $p_model::getDataKunjungan($conn,$r_subkey);
		...
		if($r_act == 'save' and $c_edit) {
			...
			if(empty($r_subkey)) {
				...
				$err = $p_model::insertKunjungan($conn,$record);
			}
			else
				$err = $p_model::updateKunjungan($conn,$record,$r_subkey);
			
			$r_msg = 'Penyimpanan data pengalaman ke luar negeri';
		}
		else if($r_act == 'delete' and $c_delete and !empty($r_subkey)) {
			$err = $p_model::deleteKunjungan($conn,$r_subkey);
			
			$r_msg = 'Penghapusan data pengalaman ke luar negeri';
		}
	...
	// ambil data
	$a_data = $p_model::getListKunjungan($conn,$r_key);
?>
<table width="100%" cellpadding="4" cellspacing="2" class="GridStyle" align="center">
	...
	<?	/********/
		...
		foreach($a_data as $row) {
			...
			$t_key = $p_model::getKeyRow($row,$p_model::keyKunjungan);
	?>
	<tr valign="top" class="<?= $rowstyle ?>">
		...
		<?	} ?>
		<td align="center">
			<img id="<?= $t_key ?>" title="Tampilkan Detail" src="images/edit.png" onclick="goDetailPop('<?= $p_label ?>',this)" style="cursor:pointer">
		<?	if($c_delete) { ?>
			<img id="<?= $t_key ?>" title="Hapus Data" src="images/delete.png" onclick="setKeyPop('<?= $p_label ?>',this); goDeletePop('<?= $p_label ?>')" style="cursor:pointer">
		<?	} ?>
		</td>
	</tr>
	<?	}
