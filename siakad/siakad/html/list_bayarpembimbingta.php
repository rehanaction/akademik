<?php
	...
	// properti halaman
	$p_title = 'Setting Pembayaran Honor Dosen Pembimbing Tugas Akhir';
	$p_tbwidth = 800;
	$p_aktivitas = 'ABSENSI';
	...
	if($r_act == 'bayarHonor' and $c_edit) {
		...
		foreach($a_penerimahonor as $nodosen=>$nodosen){
			list($p_posterr,$p_postmsg) =mEmailHonor::kirimHonorPembTa($conn,$conn_sdm,$nodosen);
		}
	...
	$a_input[] = array('label' => 'Nomor Pengajuan', 'nameid' => 'a_nopengajuan[]', 'type' => 'C', 'option' => $p_model::listNopengajuan($conn,$r_unit,$r_periodegaji,true));
?>
<html>
<head>
	<title><?= $p_title ?></title>
