<?php
	...
	foreach(Modul::getAccessRole() as $t_akses) {
		$t_label = $t_akses['namarole'].' - '.$t_akses['namaunit'];
		if(!empty($t_akses['namabasis']))
			$t_label .= ' - '.$t_akses['namabasis'];
		if(!empty($t_akses['namakampus']))
			$t_label .= ' - '.$t_akses['namakampus'];
		
		$a_hakakses[$t_akses['role'].':'.$t_akses['unit']] = $t_label;
	}
	
	// combo hak akses
	$l_hakakses = UI::createSelect('hakakses',$a_hakakses,Modul::getRole().':'.Modul::getUnit(),null,true,'style="width:280px"');
?>
		...
		<ul class="topnav" style="float:left">
			<li>
				<a href="javascript:void(0)" style="padding:0px;height:22px;">
					<img src="images/gear.png" width="25" style="position:relative;bottom:5px; float:left;">Menu
				</a>
				<ul class="subnav" style="width:350px;padding:15px 5px; border-bottom-left-radius:4px; border-bottom-right-radius:4px;">
...
function goLogout() {
		location.href = '../../front/gate/index.php?page=logout';
	}
