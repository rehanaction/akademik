<?php
	...
	$c_delete = $a_auth['candelete'];
		...
		//$conn->debug=true;
	// include
	require_once(Route::getModelPath('honorkoreksiujian'));
	require_once(Route::getModelPath('kuliah'));
	require_once(Route::getUIPath('combo'));
	
	// properti halaman
	$p_title = 'Generate Honor Koreksi Hasil Ujian (UTS & UAS)';
	$p_tbwidth = 900;
	$p_aktivitas = 'ABSENSI';
	
	
	
	$p_model = mHonorKoreksiUjian;
	...
	$a_kolom[] = array('kolom' => 'kl.sistemkuliah', 'label' => 'Basis','type'=>'S','option'=>mCombo::sistemKuliah($conn));
	...
	if($r_act == 'delete' and $c_delete) {
		...
		list($p_posterr,$p_postmsg) = $p_model::genGaji($conn,$r_unit,$r_periode,$r_periodegaji,$r_jenisujian,$r_jeniskuliah,$r_sistemkuliah);
	...
	if(!empty($r_jeniskuliah)) $a_filter[] = $p_model::getListFilter('jeniskuliah',$r_jeniskuliah); 
		...
		<div class="SideItem" id="SideItem">
			...
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<table width="<?= $p_tbwidth-200 ?>" cellpadding="6" cellspacing="0" class="GridStyle">
					...
					<tr>		
						<td width="100"> &nbsp; <strong>Pilih Jenis Pertemuan</strong></td>
						<td>
							<?php
							$a_jeniskuliah=mKuliah::jenisKuliah($conn);
							echo uCombo::combo($a_jeniskuliah,$r_jeniskuliah,'jeniskuliah','onchange="goSubmit()"',false,'Jenis Kuliah')
							?>
						</td>
					</tr>
					<tr>		
