<?php
	...
	$a_kolom[] = array('kolom' => 'validhonorkuliah', 'label' => 'Approve', 'type' => 'C', 'option' => array('-1' => ''), 'align' => 'center');
	$a_kolom[] = array('kolom' => 'tglvalidhonor', 'label' => 'Tanggal Validasi', 'type' => 'D');			
	...
	if($r_act=='Approve'){
		$r_subkey = CStr::removeSpecial($_POST['subkey']);
		$record = array();
		$record['validhonorkuliah']=-1;
		$record['tglvalidhonor']= date("Y-m-d");
		list($p_posterr,$p_postmsg) = $p_model::updateRecord($conn,$record,$r_subkey,true);
	}elseif($r_act=='CancelApprove'){
		$r_subkey = CStr::removeSpecial($_POST['subkey']);
		$record = array();
		$record['validhonorkuliah']=NULL;
		$record['tglvalidhonor']= date("Y-m-d");
		list($p_posterr,$p_postmsg) = $p_model::updateRecord($conn,$record,$r_subkey,true);
	}
			...
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<th colspan="5">Realisasi Perkuliahan</th>
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td><?php
						if($row['statusperkuliahan']=='Selesai'){
							if($row['isonline']!='Online'){
								if(empty($row['validhonorkuliah']) Or $row['validhonorkuliah']===0 ){?> 
									<img id="<?=$t_key?>" title="Approve" src="images/check.png" onclick="goApprove(this)" style="cursor:pointer">
						<?php	}else{ ?>
									<img id="<?= $t_key ?>" title="Cancel" src="images/delete.png" onclick="goCancelApprove(this)" style="cursor:pointer">
						<?php 	}
							}else{
								echo '';
							}
...
function goApprove(elem){
		document.getElementById("act").value = "Approve";
		document.getElementById("subkey").value = elem.id;
		goSubmit();
	
}
function goCancelApprove(elem){
	document.getElementById("act").value = "CancelApprove";
	document.getElementById("subkey").value = elem.id;
	goSubmit();
}
