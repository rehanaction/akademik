<?php
	...
	// ada aksi
	$r_act = $_REQUEST['act'];
	if($r_act == 'savekeu' and $c_edit) {
		if(isset($_POST['cekalKeu'])){
			foreach($_POST['cekalKeu'] as $key=>$value){
				$record=array();
				$record['statusmhs']='T';
				$record['prasyaratspp']=0;
				list($p_posterr,$p_postmsg) = $p_model::updateCRecord($conn,$a_input,$record,$key);
			}
		}
	}
	else if($r_act == 'saveakad' and $c_edit) {
		if(isset($_POST['cekalAkad'])){
			foreach($_POST['cekalAkad'] as $key=>$value){
				$record=array();
				$record['cekalakad']=0;
				list($p_posterr,$p_postmsg) = $p_model::updateCRecord($conn,$a_input,$record,$key);
			}
	...
	// filter tree
	$a_filtertree = array();
	$a_filtertree['unit'] = array('label' => 'Prodi', 'data' => mCombo::unitTree($conn));
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<th id="prasyaratspp">Cekal Keuangan <br><input type="checkbox" id="checkAllKeu" title="Check/Uncheck All"></th>
						<th id="cekalakad">Cekal Akademik <br><input type="checkbox" id="checkAllAkad" title="Check/Uncheck All"></th>
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td align="center"><input type="checkbox" class="cekalKeu" name="cekalKeu[<?=$t_key?>]" id="<?=$t_key?>" <?=($row['prasyaratspp']==0 and $row['statusmhs']=='T')?'checked':''?> title="Lepas Cekal" <?=!$c_edit?'disabled':''?>></td>
						<td align="center"><input type="checkbox" class="cekalAkad" name="cekalAkad[<?=$t_key?>]" id="<?=$t_key?>" <?=($row['cekalakad']==-1)?'checked':''?> title="Lepas Cekal" <?=!$c_edit?'disabled':''?>></td>
						<td align="center">
							<?php if($c_edit){?>
					...
					<tr class="LeftColumnBG">
						<th colspan="3">&nbsp;</th>
						<td align="center">
							<input type="button" value="Simpan Keu" onclick="goSaveKeu()" style="font-size:14px">
						</td>
						<td align="center">
							<input type="button" value="Simpan Akad" onclick="goSaveAkad()" style="font-size:14px">
						</td>
						<td>&nbsp;</td>
					</tr>
						<!--
...
$(document).ready(function() {
	$("[id='checkAllAkad']").click(function() {
			var checked = $(this).attr("checked");
			if(checked)
				$("[class='cekalAkad']").attr("checked", checked);
			else
				$("[class='cekalAkad']").removeAttr("checked", checked);
		});
	$("[id='checkAllKeu']").click(function() {
			var checked = $(this).attr("checked");
			if(checked)
				$("[class='cekalKeu']").attr("checked", checked);
			else
				$("[class='cekalKeu']").removeAttr("checked", checked);
		});	
...
function goSaveKeu(elem) {
	document.getElementById("act").value = "savekeu";
	goSubmit();
}
function goSaveAkad(elem) {
	document.getElementById("act").value = "saveakad";
	goSubmit();
}
</script>
</body>
