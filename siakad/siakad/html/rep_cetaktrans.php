...
<head>
	...
	<style>
		...
		.div_head { font-size:12px; font-weight:bold; text-align:center; text-decoration:underline; margin-top:50px; width:750px; }
...
<div style="font-family:'Arial Narrow','Arial'; font-size:20px;"><b><u>TRANSKRIP AKADEMIK</u></b></div>
<div style="font-family:'Arial Narrow','Arial'; font-size:13px;">
	<?php 
		if ($row['periodemasuk'] < "20181") { ?>
			No. INABA-<?= substr($row['kodeunit'], -5) ?>-<?=substr($row['tgllulus'], 0,4)?>/<?= substr($row['nim'],-5)?>
		<?php }else{ ?>
			No. INABA-<?= substr($row['kodeunit'], -5) ?>-<?=substr($row['tgllulus'], 0,4)?>/<?= substr($row['nim'],4,2)?><?= substr($row['nim'],-4)?>
		<?php } ?>
...
<?			if($s == 0) { ?>
	<tr valign="top">
		...
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= $row['nim'] ?></td>
		<?php if($kodeunit=='4302161101'){ ?>
			<td style="font-family:'Arial Narrow','Arial';font-size:13px;width:100px">Program Pendidikan</td>
		<td align="center" width="10">:</td>
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;">
			<?php if ($row['program']=="S1") {
				echo "Sarjana";
			}else{
				echo "Magister";
			} ?>
	...
	<tr valign="top">
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;">Nama</td>
		<td align="center">:</td>
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= ucwords(strtolower($row['nama'])) ?></td>
		<?php if($kodeunit=='4302161101'){ ?>
			<td style="font-family:'Arial Narrow','Arial';font-size:13px;width:100px">Program Studi</td>
			<td align="center" width="10">:</td>
			<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= $row['namaunit'] ?></td>

		<?php } ?>
	</tr>
	<tr valign="top">
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;">Tempat & Tanggal Lahir</td>
		<td align="center">:</td>
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= ucwords(strtolower($row['tmplahir'])).", ". CStr::formatDateInd($row['tgllahir']) ?></td>
...
<tr valign="top">
	...
	<tr valign="top">
		...
		<?php } ?>
	<?php
		if ($row['kodeunit'] == "4302161201") { ?>
		 	<tr valign="top">
				<td style="font-family:'Arial Narrow','Arial';font-size:13px;">Peminatan</td>
				<td align="center">:</td>
				<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= $prodi_mhs['bidang'] ?></td>
			</tr>
			
	<?php }else{ ?>
		<tr valign="top">
			<td style="font-family:'Arial Narrow','Arial';font-size:13px;">&nbsp;</td>
			<td align="center">&nbsp;</td>
			<td style="font-family:'Arial Narrow','Arial';font-size:13px;">&nbsp;</td>
		</tr>
...
?>
	...
	<tr height="16">
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= $rowt['kodemk'] ?></td>
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= strtoupper($rowt['namamk']) ?></td>
		<td align="center" style="font-family:'Arial Narrow','Arial';font-size:13px;"><?= $rowt['sks'] ?></td>
		<td style="font-family:'Arial Narrow','Arial';font-size:13px;">&nbsp;&nbsp;&nbsp;&nbsp;<?= $rowt['nhuruf'] ?></td>
	</tr>  	
<?php
					}
...
<table class="tb_data" width="100%">
<tr valign="top">
	<td align="left" style="font-family:'Arial Narrow','Arial';font-size:13px;">Judul <?= $row['program']=="S1" ? 'Skripsi' : 'Tesis'; ?> :<br><br>
		<div class="ta"><?= $row['judulta'] ?></div>
	</td>
</tr>
</table>
<div style="height:20px"></div>
<table class="tb_data" width="100%" style='font-family: "Arial Narrow","Arial"; font-size:13px;'> 
	<tr>
		<td style="width:140px">Lulus Tanggal</td><td>:</td><td><?= CStr::formatDateInd($row['tgllulus']) ?></td>
	</tr>
	<tr>
		<td>Jumlah SKS Diperoleh</td><td>:</td><td><?= $row["skslulus"] ?></td>
	</tr>
	<tr>
		<td>Indeks Prestasi Kumulatif</td><td>:</td><td><?php if(!empty($row["ipk"])){ echo $row["ipk"]; }else{ echo $row["ipklulus"];} ?></td>
	</tr>
	<tr>
