<?php
	...
	// properti halaman
	$p_title = 'Detil Slip Honor Transport Dosen Penguji';
	$p_tbwidth = 700;
	...
	$a_data = mLaporanHonor::getHonorTransportPenguji($conn,'',$r_periode,$r_periodegaji,'','','',$r_nip);
	$a_gaji=array();
...
<tbody>	
	...
	<tr>	
		<td class="marger"><?=$a_jenispenguji[$row['jenispenguji']]?></td>
		<td class="marger"><?=$row['nim'].' '.$row['nama']?></td>
		<td class="marger"><?=CStr::formatDateInd($row['tglujian'])?></td>
