...
$(document).ready(function() {
	...
	$("#<?= $t_xauto['text'] ?>").xautox({strpost:"f=<?= $t_xauto['param'] ?>", targetid: "<?= $t_xauto['nameid'] ?>"});
	<? } ?>
});

	function loadBProdiFakultas(){
		var param = $("#nim").val();
	
		var jqxhr = $.ajax({
						url: ajaxpage,
						timeout: ajaxtimeout,
						data: { f: "acmahasiswaprodifakultas", q: param }
					});
	
		jqxhr.done(function(data) {
			var obj = JSON.parse(data);
			var jnssurat = $("#jnssurat").val();
			var isisurat ='';
			if ( jnssurat=="1" ){
				var str = $("#nmrsuket").val();
				var arr = str.split('/');
				arr[3]='KET';
				str = arr.join('/');
				$("#nmrsuket").val(str);
				
				isisurat = 'Adalah benar mahasiswa STIE INABA Jurusan ' +  obj.namaprodi ;
			}
			else if ( jnssurat=="2" ){
				var str = $("#nmrsuket").val();
				var arr = str.split('/');
				...
				isisurat = 'Telah mengajukan permohonan mengundurkan diri sebagai mahasiswa STIE INABA terhitung mulai '+obj.semesterakhir ;
			}
			
			$("#isisuket").html( isisurat );
		});
