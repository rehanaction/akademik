<?php
	...
	// properti halaman
	$p_title = 'Laporan Pemindah Bukuan Honor Pengawas Ujian';
	$p_tbwidth = 700;
	$p_namafile = 'rekap_ajar'.$r_kodeunit;
	
	$a_jenisujian=mCombo::getJenisUjian();
	$a_data = mLaporanHonor::getPemindahBukuanPengawas($conn,$r_periode,$r_periodegaji,$r_nopembayaran);
	...
	$arr_pegawaipenunjang=mJadwalUjian::getDataPengawas($conn,$a_nipdosen);
	// header
...
<table width="<?= $p_tbwidth?>" class="tb_data">
	...
	foreach($a_data['data'] as $data){
		...
		//cek array pegawai, jika kosong ambil dr pegawai penunjang
		$a_pegawai=$arr_pegawai[$nip];
		if(empty($a_pegawai))
			$a_pegawai=$arr_pegawaipenunjang[$nip];
			
		$no++;
		
		//kurangi dengan PPH
		$pros_pajak=$a_pegawai['pph'];
		$pajak=($pros_pajak/100)*$data['jumlahhonor'];
		$pajak=round($pajak);
		$honor=$data['jumlahhonor']-$pajak;
	
		//honor dikurangi bia transfer
		$biatrans=round($a_pegawai['biatrans']);
	...
	?>
	<tr>
		<td align="center"><?= $a_pegawai['norekeninghonor'] ?></td>
		<td align="center">+</td>
		<td align="right"><?= number_format($honor,2,',','.') ?></td>
		<td align="center">C</td>
		<td align="right"><?= str_pad($no, 4, "0", STR_PAD_LEFT); ?></td>
		<td align="left"><?=!empty($a_pegawai['anrekeninghonor'])?strtoupper($a_pegawai['anrekeninghonor']):$data['namadosen'].' belum mempunyai rekening'?></td>
		<td align="left">HONOR PENGAWAS</td>
