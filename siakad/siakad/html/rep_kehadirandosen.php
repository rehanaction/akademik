<?php
	...
	$r_nip = Modul::setRequest($_POST['username'],'USERNAME');
	...
	// properti halaman
	$p_title = 'Jumlah Kehadiran dosen';
	$p_tbwidth = 1000;
	
	$a_jeniskuliah=mKuliah::jenisKuliah($conn);
	$a_data =  mLaporanKelas::getAbsensiDosen($conn,$r_unit,$r_periode,$r_jeniskuliah,$r_sistemkuliah,$r_starttgl,$r_endtgl,$r_nip);
	
	$a_jumkelas=array();
	foreach($a_data as $data){
		$idx=$data['thnkurikulum'].$data['kodeunit'].$data['kodemk'].$data['kelasmk'];
		$a_jumkelas[$idx]+=$data['jumlahrealisasi'];
	}
...
<body>
<div align="center">
	<?=strtoupper('Laporan Jumlah Kehadiran dosen '.$namaunit.' '.Akademik::getNamaPeriode($r_periode))?><br>
	...
	<table class="tb_data" width="<?= $p_tbwidth ?>">
		...
		<tr>
			...
			<th width="30">Sesi</th>
			<th width="30">Basis</th>
			<th width="10">SKS</th>
			<th width="100">Jadwal</th>
			<th width="30">Ruang</th>
			<th width="150">Dosen Pengajar</th>
			<th width="5">Hadir Dosen</th>
			<th width="5">Realisasi Kelas</th>
		</tr>
		</thead>
		<tbody>
		<?	
		...
		<tr>
			...
			<td align="center"><?=$r_sistemkuliah?></td>
			<td align="center"><?=$row['sks']?></td>
			<td><?= implode('<br>',$a_jadwal) ?></td>
			<td align="center"><?=$row['koderuang']?></td>
			<td align="left"><?=$row['nipdosen']?> -  <?=$row['namadosen']?></td>
			<td align="center"><?=$row['jumlahrealisasi']?></td>
			<td align="center"><?=$a_jumkelas[$idx]?></td>
		</tr>
