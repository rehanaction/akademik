<?php
	...
	// properti halaman
	$p_title = 'Setting Pembayaran Honor Mengajar';
	$p_tbwidth = 800;
	$p_aktivitas = 'ABSENSI';
	
	
	
	$p_model = mHonorAsisten;
	...
	$a_valid=array('0'=>'&nbsp','-1'=>'<img src="images/check.png">');
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'h.nipasisten', 'label' => 'Dosen');
	$a_kolom[] = array('kolom' => 'namamk', 'label' => 'Nama Mata Kuliah');
	$a_kolom[] = array('kolom' => 'h.skshonor', 'label' => 'SKS');
	...
	$a_kolom[] = array('kolom' => 'isvalid', 'label' => 'Valid?','type'=>'S','option'=>$a_valid);
	$a_kolom[] = array('kolom' => 'nopengajuan', 'label' => 'Nomor Pengajuan');
	$a_kolom[] = array('kolom' => 'nopembayaran', 'label' => 'Nomor Pembayaran');
	...
	if($r_act == 'bayarHonor' and $c_edit) {
		...
		foreach($a_penerimahonor as $nodosen=>$nodosen){
			list($p_posterr,$p_postmsg) =mEmailHonor::kirimHonorMengajar($conn,$conn_sdm,$nodosen);
		}
	...
	$a_input[] = array('label' => 'Bulan/Tahun Pengajuan', 'input' => uCombo::bulan($r_bulanbayar,'bulanbayar','onchange="goSubmit()"',false).' '.uCombo::tahun($r_tahunbayar,true,'tahunbayar','onchange="goSubmit()"',false));
			...
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						<td><?= $row['nipasisten'] ?>-<?= $row['namaasisten'] ?></td>
						<td><?= $row['kodemk'] ?> - <?= $row['namamk'] ?></td> 
						<td><?= $row['sks'] ?></td>
						<td><?= $row['kelasmk'] ?></td>
						<td><?= $row['basis'] ?></td>
						<td><?= $row['tglkuliahrealisasi'] ?></td> 
						<td><?= number_format($row['honordosen']) ?></td> 
						<td><?= $row['isvalid'] ?></td> 
