<?php
	...
	if(!empty($r_mail)) {
		...
		else if(empty($a_cek['flagbaca']))
			$p_model::setBaca($conn,$r_mail);
	}
	...
	if($r_act == 'delete' and $c_delete) {
		...
		list($p_posterr,$p_postmsg) = $p_model::delete($conn,$r_key,'inbox');
	}
		...
		if(!empty($a_centang)) {
			...
			if($r_subact == 'delete' and $c_delete)
				list($p_posterr,$p_postmsg) = $p_model::delete($conn,$a_centang,'inbox');
			else if($r_subact == 'read' and $c_edit)
				list($p_posterr,$p_postmsg) = $p_model::setBaca($conn,$a_centang);
			else if($r_subact == 'unread' and $c_edit)
				list($p_posterr,$p_postmsg) = $p_model::setBelumBaca($conn,$a_centang);
	...
	if(empty($r_mail)) {
		...
		$a_data = $p_model::getListInbox($conn,$r_page);
	...
	<div id="wrapper">
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post">
				...
				<table width="100%" cellpadding="4" cellspacing="0" class="GridStyle" align="center" style="cursor:pointer">
					<tr>
						<td class="Selected" onclick="goInbox()">
				...
				<?			}
							if($c_edit) {
				?>
				<u class="ULink" onclick="goBacaCentang()">Set Dibaca</u> &nbsp;&nbsp;
				<u class="ULink" onclick="goBelumBacaCentang()">Set Belum Dibaca</u>
				<?			} ?>
				<div class="Break"></div>
				<?		} ?>
				<table width="100%" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<?	$i = 0;
						foreach($a_data as $row) {
							...
							if (empty($row['flagbaca'])) $rowstyle = 'NormalBG';  else $rowstyle = 'GreyBG'; $i++;
					...
					<tr class="<?= $rowstyle ?>">
						...
						<td align="center">
							<img id="<?= $t_key ?>" title="Tampilkan Pesan" src="images/list.png" onclick="goDetail(this)" style="cursor:pointer">
							<?	if($c_delete) { ?>
				...
				<table width="100%" cellpadding="4" cellspacing="0" class="NoGrid">
					<tr valign="top">
						<td width="80"><strong>Pengirim</strong></td>
						<td width="10" align="center"><strong>:</strong></td>
						<td><?= $a_cek['namapengirim'] ?></td>
					</tr>
					<tr valign="top">
				...
				<table border="0" cellspacing="4">
					<tr>
						<td class="TDButton" onclick="goInbox()">
							<img src="images/list.png"> Daftar
						</td>
						<td id="<?= $r_mail ?>" class="TDButton" onclick="goReply(this)">
							<img src="images/reply.png"> Reply
						</td>
						<td id="<?= $r_mail ?>" class="TDButton" onclick="goForward(this)">
							<img src="images/forward.png"> Forward
						</td>
...
function goBacaCentang() {
	document.getElementById("act").value = "check_read";
	goSubmit();
}

function goBelumBacaCentang() {
	document.getElementById("act").value = "check_unread";
	goSubmit();
}

function goDetail(elem) {
	goView('list_inbox&mail='+elem.id);
}
