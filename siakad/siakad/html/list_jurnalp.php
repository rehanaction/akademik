			...
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<?php /*<th width="30">Rencana Materi</th> */ ?>
						<th width="30">Realisasi</th>
						<?  if($c_edit and !$pengajar) { ?>
						<th width="30">Edit</th>
						<?	}
							if($c_delete and !$pengajar) { ?>
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						...
						<td><?php if($row['perkuliahanke']==8)
										{ echo "UTS";}
										elseif($row['perkuliahanke']==16){
											 echo "UAS"; 
										}else{ 
											echo $row['isonline']; 
										} ?></td>
						...
						<td align="center"><?= $row['validhonorkuliah']; ?> </td> 
						<?php /*<td align="center"><?php if($row['statusperkuliahan']=='Terjadwal'){ ?>
							...
							<?php } ?></td> */?>
						<td align="center">
						<?php if($row['statusperkuliahan']!='Terjadwal' or ($row['statusperkuliahan']=='Terjadwal') and ($row['perkuliahanke']!=8 and $row['perkuliahanke']!=16) and $row['isonline']!="Online"){ ?>
							<img id="<?= Akademik::base64url_encode($t_key) ?>" name="<?=$row['isopen']?>" title="Realisasi Perkuliahan" src="images/link.png" onclick="goDetailRealisasi(this)" style="cursor:pointer">
						<?php }?>	
						</td>
						<?	if($c_edit and !$pengajar) { ?>
						<td align="center">
							<?php if($row['statusperkuliahan']=='Terjadwal'){ ?>
							<img id="<?= Akademik::base64url_encode($t_key) ?>" title="Tampilkan Detail" src="images/edit.png" onclick="goDetail(this)" style="cursor:pointer">
							<?php } ?>
						</td>
						<?		}
								if($c_delete and !$pengajar) { ?>
						<td align="center"><img id="<?= Akademik::base64url_encode($t_key) ?>" title="Hapus Data" src="images/delete.png" onclick="goDelete(this)" style="cursor:pointer"></td>
						<?		}
								if($c_edit and !$pengajar) { ?>
						<td align="center">
							<input type="checkbox" value="<?= Akademik::base64url_encode($t_key) ?>" onclick="goOpenJurnal(this)"<?= empty($row['isopen']) ? '' : ' checked' ?>>
						</td>
						<?		} ?>
					</tr>
					<?	}
				...
				<table width="100%">
					<tr>
						<td><center><b>Pertemuan ke 8 = UTS <br> Pertemuan ke 16 = UAS</b></center></td>
					</tr>
				</table>
...
var detailpage = "<?= Route::navAddress($p_detailpage) ?>&pkey=<?= Akademik::base64url_encode($r_key) ?>";
var jurnalpage = "<?= Route::navAddress($p_jurnalpage) ?>&pkey=<?= Akademik::base64url_encode($r_key) ?>";
var jurnalrealisasipage = "<?= Route::navAddress('data_jurnal')?>&pkey=<?= Akademik::base64url_encode($r_key) ?>";
var realisasipage = "<?= Route::navAddress($p_detailrealisasipage)?>&pkey=<?= Akademik::base64url_encode($r_key) ?>";
...
function goDetailRealisasi(elem) {
	...
	var win = window.open(detailpage + "&key=" + elem.id+'&jenis=R'+'&open='+open,'_blank');
	win.focus();
