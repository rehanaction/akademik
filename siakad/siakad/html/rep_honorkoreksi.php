<?php
	...
	$r_jeniskuliah = CStr::removeSpecial($_REQUEST['jeniskuliah']);
	...
	$a_data = mLaporanHonor::getHonorKoreksi($conn,$r_kodeunit,$r_periode,$r_periodegaji,$r_jenisujian,$r_jeniskuliah,$r_nopengajuan,$r_sistemkuliah,$r_nip);
...
<table width="<?= $p_tbwidth ?>" class="tb_head">
	...
	<tr>
		<td>jenis Kuliah</td>
		<td>:</td>
		<td><?=$jeniskuliah[$r_jeniskuliah]?></td>
		<td>Basis</td>
		<td>:</td>
...
<table width="<?= $p_tbwidth ?>" class="tb_head" >
	<tr>
		...
		<td width="310" rowspan="2" align="right">
			Rate Honor Koreksi <?=$a_ratetransport['KU|'.$r_sistemkuliah]?>
