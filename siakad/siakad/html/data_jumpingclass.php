<?php
	...
	require_once(Route::getModelPath('akjumpingclass'));
	...
	// properti halaman
	$p_title = 'Data KRS Mahasiswa (Via Jumping Class)';
	$p_tbwidth = 600;
	$p_aktivitas = 'NILAI';
	...
	if(empty($r_key) or $r_act == 'change') {
		...
		$post['nilai'] = Modul::setRequest($_POST['nilai']);
		$post['kodejumping'] = Modul::setRequest($_POST['kodejumping']);
	...
	$a_input = array();
	//data jumping class
	$a_input[] = array('kolom' => 'kodejumping', 'label' => 'Kriteria Jumping Class', 'type' => 'S', 'option' => mCombo::listJumpingClass($conn));
	$a_input[] = array('kolom' => 'nilai', 'label' => 'Nilai', 'maxlength' => 5, 'size' => 3);
	...
	if($r_act == 'save' and $c_edit) {
		...
		if(empty($r_key)){
			...
			if(!$p_posterr){
				$r_keyjumping=$r_key.'|'.$record['kodejumping'];
				list($p_posterr,$p_postmsg) = mAkJumpingClass::insertCRecord($conn,$a_input,$record,$r_keyjumping);
			}
		}else{
			list($p_posterr,$p_postmsg) = $p_model::updateCRecord($conn,$a_input,$record,$r_key);
			if(!$p_posterr){
				$r_keyjumping=$r_key.'|'.$record['kodejumping'];
				list($p_posterr,$p_postmsg) = mAkJumpingClass::updateCRecord($conn,$a_input,$record,$r_keyjumping);
			}
		}
		...
		$r_keyjumping=$r_key.'|'.$_POST['kodejumping'];
		list($p_posterr,$p_postmsg) = mAkJumpingClass::delete($conn,$r_keyjumping);
		if(!$p_posterr)
	...
	// ambil data halaman
	//if(!empty($r_key))
	$row = $p_model::krsJumpingClass($conn,$a_input,$r_key,$post);
?>
<html>
<head>
	<title><?= $p_title ?></title>
			...
			<form name="pageform" id="pageform" method="post">
				...
				<center>
					...
					<table width="<?= $p_tbwidth-22 ?>" cellpadding="4" cellspacing="2" align="center">
						<tr>
							<td colspan="2" class="DataBG">Data Jumping Class</td>
						</tr>
						<?= Page::getDataTR($row,'kodejumping') ?>
						<?= Page::getDataTR($row,'nilai') ?>
						<tr>
							<td colspan="2" class="DataBG">Data KRS</td>
						</tr>
