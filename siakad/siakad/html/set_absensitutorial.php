<?php
	...
	// properti halaman
	$p_title = 'Pengisian Absensi Kegiatan Tutorial Kelompok ';
	$p_tbwidth = 900;
	$p_aktivitas = 'ABSENSI';
	...
	$a_kuliah = mKuliah::getListPerKelas($conn,$r_key,true,true,false);
	$a_kelompok = mPesertaruang::getDataKelompok($conn,$r_key);
	$kul_kelompok=array();
	foreach($a_kuliah as $pertemuan)
		$kul_kelompok[$pertemuan['kelompok']]=$pertemuan['kelompok'];
	...
	if($r_act == 'save' and $c_edit) {
		...
		foreach($a_kuliah as $t_kuliah) {
			...
			foreach($a_data as $t_data) {
				$cek=$_POST['check_'.$t_kuliah['perkuliahanke'].'_'.$t_kuliah['ftglkuliah'].'_'.$t_data['nim']];
				if(!empty($cek) and $cek!='A')
					$a_absen[$t_data['nim']] = $cek;
			}
			//print_r($a_absen);
			 //masukkan data
	...
	<div id="wrapper">
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<br>
				<?php for($x=1;$x<=$a_kelompok['keltutorial'];$x++){ ?>	
				<center>
