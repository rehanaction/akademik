<?php
	...
	// properti halaman
	$p_title = 'Data Tagihan Registrasi';
	$p_tbwidth = 600;
	$p_aktivitas = 'KULIAH';
	...
	// cek data
	if(!empty($r_key)) {
		$g_lunas = $p_model::getLunasReg($connh,$r_key);
	...
	// struktur view
	$a_input = array();
	$a_input[] = array('kolom' => 'periode', 'label' => 'Periode', 'type' => 'S', 'option' => $p_model::periodeDaftar($conn));
	$a_input[] = array('kolom' => 'notest', 'label' => 'Nomor Tes', 'maxlength' => 20, 'size' => 20);
	$a_input[] = array('kolom' => 'nama', 'label' => 'Nama Pendaftar', 'maxlength' => 60, 'size' => 50);
	$a_input[] = array('kolom' => 'jalur', 'label' => 'Jalur', 'maxlength' => 50, 'size' => 50);
	$a_input[] = array('kolom' => 'jurusan', 'label' => 'Prodi', 'type' => 'S', 'option' => $p_model::unit($connh), 'notnull' => true);
	...
	if($r_act == 'save' and $c_edit) {
		...
		$record['billcode'] = $p_model::billreg;
		...
		if(!$err)
			$r_key = $record['billcode'].'|'.$record['periode'].'|'.$record['notest'];
	...
	// ambil data halaman
	$row = $p_model::getDataEditReg($connh,$a_input,$r_key,$post);
