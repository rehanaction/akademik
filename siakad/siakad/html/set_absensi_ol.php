<?php
	...
	// include
	require_once(Route::getModelPath('absensikuliahol'));
	require_once(Route::getModelPath('kelas'));
	require_once(Route::getModelPath('kuliahol'));
	...
	// properti halaman
	$p_title = 'Pengisian Absensi '.$a_jeniskul[$jeniskul].' ONLINE ';
	$p_tbwidth = 900;
	$p_aktivitas = 'ABSENSI';
	$p_listpage = 'list_absensi';
	$p_printpage = 'rep_isiabsensi';

	$p_model = mAbsensiKuliahOl;
	
	
	
	// mendapatkan data ak_kelas
	$a_infokelas = mKelas::getDataSingkat($conn,$r_key);
	if($jeniskul=='P')
		$kelprak=$kelompok;
    
  // mendapatkan data ak_krs
			...
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<? if(!empty($p_kulnum)) { ?>
						<th colspan="<?= $p_kulnum ?>" style="white-space:nowrap">Kegiatan</th>
						<? } ?>
					</tr>
					<tr>
						<?	foreach($a_kuliah as $t_kuliah) { ?>
						<th width="25">KE-<?= $t_kuliah['perkuliahanke'] ?></th>
						<?	} ?>
					</tr>
					<tr>
						<?	foreach($a_kuliah as $t_kuliah) { ?>
              ...
              <? if($t_selected=="H")  { ?>
                ...
                <option <?=$t_selected=='A'?'selected':''?>><?=$t_selected?></option>
                <!--<option <?=$t_selected=='H'?'selected':''?>>H</option>-->
								<option <?=$t_selected=='I'?'selected':''?>><?=$t_selected?></option>
								<option <?=$t_selected=='S'?'selected':''?>><?=$t_selected?></option>  
							</select>
