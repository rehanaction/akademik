<?php
	...
	// properti halaman
	$p_title = 'Perencanaan Jurnal Praktikum';
	$p_tbwidth = 840;
	$p_aktivitas = 'ABSENSI';
	$p_listpage = 'list_absensi';
	$p_printpage = 'rep_jurnal';
	$p_detailrealisasipage = 'data_jurnaldppu';
	$p_detailpage = 'data_jurnaldppu';
	...
	$f = 3;
	$a_kolom[] = array('kolom' => 'perkuliahanke', 'label' => 'Pertemuan ke /<br> Kelompok', 'size' => 2, 'maxlength' => 2, 'notnull' => true);
	...
	$a_infokelas = mKelas::getDataSingkatPrakt($conn,$r_key);
?>
<html>
<head>
	<title><?= $p_title ?></title>
	...
	<div id="wrapper">
		<div class="SideItem" id="SideItem">
			...
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<?php require_once('inc_headerkelasprakt.php') ?>
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					?>
					<tr>
						<th colspan="6">Rencana Perkuliahan</th>
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						<td><?= $rowc[++$j].'/'.$row['kelompok'] ?> <?=$openwithtime?></td>
						<td>
							<?= $rowc[2] ?>, <?= $rowc[++$j] ?>
							<div class="Break"></div>
							<?= $rowc[0] ?> - <?= $rowc[1] ?>
						...
						<td><?= $row['validhonorkuliah'] ?></td> 
						<td align="center">
						<?php if($row['statusperkuliahan']!='Terjadwal' or ($row['statusperkuliahan']=='Terjadwal' and !empty($row['isopen'])) or $openwithtime){ ?>
							<img id="<?= $t_key ?>" name="<?=$row['isopen']?>" title="Realisasi Perkuliahan" src="images/link.png" onclick="goDetailRealisasi(this)" style="cursor:pointer">
						<?php }?>	
						</td>
						<?	if($c_edit and !$pengajar) { ?>
						<td align="center">
						...
						<?		}
								if($c_edit and !$pengajar) { ?>
						<td align="center">
							<input type="checkbox" value="<?= $t_key ?>" onclick="goOpenJurnal(this)"<?= empty($row['isopen']) ? '' : ' checked' ?>>
						</td>
						<?		} ?>
					</tr>
					<?	}
...
var jurnalrealisasipage = "<?= Route::navAddress('data_jurnaldppu')?>&pkey=<?= $r_key ?>";
