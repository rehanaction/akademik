<?php
	...
	// properti halaman
	$p_title = 'Daftar KRS yang Pernah Diambil (Via Jumping Class)';
	$p_tbwidth = 700;
	$p_aktivitas = 'NILAI';
	...
	if($r_act == 'set' and $c_edit) {
		...
		$t_key = CStr::removeSpecial($_POST['key']);
		$del = mAkJumpingClass::deletejumping($conn,$t_key);
		if($del)
	...
	$a_data = mKRS::getDataPerSemester($conn,$r_key,$a_infomhs['periodedaftar'],true,false,true);
...
<div id="main_content">
	...
	<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle">
		<tr class="DataBG">
			<td colspan="10" align="center"><?= Akademik::getNamaPeriode($t_data[0]['periode']) ?></td>
		</tr>
		<tr>
			<th>No</th>
			<th>Kurikulum</th>
