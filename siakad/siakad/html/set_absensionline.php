<?php
	...
	$r_key2 = $kodemk."|".$kodeunit."|".$periode."|".$kelasmk."|".$jeniskul."|".$kelompok;
	...
	$a_kuliah = mKuliah::getListPerKelasOnline($conn_moodle,$r_key2,true);
	...
	if($r_act == 'save' and $c_edit) {
		...
		foreach($a_kuliah as $t_kuliah) {
			...
			if(!empty($a_absen)){
				$err = $p_model::savePerPertemuan2($conn,$r_key.'|'.$t_kuliah['perkuliahanke'].'|'.$t_kuliah['tglkuliah'],$a_absen);
				if($err) break;
			}
		}
		//die();
	...
	// mendapatkan data
	$a_absen = $p_model::getListPerKelasOnline($conn_moodle,$r_key2);
	//print_r($a_absen);
		...
		<div class="SideItem" id="SideItem">
			<form name="pageform" id="pageform" method="post" enctype="multipart/form-data">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<? if(!empty($p_kulnum)) { ?>
						<th colspan="<?= $p_kulnum ?>" style="white-space:nowrap">Online</th>
						<? } ?>
					</tr>
					<tr>
						<?	foreach($a_kuliah as $t_kuliah) { ?>
			...
			<?php /*
              ...
              <?  } ?>  
						...
						*/?> 
						<?= $t_selected ?>
						<input type="hidden"  name="check_<?= $t_kuliah['perkuliahanke'] ?>_<?= $t_kuliah['ftglkuliah'] ?>_<?= $t_key ?>" value=<?=$t_selected?>>
						</td>
						<?	} ?>
					</tr>
					<?	}
