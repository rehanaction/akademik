<?php
	...
	else if($f == 'acmahasiswakelas') {
		require_once(Route::getModelPath('kelas'));
		
		$a_data = mKelas::findPeserta($conn,$q[1],"m.nim||' - '||m.nama",$q[0],"m.nim");
		
		echo json_encode($a_data);
	}
	else if( $f == "acmahasiswaprodifakultas" ){
		require_once(Route::getModelPath('mahasiswa'));

		$a_data = mMahasiswa::getProdiFakultas($conn, $q);
		$a_data["semesterakhir"] = Akademik::getNamaPeriode($a_data['semesterakhir']);
...
<?php
	...
	//$conn->debug=true;
	...
	}else if($f == 'setDosenWali'){
		require_once(Route::getModelPath('dosenwali'));
		$ok = mDosenwali::setWaldosen($conn,$q[0]);
		...
		$id=explode("|",$q[0]);
		$kolom = $id[9]=="R07" || $id[9]=="R10" ? "validhonoronline" : "validhonorkuliah" ;
		$kelas=$conn->Execute("update akademik.ak_kuliah set $kolom=-1 where ".mKuliah::getCondition($q[0],'thnkurikulum , kodemk , kodeunit , periode , kelasmk , perkuliahanke, tglkuliah, jeniskuliah, kelompok'));
		if($kelas)
		...
		if($honor){
			$cekvalid=$conn->GetRow("select 1 from akademik.ak_honordosen where validhonor=-1 and ".mKuliah::getCondition($q[0],'thnkurikulum , kodemk , kodeunit , periode , kelasmk , perkuliahanke, tglkuliah, jeniskuliah, kelompok, isonline2'));
			if(empty($cekvalid))
				$id=explode("|",$q[0]);
				$kolom = $id[9]=="R07" || $id[9]=="R10" ? "validhonoronline" : "validhonorkuliah" ;
				$kelas=$conn->Execute("update akademik.ak_kuliah set $kolom=null where ".mKuliah::getCondition($q[0],'thnkurikulum , kodemk , kodeunit , periode , kelasmk , perkuliahanke, tglkuliah, jeniskuliah, kelompok'));
		}
		if($honor){
