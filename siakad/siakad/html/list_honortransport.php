<?php
	...
	$c_delete = $a_auth['candelete'];
		...
		//$conn->debug=true;
	// include
	require_once(Route::getModelPath('honortransport'));
	require_once(Route::getModelPath('kuliah'));
	require_once(Route::getUIPath('combo'));
	
	// properti halaman
	$p_title = 'Generate Honor Transport Dosen';
	$p_tbwidth = 800;
	$p_aktivitas = 'ABSENSI';
	
	
	
	$p_model = mHonorTransport;
	...
	$a_kolom[] = array('kolom' => 'tglmengajar', 'label' => 'Tanggal Mengajar', 'type' => 'D');
	...
	if($r_act == 'delete' and $c_delete) {
		...
		list($p_posterr,$p_postmsg) = $p_model::genGaji($conn,$conn_sdm,$r_periode,$r_periodegaji);
	}
