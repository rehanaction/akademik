<?php
	...
	// include
	require_once(Route::getModelPath('honorpengawas'));
	require_once(Route::getModelPath('kuliah'));
	require_once(Route::getModelPath('emailhonor'));
	require_once(Route::getUIPath('combo'));
	
	// properti halaman
	$p_title = 'Setting Pembayaran Honor Pengawas Ujian';
	$p_tbwidth = 800;
	$p_aktivitas = 'ABSENSI';
	
	
	
	$p_model = mHonorPengawas;
	...
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'userdesc', 'label' => 'Dosen');
	$a_kolom[] = array('kolom' => 'tglujian', 'label' => 'Tanggal Ujian', 'type' => 'D');
	$a_kolom[] = array('kolom' => 'ju.kodemk', 'label' => 'Mata Kuliah');
	$a_kolom[] = array('kolom' => 'ju.kelasmk', 'label' => 'Seksi');
	$a_kolom[] = array('kolom' => 'ju.jenisujian', 'label' => 'Jenis','type'=>'S','option'=>array('U'=>'UAS','T'=>'UTS'));
	...
	if($r_act == 'bayarHonor' and $c_edit) {
		...
		foreach($a_penerimahonor as $nodosen=>$nodosen){
			list($p_posterr,$p_postmsg) =mEmailHonor::kirimHonorPengawas($conn,$conn_sdm,$nodosen);
		}
