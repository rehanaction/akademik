<?php
	...
	// properti halaman
	$p_title = 'Laporan Pemindah Bukuan Honor Asisten';
	$p_tbwidth = 700;
	$p_namafile = 'rekap_ajar'.$r_kodeunit;
	
	
	$a_data = mLaporanHonor::getPemindahBukuanAsisten($conn,$r_periode,$r_periodegaji,$r_nopembayaran);
	
	//buat array nipdosen
	$a_nipasisten=array();
	foreach($a_data['data'] as $key=>$dataasisten)
		$a_nipasisten[]=$dataasisten['nipasisten'];
	
	//ambil data dari sdm
	$arr_pegawai=mPegawai::getDataDosen($conn_sdm,$a_nipasisten);
...
<table width="<?= $p_tbwidth?>" class="tb_data">
	...
	<tr>
		...
		<td align="left"><?=!empty($arr_pegawai[$nip]['anrekeninghonor'])?strtoupper($arr_pegawai[$nip]['anrekeninghonor']):$data['namaasisten'].' belum mempunyai rekening'?></td>
