...
function formatStr(num) {
	...
	num = arrnum[0].replace(/\B(?=(\d{3})+(?!\d))/g, ".");
	if(arrnum[1])
		num += ("," + arrnum[1]);
...
function goGuide() {
	window.open("help/Guide IAIN Administrator Akademik.pdf");
}
...
(function($){
	...
	$.fn.xLoad = function() {
		return $(this).each(function() {
			...
			if(typeof(page) == "undefined" || typeof(key) == "undefined")
				return true;
			
			$.get("index.php?page="+page,"key="+key,function(data) {
				div.html(data);
			});
		});
	};
