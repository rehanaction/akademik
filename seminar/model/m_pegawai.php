<?php
	...
	class mPegawai extends mBiodata {
		...
		function listQuery() {
			$sql = "select ".static::schema.".f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as namalengkap,u.namaunit,p.nik,
		...
		function getNikPegawai($conn, $key){
			$sql = "select nik from ".static::schema.".ms_pegawai where idpegawai='$key'";
		...
		function getAIDPegawai($conn){
			$sql = "select idpegawai,nik from ".static::schema.".ms_pegawai where idstatusaktif in (select idstatusaktif from ".static::schema()."lv_statusaktif where iskeluar='T')";
			$rs = $conn->Execute($sql);
			
			$a_data = array();
			while ($row = $rs->FetchRow())
				$a_data[$row['nik']] = $row['idpegawai'];
		...
		function getArrayTtd($conn,$unit) {
				$info = $conn->GetRow("select infoleft, inforight from gate.ms_unit where kodeunit = '$unit'");
				$sql = "select p.idpegawai, akademik.f_namalengkap(p.gelardepan,p.namadepan,p.namatengah,p.namabelakang,p.gelarbelakang) as nama 
						from ".static::table()." p 
						join gate.ms_unit u on p.nik = u.ketua and
						u.infoleft >= '".$info['infoleft']."' and 
						u.inforight <= '".$info['inforight']."' 
						order by nama";
		...
		function getSimplePegawai($conn,$idpegawai) {
			$sql = "select ".static::schema.".f_namalengkap(gelardepan,namadepan,namatengah,namabelakang,gelarbelakang) as namalengkap,nik from ".static::table()." where idpegawai = '$idpegawai'";
		...
		function createNIP($conn){
				...
				$bln = date('m');
				$urut = $conn->GetOne("select coalesce(max(cast(substring(nik,6,4) as int)),0)+1 from ".static::schema.".ms_pegawai where length(nik) = 9 and substring(nik,2,2) = '$thn'");
