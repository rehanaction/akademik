...
class mLaporan {
		...
		function getRekapKuisioner($conn,$idseminar,$periode){
			$sql = "select * from seminar.ms_seminar where idseminar = '$idseminar'";
			$seminar = $conn->getRow($sql);
			
			$peserta = $conn->getArray("select ps.nopeserta,ps.nopendaftar,ps.idseminar,ps.waktudaftar,ps.rfid, ps.isvalid,ps.islunas,ps.waktucheckin,ps.waktucheckout,
						ps.isisikuis, ps.kritik,ps.saran, pd.nama 
						from seminar.ms_peserta ps
						left join seminar.ms_pendaftar pd using (nopendaftar) 
						where idseminar = '$idseminar' ");
			$pertanyaan = $conn->getArray("select * from seminar.ms_pertanyaankuisseminar where idseminar = '$idseminar' order by nomor");
			
			$sql_j = "select a.*, j.teksjawaban from seminar.ms_jawabankuispeserta a 
						join seminar.ms_pertanyaankuisseminar b using (idpertanyaankuisseminar)
						left join seminar.ms_jawabankategorikuis j on j.idkategori = b.idkategori and j.kodejawaban = a.kodejawaban
						where b.idseminar = '$idseminar' ";
			$rs_j = $conn->getArray($sql_j);
			foreach ($rs_j as $k => $v){
				$jawaban[$v['nopeserta']][$v['idpertanyaankuisseminar']] = $v['kodejawaban'].' ('.$v['teksjawaban'].')';
			}
