<?php
	...
	class mModel {
		...
		function insertCRecord($conn,$kolom,$record,&$key) {
			...
			// $err = Query::recInsert($conn,$record,static::table());
			list($err,$row) = Query::recInsert($conn,$record,static::table(),static::key);
			if(!$err) {
				/* $seq = static::sequence;
				if(empty($seq))
		...
		// array combo
		function arrQuery($conn,$filter=null) {
			$sql = "select ".static::key.", ".static::value." from ".static::table();
			if(!empty($filter))
				$sql .= " where $filter";
