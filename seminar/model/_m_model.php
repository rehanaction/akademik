<?php
	...
	class mModel {
		...
		function insertCRecord($conn,$kolom,$record,&$key) {
				...
				foreach($kolom as $datakolom) {
					if($datakolom['readonly']) {
						...
						if(!empty($file) and empty($file['error'])) {
							...
							$file['recordname'] = $datakolom['kolom'];
			...
			if(!$err) {
				...
				if(!empty($upload)) {
					...
					foreach($upload as $file) {
						...
						$filename=str_replace('|','_',$key).'.'.$ext['extension'];
						$rec=array();
						$rec[$file['recordname']]=$filename;
						$up=static::updateCRecord($conn,"",$rec,$key);
						$file['name'] = $conf['upload_dir'].$file['uptype'].'/'.$filename;
						
						$ok = Route::uploadFile($file['arrtype'],$ext['extension'],$file['tmp_name'],$file['name']);
						if(!$ok) break;
					}
			...
			return static::insertStatus($conn,$kolom,'','','',$record);
		}
		...
		function insertStatus($conn,$kolom='',$label='',$table='',$schema='',$record='') {
			if(empty($label))
				$label = static::label;
			if(empty($table))
				$table = static::table;
					...
					case 'TOO LONG':
						...
						foreach($meta as $t_kolom => $t_meta) {
							if($t_meta['tipe'].':'.$t_meta['panjang'] == $info) {
								...
								if(!empty($record) and strlen($record[$t_kolom]) <= $t_meta['panjang'])
									continue;
						...
						if(empty($t_msg))
							...
							$msg .= $t_msg;
						break;
						case 'NOT PRESENT': $msg.='<br>'.$info; break;
						default : $msg.='<br>'.$info;
				}
			}
			else {
		...
		function updateCRecord($conn,$kolom,$record,&$key) {
			...
			if(!$err) {
				...
				if(!empty($upload)) {
					...
					foreach($upload as $file) {
						$ext=pathinfo($file['name']);
						$filename=str_replace('|','_',$key).'_'.$file['recordname'].'.'.$ext['extension'];
						$rec=array();
						$rec[$file['recordname']]=$filename;
						$up=static::updateCRecord($conn,"",$rec,$key);
						$file['name'] = $conf['upload_dir'].$file['uptype'].'/'.$filename;
						...
						//$ext=pathinfo($file['name']);
						//$file['name'] = $conf['upload_dir'].$file['uptype'].'/'.str_replace('|','_',$key).'.'.$ext['extension'];
