<?php
	...
	class mCombo {
		function kategorikuis($conn) {
			require_once(Route::getModelPath('kategorikuis'));
			
			return mKategoriKuis::getArray($conn);
		}

		function dosen($conn,$unit) {
			require_once(Route::getModelPath('pegawai'));
			
			return mPegawai::getArrayTtd($conn,$unit);
		}
		...
		function periode($conn) {
			$sql = "select periode from akademik.ms_periode order by periode DESC";
			
			return Query::arrQuery($conn,$sql);
		}

		function seminar($conn,$periode=null) {
			$sql = "select idseminar,namaseminar from seminar.ms_seminar";
			if(!empty($periode))
				$sql .= " where periode = ".Query::escape($periode);
			$sql .= " order by namaseminar";
		...
		function kampus($conn){
			$sql = "select kodekampus, namakampus from akademik.lv_kampus order by kodekampus DESC";
