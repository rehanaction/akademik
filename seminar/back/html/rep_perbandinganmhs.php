<?
	...
	//model
	$p_model=mLaporan;
	$p_title='Laporan Perbandingan Mahasiswa';
	$p_tbwidth = 950;
	
	$data=$p_model::getDataPerbandinganMahasiswa($conn, $r_tglawal,$r_tglakhir);
	$list_prodi = mCombo::unitProdi($conn);
	$arrTahun = date::getYear($r_tglawal,$r_tglakhir);
	$namaperiode = mSetting::getPeriode($conn);
	
	$arrData = array();
	foreach ($data as $val){
		$arrData[$val['periodemasuk']][$val['kodeunit']] = $val['jumlah'];
	}
	
	$p_namafile = 'perbandinganmahasiswa_'.$periode;
...
<html>
<head>
	<title>Cetak Perbandingan Mahasiswa</title>   
	...
	<center>
		...
		LAPORAN PERBANDINGAN MAHASISWA
		<table width="100%">
			<tr>
