<?php
	...
	// properti halaman
	$p_title = 'Kelulusan Tes Wawancara';
	$p_tbwidth = 550;
	$p_aktivitas = 'DAFTAR';
	$p_model = mPendaftar;
	...
	$l_jalur 	= uCombo::jalurWawancara($conn,$r_jalur,'','jalur');
	...
	if(isset($_POST['save'])){
		...
		if($lulus==-1)
		$record['luluswawancara']		=$lulus;
		else
		$record['luluswawancara']		=0;
				    ...
				    <?
						...
						while($pendaftar = $data->FetchRow()){
							...
							<tr align="center">
								...
								<td><input type="checkbox" value="-1" id=<?=$pendaftar['luluswawancara']==true?"checked":"checklist"?> name="<? echo "check".$pendaftar['nopendaftar']; ?>" <? if($pendaftar['luluswawancara']==true) echo ' checked="checked" '; ?>/></td>
