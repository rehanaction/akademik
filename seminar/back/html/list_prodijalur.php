<?php
	...
	// variabel request
	$r_prodi 	= Modul::setRequest($_POST['kodeunit'],'Program Studi');
	$r_jalur 	= Modul::setRequest($_POST['jalur'],'Jalur Penerimaan');
	

	//combo
	$l_prodi 	= uCombo::unit($conn,$r_prodi,'','kodeunit','onchange="goSubmit()"');
	$l_jalur 	= uCombo::jalur($conn,$r_jalur,'','jalur','onchange="goSubmit()"');
	
	// struktur view
	$a_kolom = array();
	$a_kolom[] = array('kolom' => 'namaunit', 'label' => 'Program Studi');
	$a_kolom[] = array('kolom' => 'jalurpenerimaan', 'label' => 'Jalur');	
	$a_kolom[] = array('kolom' => 'kuotaprodi', 'label' => 'Kuota');	
	$a_kolom[] = array('kolom' => 'keterangan', 'label' => 'Keterangan');
	
	// properti halaman
	$p_title = 'Daftar Setting Prodi Jalur Penerimaan';
	$p_tbwidth = 900;
	$p_aktivitas = 'SPMB';
	...
	$a_datafilter = Page::getFilter($a_kolom);
		...
		// mendapatkan data
	if(!empty($r_prodi)) $a_filter[] 	= $p_model::getListFilter('kodeunit',$r_prodi);
	...
	// membuat filter
	$a_filtercombo = array();
	$a_filtercombo[] = array('label' => 'Program Studi', 'combo' => $l_prodi);
