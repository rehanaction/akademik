<?php
	...
	class uCombo {
		function kategorikuis($conn,&$valvar,$nameid='',$add='',$empty=false) {
			$data = mCombo::kategorikuis($conn);
		...
		function periode($conn,&$valvar,$nameid='',$add='',$empty=true) {
			$data = mCombo::periode($conn,$unit);
						
			return self::combo($data,$valvar,$nameid,$add,$empty,'Periode');
		}
		function seminar($conn,&$valvar,$nameid='',$add='',$empty=true) {
			$data = mCombo::seminar($conn);
						
			return self::combo($data,$valvar,$nameid,$add,$empty,'Seminar');
		}
