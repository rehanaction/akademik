<?php
	...
	$a_filtertree['p.isbatalnim'] = array('label' => 'Batal NIM', 'data' => mCombo::batalNim());
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr valign="top" class="<?= $rowstyle ?>">
						<td><?= substr($row['nim'],0,4).'-'.substr($row['nim'],4,2).'-'.substr($row['nim'],6) ?></td>
						<td><?= $row['nama'] ?></td>
						<td><?= $row['sex'] ?></td>
						<td><?= $row['namaunit'] ?></td>
						<? /* <td><?= $rowc[$j++] ?></td> */ ?>
						...
						<? /* <td>
							...
							&nbsp;&nbsp;&nbsp;&nbsp;
							<? } else { ?>
							<span class="hint--top" data-hint="<?= $row['alamat'] ?>"><img id="imgcontact" src="images/house.png"></span>
							<? } if(empty($row['telp'])) { ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<? } else { ?>
							<span class="hint--top" data-hint="<?= $row['telp'] ?>"><img id="imgcontact" src="images/telp.png"></span>
							<? } if(empty($row['hp'])) { ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<? } else { ?>
							<span class="hint--top" data-hint="<?= $row['hp'] ?>"><img id="imgcontact" src="images/hp.png"></span>
							<? } if(empty($row['email'])) { ?>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<? } else { ?>
							<span class="hint--top" data-hint="<?= $row['email'] ?>"><img id="imgcontact" src="images/mail.png"></span>
							<? } ?>
						</td> */ ?>
						<td align="center"><?= $row['statusmhs'] // ++$j] ?></td>
						<td align="center"><?= $row['sistemkuliah'] ?></td>
						<td align="center"><img id="<?= $t_key ?>" title="Tampilkan Detail" src="images/search.png" onclick="goDetail(this)" style="cursor:pointer"></td>
						<?
							if($c_delete) { ?>
...
<table class="menu-body">
	...
	<tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
        <td onClick="showPage('npm','<?= Route::navAddress('view_nilaimhs') ?>')">Daftar Nilai</td>
    </tr>
	<tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
	...
	<tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
        <td onClick="showPage('npm','<?= Route::navAddress('set_transkrip') ?>')">Preview Transkrip</td>
    </tr>
	<tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
        <td onClick="showPage('npm','<?= Route::navAddress('list_krs') ?>')">Edit KRS</td>
    </tr>
	<tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
        <td onClick="showPage('npm','<?= Route::navAddress('set_rfid') ?>')">Registrasi RFID</td>
    </tr>
    <tr class="menu-button" onMouseMove="this.className = 'hover'" onMouseOut="this.className = ''">
