<?php
	...
	echo 'c_edit='.$c_edit.' and !p_limited='.$p_limited.'<<<<=<';
				...
				<center>				<div class="tabs" style="width:<?= $p_tbwidth ?>px">
					...
					<div id="items">
						...
						<table width="100%" cellspacing="2" cellpadding="4" align="center" class="GridStyle">
							...
							<?php 
							$i = 1;
							foreach($rowd['penghargaan'] as $rowh){ 
								?>
							<tr>
								<td><?=$i++;?></td>
								<td><?=$rowh['tglpenghargaan']?></td>
								...
								<? if($c_update and $rowh['isvalid']!='-1'){ ?>
							...
							<tr valign="top" class="LeftColumnBG" id="edit" style="display:none">
								<td></td>
								<? if($c_update){
									foreach($a_detail['penghargaan']['data'] as $datakolom) {
											$datakolom['nameid'] = 'penghargaan_'.CStr::cEmChg($datakolom['nameid'],$datakolom['kolom']);
									?>
										<td><?= uForm::getInput($datakolom) ?></td>
									<?	} ?>
									<td align="center">
										<img title="Tambah Data" src="images/disk.png" onclick="goInsertDetail('penghargaan')" style="cursor:pointer">
									</td>
								<? } ?>
