<?php
	...
	class Route {
		...
		// header laporan
		function reportHeader($format,$file) {
			switch($format) {
				case 'doc' :
		...
		// path model
		function getModelPath($name,$modul='') {
			global $conf;
			
			return $conf[$modul.'model_dir'].'m_'.strtolower($name).'.php';
		}
		function getControllerPath($name) {
			global $conf;
			
			return $conf['frontcontroller_dir'].'c_'.strtolower($name).'.php';
		}
		...
		// upload file
		/*
		function uploadFile($arr_tipe,$tipe,$from,$to) {
