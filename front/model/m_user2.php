<?php
	...
	require_once('../helpers/modul2.class.php');
	require_once('../helpers/cstr2.class.php');
	
	class mUser2 extends mModel2 {
		const schema = 'gate';
		const table = 'sc_user';
		const order = 'userid';
		const key = 'userid';
		...
		function deleteRole($conn,$key) {
			Query2::qDelete($conn,static::table('sc_userrole'),static::getCondition($key));
		...
		function changePassword($conn,$userid) {
			$sql = "update ".self::table()." set password = md5(coalesce(hints,'')),
					t_updateact = 'resetpass', ".Query2::logUpdate()." where userid = '$userid'"; 
			$conn->Execute($sql);
			
			return $conn->ErrorNo();
		}
		function UpdateSyncMoodle($conn,$record,$key,$status=false) {
			$err = Query2::recUpdate($conn,$record,static::table(),static::getCondition($key));
			if($status)
		...
		function resetPasswordForget($conn,$token) {
			$sql = "select userid, hints from ".self::table()." where tokenreset = ".Query2::escape($token);
			$row = $conn->GetRow($sql);
			...
			$err = Query2::recUpdate($conn,$record,self::table(),'userid = '.Query2::escape($userid));
			if($err)
		...
		function logIn($conn,$userid,$passwd) {
			$userid = CStr2::removeSpecial($userid);
			//$conn->debug=true;
			...
			if(!empty($row)) {
				...
				if($ok) {
					require_once('m_usersession2.php');
					
					$record = array();
					$record['t_userid'] = $userid;
					$record['t_username'] = $row['userdesc'];
					$record['t_logintime'] = $now;
					$record['t_loginattempt'] = Modul2::getLoginAttempt();
					$record['t_ipaddress'] = $ip;
					$record['t_hostname'] = $_SERVER['REMOTE_HOST'];
					$record['t_osname'] = Modul2::getAgent();
					
					$err = mUserSession2::insertRecord($conn,$record);
					$ok = Query2::isOK($err);
					
					$row['loginsession'] = mUserSession2::getLastValue($conn);
		...
		function logOut($conn) {
			require_once(Route2::getModelPath('usersession'));
			
			// update waktu logout
			$record = array();
			$record['t_logouttime'] = date('Y-m-d H:i:s');
			
			$err = mUserSession2::updateRecord($conn,$record,Modul2::getLoginSession());
