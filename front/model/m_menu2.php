<?php
	...
	class mMenu2 extends mModel2 {
		...
		function delete($conn,$key) {
			...
			if(!$err)
				$err = Query2::qDelete($conn,static::table('sc_menurole'),static::getCondition($key));
			if(!$err)
				$err = Query2::qDelete($conn,static::table('sc_menufile'),static::getCondition($key));
			if(!$err)
				$err = Query2::qDelete($conn,static::table('sc_menuakses'),static::getCondition($key));
			if(!$err)
				$err = Query2::qDelete($conn,static::table(),static::getCondition($key));
		...
		function deleteLeaf($conn,$left,$urutan,$level) {
			...
			if($ok) {
				...
				$sql = "update ".static::table()." set kodeurutan = substring(kodeurutan,1,$step)||lpad((substring(kodeurutan,$step+1,2)::int-1)::text,2,'0')||substring(kodeurutan,$step+3)
		...
		function insertRecord($conn,$record,$status=false) {
			if($record['idmenuparent'] != 'null') {
				$a_parent = $conn->GetRow("select levelmenu, inforight, kodeurutan from ".static::table()." where idmenu = '".$record['idmenuparent']."'");
				$t_urutan = $conn->GetOne("select max(kodeurutan) from ".static::table()." where idmenuparent = '".$record['idmenuparent']."'");
			...
			if(!$err) {
				$err = Query2::recInsert($conn,$record,static::table());
				if(!$err) {
					$seq = static::sequence;
					if(empty($seq))
		...
		function moveUp($idmenu) {
			...
			if(!$err) {
				...
				$sql = "update ".static::table()." set infoleft = infoleft - $t_selisih, inforight = inforight - $t_selisih, t_updateact = 'wait'
						...
						update ".static::table()." set ".Query2::logUpdate().", t_updateact = 'moveup'
		...
		function moveDown($idmenu) {
			...
			if(!$err) {
				...
				$sql = "update ".static::table()." set infoleft = infoleft - $t_selisih, inforight = inforight - $t_selisih, t_updateact = 'wait'
						...
						update ".static::table()." set ".Query2::logUpdate().", t_updateact = 'movedown'
		...
		function saveAksesRole($conn,$role,$akses) {
			...
			if(!$err and !empty($a_hapus)) {
				...
				Query2::qDelete($conn,static::table($table),$cond);
		...
		function saveAksesKode($conn,$idmenu,$akses) {
			...
			Query2::qDelete($conn,static::table($table),static::getCondition($idmenu));
			list($err,$msg) = static::deleteStatus($conn);
			...
			if(!$err) {
				...
				foreach($akses as $kode => $nama) {
					...
					Query2::recInsert($conn,$record,static::table($table));
