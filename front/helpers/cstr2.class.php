<?php
	...
	class CStr2 {
		...
		// bila $item kosong, set null, bila tidak, strip
		function cStrNullNoRemove($item) {
			if($item == '')
				return 'null';
			else
				return $item;
		...
		function cStrFill($src,$elem='',$idx=false, $noremove=false) {
			if(is_array($elem)) {
				$retarr = array();
				for($i=0;$i<count($elem);$i++) {
					if(is_array($src[$elem[$i]])) {
						if($idx === false)
							...
							if ($noremove)
								$retarr[$elem[$i]] = CStr::cStrNullNoRemove($src[$elem[$i]][$idx]);
							else
						...
						if ($noremove)
							$retarr[$elem[$i]] = CStr::cStrNullNoRemove($src[$elem[$i]]);
						else
				...
				foreach($src as $key => $val) {
					if(is_array($val)) {
						if($idx === false) 
							...
							if ($noremove)
							$retarr[$key] = CStr::cStrNullNoRemove($val[$idx]);
							else
						...
						if ($noremove)
						$retarr[$key] = CStr::cStrNullNoRemove($val);
						else
