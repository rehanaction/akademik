<?php
	...
	if($r_act == 'changepass') {
		...
		$r_usernm = Modul::getUserName();
		$r_oldpass = $_POST['oldpass'];
		$r_newpass = $_POST['newpass'];
		
		// cek kebenaran password
		if ( mUser::isLdapUser($conn, $r_usernm) ){
			$ldap=new Ldap(); 
			$ok = $ldap->userPasswordisMatch($r_usernm, $r_oldpass, $err);
		}else{
		...
		if($ok) {
			...
			if($p_posterr)
				...
				if ( mUser::isLdapUser($conn, $r_usernm) ){
					$ldap=new Ldap(); 
					$ldap->userUpdatePassword($r_usernm, $r_newpass, $err);
				}
		}
		else {
			$p_posterr = -1;
...
<form id="menuform" method="post">
	<div class="logo-menus background-01 margin-up30px">
    	<div class="img-menus"><img src="images/header-top.jpg" width="auto;" height="100px" /></div>
    	<div class="back-shadow"><img src="images/back-shadow.jpg" width="100%;" height="auto" /></div>
    </div>
	...
	<div id="main" class="well">
		...
		<div id="<?= $t_data['kode'] ?>" class="panel">
			<div class="content">
				...
				<div class="role_box">
					...
					<span class="role_container" onClick="goAccess('<?= $t_modul ?>','<?= $t_kolom['koderole'] ?>','<?= $t_kolom['kodeunit'] ?>','<?= $t_kolom['kodebasis'] ?>','<?= $t_kolom['kodekampus'] ?>')" style="float:left;width:100%;">
						<span class="rolename"><?= $t_kolom['namarole'] ?> -&nbsp;</span>
						<span class="unitname"><?= $t_kolom['namaunit'] ?>
						<?= empty($t_kolom['namabasis']) ? '' : '- '.$t_kolom['namabasis'] ?>
						<?= empty($t_kolom['namakampus']) ? '' : '- '.$t_kolom['namakampus'] ?></span>
					</span>
					<? } ?>
				</div>
			</div>
		...
		<div class="clr"></div>
		<div id="bottom" class="margin-bottom20px">
...
<script type="text/javascript">
	...
	function goAccess(modul,role,unit,kodebasis,kodekampus) {
		$.ajax({
			type: "POST",
			url: "<?= Route::navAddress('ajax') ?>",
			data: "c=access&p="+modul+"_"+role+"_"+unit+"_"+kodebasis+"_"+kodekampus,
			timeout: 90000,
