<?php
	...
	else if($r_act == 'goUser'){
		$a_filter = Page::setFilter($_POST['filter']);
		$a_data = array();
		$record['issync']='-1';
		//print_r($_POST['userid']);
		foreach($_POST['userid'] as $rows){
			...
			<form name="pageform" id="pageform" method="post">
				...
				<table width="<?= $p_tbwidth ?>" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
					...
					<tr>
						...
						<th align="center" class="HeaderBG" width="70">Aksi <input type="checkbox" id="chk_new" onclick="checkAll('chk');"></th>
						<?	} ?>
					</tr>
					<?	/********/
						/* ITEM */
					...
					<tr valign="top" class="<?= $rowstyle ?>" onMouseOver="this.style.backgroundColor='#FFDAB9'" onMouseOut="this.style.backgroundColor=''">
						...
						<td align="center">
						<?		if($c_editpass) { ?>
							<input type="checkbox" name="userid[]" value=<?=$row['userid']?> id="chk">
						<?		}}
								?>
					...
					<?
							...
							//mUser::syncUserToElearning($conn,$row);
					}
						if($i == 0) {
					?>
					<tr>
				...
				<br/>
				<br/>
				<table width="700px" cellpadding="4" cellspacing="0" class="GridStyle" align="center">
				<tr class="NoHover NoGrid">		
						<td align="center" ><input type="button" value="Syncronize User To elearning" onclick="goUser()"></td>
				</tr>
...
function goUser(){
	document.getElementById("act").value = "goUser";
	goSubmit();
}
