<?php
	...
	if($r_act == 'save' and $c_edit) {
		...
		$record = CStr::cStrFill($_POST,array('namamenu','namafile'), false, true);
		
		if(empty($r_key)) {
			$record['kodemodul'] = $r_modul;
			$record['idmenuparent'] = CStr::cStrNull($_POST['idmenuparent']);
	...
	foreach($a_data as $row) {
		$a_last[$row['idmenuparent']] = $row['idmenu'];
		if(empty($a_first[$row['idmenuparent']]))
			$a_first[$row['idmenuparent']] = $row['idmenu'];
	}
...
<?php
	...
	foreach($a_data as $row) {
		...
		// cek level
		if($row['levelmenu'] > 0 and !in_array($row['idmenuparent'],$a_expand))
			$t_trclass .= ' Hidden';
		
		// cek parent
		$t_trid = 'item_'.$t_id;
		$t_imgid = 'img_'.$t_id;
		if(!empty($row['idmenuparent']))
			$t_trid .= '_parent_'.$row['idmenuparent'];
		...
		if($t_id == $r_key) {
			$rowe = $row;
			$rowe['namaparent'] = $a_namamenu[$rowe['idmenuparent']];
...
<div id="div_fix"<?= $conn->debug ? '' : ' style="position:relative"' ?>>
	<table width="305" cellpadding="4" cellspacing="0" class="GridStyle">
		...
		<tr>
			...
			<td class="RightColumnBG">
				<input type="hidden" name="idmenuparent" id="idmenuparent" value="<?= $rowe['idmenuparent'] ?>">
				<span id="span_parent"><?= empty($rowe['idmenuparent']) ? '(Tidak memiliki parent)' : $rowe['namaparent'] ?></span>
			</td>
		</tr>
		<tr>
			<td class="LeftColumnBG">Label Menu</td>
			<td class="RightColumnBG"><?= UI::createTextBox('namamenu',$rowe['namamenu'],'ControlStyle',50,30,$c_update) ?></td>
		</tr>
		<tr>
